...
class Tracker:
    ...
    def run_sequence(self, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None):
        ...
        #output = self._track_sequence_MU(tracker, seq, init_info)
        return output
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        #
        # heatmap
        # vis_path = os.path.join("/ssd2/gaoshang/visualization/mm2022/feature_vis_all", seq.name)
        # score_path = os.path.join(vis_path, 'scoremap')
        # impatch_path = os.path.join(vis_path, 'impatch')
        # depthpatch_path = os.path.join(vis_path, 'depth')
        # if not os.path.exists(score_path):
        #     os.makedirs(score_path)
        #     os.makedirs(impatch_path)
        ...
        # for frame_num, frame_path in enumerate(zip(seq.frames[1:], seq.depth_frames[1:]), start=1):
            ...
            # out, score_map, im_patches = tracker.track(image, info)
            # score_path = os.path.join(vis_path, 'scoremap',  "%08d.png"%(frame_num+1))
            # impatch_path = os.path.join(vis_path, 'impatch',  "%08d.png"%(frame_num+1))
            # depth_path = os.path.join(vis_path, 'depth',  "%08d.png"%(frame_num+1))
            # score_map = score_map
            # im_patches = im_patches[:3,:,:].T
            # depth_patches = im_patches[3:,:,:].T
            # try:
            #     score_map +=2
            #     # score_map[score_map<0] = 0 # ignore some large values,
            # except:
            #     score_map = score_map
            # score_map = cv.normalize(score_map, None, 0, 255, cv.NORM_MINMAX)
            # score_map = cv.applyColorMap(np.uint8(score_map), cv.COLORMAP_JET)

            # cv.imwrite(score_path, score_map)
            # cv.imwrite(impatch_path, cv.cvtColor(im_patches,cv.COLOR_RGB2BGR))
            #cv.imwrite(depth_path, cv.cvtColor(im_patches,cv.COLOR_RGB2BGR))
    ...
    def _track_sequence_MU(self, tracker, seq, init_info):
        # Define outputs
        # Each field in output is a list containing tracker prediction for each frame.
        ...
        image = self._read_image(seq.frames[0], dtype=seq.dtype)
        groundtruth = init_info['init_bbox']
        ...
        all_map[0] = cv.resize(score_map, (m_shape, m_shape))
        ...
        # for frame_num, frame_path in enumerate(zip(seq.frames[1:], seq.depth_frames[1:]), start=1):
            ...
            tic = time.time()
            image = self._read_image(frame_path, dtype=seq.dtype)
            print(seq.name + ": %d /" % frame_num + "%d" % len(seq.frames))
            ...
            #out = tracker.track(image, info)
            out = {'target_bbox': output_state, 'confidence': max_score, 'score_map': score_map}
            prev_output = OrderedDict(out)
            _store_outputs(out, {'time': time.time() - start_time})
    ...
    def run_vot2022rgbd(self, debug=None, visdom_info=None):
        params = self.get_parameters()
        params.tracker_name = self.name
        params.param_name = self.parameter_name
        params.run_id = self.run_id
        ...
        image_file = {}
        image_file['color'] = image_path[0]
        image_file['depth'] = image_path[1]
        image = self._read_image(image_file, dtype='rgb3d')
 

        # if output_segmentation:
        #     vot_anno_mask = vot.make_full_size(vot_anno, (color.shape[1], color.shape[0]))
        #     bbox = masks_to_bboxes(torch.from_numpy(vot_anno_mask), fmt='t').squeeze().tolist()
        # else:
        #     bbox = _convert_anno_to_list(vot_anno)
        #     vot_anno_mask = None
        ...
        out = tracker.initialize(image, info={'init_mask': vot_anno_mask, 'init_bbox': bbox})
        ...
        while True:
            ...
            segmentation = out['segmentation'] if 'segmentation' in out else None
            if self.visdom is not None:
                tracker.visdom_draw_tracking(color, out['target_bbox'], segmentation)
            elif tracker.params.visualization:
                self.visualize(color, out['target_bbox'], segmentation)
    
    ''' VOT-Depth Only 2022 Challenges '''
    def run_vot2022d(self, debug=None, visdom_info=None):
        params = self.get_parameters()
        params.tracker_name = self.name
        params.param_name = self.parameter_name
        params.run_id = self.run_id
        ...
        #     handle = vot.VOT("mask")
        # else:
        handle = vot.VOT("rectangle", channels='depth')
        ...
        image_path = _convert_image_path(image_path)
        # print(image_path)
        depth = self._read_image(image_path, dtype='depth')
        output_segmentation = False
        if output_segmentation:
            vot_anno_mask = vot.make_full_size(vot_anno, (depth.shape[1], depth.shape[0]))
            bbox = masks_to_bboxes(torch.from_numpy(vot_anno_mask), fmt='t').squeeze().tolist()
        else:
            bbox = _convert_anno_to_list(vot_anno)
            vot_anno_mask = None

        out = tracker.initialize(depth, info={'init_mask': vot_anno_mask, 'init_bbox': bbox})
        ...
        while True:
            ...
            out = tracker.track(depth, info=info)
            prev_output = OrderedDict(out)
            output_segmentation = False
            if output_segmentation:
                pred = out['segmentation'].astype(np.uint8)
            ...
            segmentation = out['segmentation'] if 'segmentation' in out else None
            if self.visdom is not None:
                tracker.visdom_draw_tracking(depth, out['target_bbox'], segmentation)
            elif tracker.params.visualization:
                self.visualize(depth, out['target_bbox'], segmentation)
    ...
    def run_vot(self, debug=None, visdom_info=None):
        ...
        # def _convert_anno_to_list(vot_anno):
        #     vot_anno = [int(vot_anno[0][0][0]), vot_anno[0][0][1], vot_anno[0][1][0], vot_anno[0][1][1],
        ...
        init_state = [vot_anno_polygon.x, vot_anno_polygon.y, vot_anno_polygon.width, vot_anno_polygon.height]
        # image_path = handle.frame()
        # imagefile, depth = handle.frame() 
        if not image_path:
            return
        
        image_file['color'] = image_file
        image_file['depth'] = depth

        image = self._read_image(image_path, dtype='rgb3d')
