...
class DiMPnet_DeT(nn.Module):
    ...
    def __init__(self, feature_extractor, feature_extractor_depth, classifier, bb_regressor, classification_layer, bb_regressor_layer,
        ...
        #template 
        self.featurefusion_layer2_template = build_featurefusion_network(d_model=256,dropout=0.1, nhead=8, dim_feedforward=1024,num_featurefusion_layers=1)
        self.featurefusion_layer3_template = build_featurefusion_network(d_model=256,dropout=0.1, nhead=8, dim_feedforward=1024,num_featurefusion_layers=1)
        self.RGB_template_down = nn.Conv2d(in_channels=1024, out_channels=256, kernel_size=1, stride=1, padding=0)
        self.D_template_down = nn.Conv2d(in_channels=1024, out_channels=256, kernel_size=1, stride=1, padding=0)
        self.template_up = nn.Conv2d(in_channels=256, out_channels=1024, kernel_size=1, stride=1, padding=0)
        self.template_RGB_layer2_conv = nn.Conv2d(in_channels=512, out_channels=256, kernel_size=1,stride=1, padding=0)
        self.template_D_layer2_conv = nn.Conv2d(in_channels=512, out_channels=256, kernel_size=1,stride=1, padding=0)
        self.template_layer2_up = nn.Conv2d(in_channels=256, out_channels=512, kernel_size=1,stride=1, padding=0)


        #template
        # self.featurefusion_layer2_search = build_featurefusion_network(d_model=512,dropout=0.1, nhead=8, dim_feedforward=1024,num_featurefusion_layers=2)
        # self.featurefusion_layer3_search = build_featurefusion_network(d_model=512,dropout=0.1, nhead=8, dim_feedforward=1024,num_featurefusion_layers=2)
        # self.RGB_search_down = nn.Conv2d(in_channels=1024, out_channels=512, kernel_size=1, stride=1, padding=0)
        # self.D_search_down = nn.Conv2d(in_channels=1024, out_channels=512, kernel_size=1, stride=1, padding=0)
        # self.search_up = nn.Conv2d(in_channels=512, out_channels=1024, kernel_size=1, stride=1, padding=0)
        # self.search_RGB_layer2_conv = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=1,stride=1, padding=0)
        # self.search_D_layer2_conv = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=1,stride=1, padding=0)
    ...
    def merge(self, color_feat, depth_feat):
        ...
        feat_RGB_layer2 = self.template_RGB_layer2_conv(color_feat['layer2'].tensors)
        feat_D_layer2 =  self.template_D_layer2_conv(depth_feat['layer2'].tensors)
        ...
        featurefusion_layer2 = self.featurefusion_layer2_template
        ...
        merged_feat['layer2'] = self.template_layer2_up(fusionfeat_layer2)

        #fusion layer3
        #down sampling 1024->512
        feat_RGB_layer3 = self.RGB_template_down(color_feat['layer3'].tensors)
        feat_D_layer3 = self.D_template_down(depth_feat['layer3'].tensors)
        ...
        featurefusion_layer3 = self.featurefusion_layer3_template
        ...
        merged_feat['layer3'] = self.template_up(fusionfeat_layer3)

        
        return merged_feat

    def merge1(self, color_feat, depth_feat):
        init_feat = {}
        feat = {}
        merged_feat = {}

        #the init fusion: layer2
        feat_RGB_layer2 = self.search_RGB_layer2_conv(color_feat['layer2'].tensors)
        feat_D_layer2 = self.search_D_layer2_conv(depth_feat['layer2'].tensors)
        init_feat['layer2'] = NestedTensor(torch.max(feat_RGB_layer2, feat_D_layer2), color_feat['layer2'].mask)
        pos1_layer2 = []
        pos1_layer2.append(self.position_encoding(init_feat['layer2']).to(torch.float32))
        ...
        #layer2 transformer fusion
        featurefusion_layer2 = self.featurefusion_layer2_search
        fusionfeat_layer2 = featurefusion_layer2(init_feat['layer2'].tensors, init_feat['layer2'].mask, feat['layer2'].tensors, feat['layer2'].mask, pos1_layer2[-1], pos2_layer2[-1])
        ...
        #fusion layer3
        #down sampling 1024->512
        feat_RGB_layer3 = self.RGB_search_down(color_feat['layer3'].tensors)
        feat_D_layer3 = self.D_search_down(depth_feat['layer3'].tensors)
        RGBm = color_feat['layer3'].mask
        dm = color_feat['layer3'].mask
        assert RGBm is not None
        assert RGBm.equal(dm)
        ...
        #layer3 transformer fusion
        featurefusion_layer3 = self.featurefusion_layer3_search
        fusionfeat_layer3 = featurefusion_layer3(init_feat['layer3'].tensors, init_feat['layer3'].mask, feat['layer3'].tensors, feat['layer3'].mask, pos1_layer3[-1], pos2_layer3[-1])
        #up sampling 512->1024
        merged_feat['layer3'] = self.search_up(fusionfeat_layer3)
