...
from ltr.models.neck.featurefusion_network_en_de import build_featurefusion_network
...
class DiMPnet_DeT(nn.Module):
    ...
    def __init__(self, feature_extractor, feature_extractor_depth, classifier, bb_regressor, classification_layer, bb_regressor_layer,
        ...
        #template 
        self.featurefusion_layer2_template = build_featurefusion_network(d_model=256,dropout=0.1, nhead=8, dim_feedforward=1024,num_featurefusion_layers=2)
        self.featurefusion_layer3_template = build_featurefusion_network(d_model=512,dropout=0.1, nhead=8, dim_feedforward=1024,num_featurefusion_layers=2)
        self.RGB_template_down = nn.Conv2d(in_channels=1024, out_channels=512, kernel_size=1, stride=1, padding=0)
        self.D_template_down = nn.Conv2d(in_channels=1024, out_channels=512, kernel_size=1, stride=1, padding=0)
        self.template_up = nn.Conv2d(in_channels=512, out_channels=1024, kernel_size=1, stride=1, padding=0)
        self.template_RGB_layer2_conv = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=1,stride=1, padding=0)
        self.template_D_layer2_conv = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=1,stride=1, padding=0)
