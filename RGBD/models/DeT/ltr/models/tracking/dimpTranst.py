...
class DiMPnet_DeT(nn.Module):
    ...
    def __init__(self, feature_extractor, feature_extractor_depth, classifier, bb_regressor, classification_layer, bb_regressor_layer,
        ...
        self.position_encoding = build_position_encoding(hidden_dim=512, position_embedding='sine')

        #template 
        self.featurefusion_layer2_template = build_featurefusion_network(d_model=512,dropout=0.1, nhead=8, dim_feedforward=1024,num_featurefusion_layers=2)
    ...
    def merge(self, color_feat, depth_feat):
        ...
        init_feat['layer2'] = NestedTensor(torch.max(color_feat['layer2'].tensors, depth_feat['layer2'].tensors), color_feat['layer2'].mask)
        pos1_layer2 = []
        pos1_layer2.append(self.position_encoding(init_feat['layer2']).to(torch.float32))

        #RGB+Depth information: layer2
        infor_layer2 = torch.cat((color_feat['layer2'].tensors, depth_feat['layer2'].tensors),dim=2)
        #clip mask 
