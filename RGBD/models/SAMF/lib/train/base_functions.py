...
from lib.train.dataset import MSCOCOSeq_depth, Lasot_depth, DepthTrack
...
def update_settings(settings, cfg):
    ...
    settings.input_type = cfg.DATA.INPUT_TYPE
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "COCO17", "VID", "TRACKINGNET",
                        "COCO_RGBD", "LaSOT_RGBD", "DepthTrack_train", "DepthTrack_val"], "unexpected name: {}".format(name)
        if name == "LASOT":
        ...
        if name == "COCO_RGBD":
            if settings.use_lmdb:
                ...
                datasets.append(MSCOCOSeq_depth(settings.env.coco_rgbd_dir, split="train", version="2017",
                                                dtype=settings.input_type, image_loader=image_loader))
        if name == "LaSOT_RGBD":
            if settings.use_lmdb:
                raise NotImplementedError
            else:
                datasets.append(Lasot_depth(settings.env.lasot_rgbd_dir,
                                            dtype=settings.input_type, image_loader=image_loader))
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_score:
        ...
        for n, p in net.named_parameters():
            if "stage2" not in n and "box_head" not in n and "stage1" not in n and "sa_gate" not in n:
                p.requires_grad = False
            else:
                if is_main_process():
                    print(n)
