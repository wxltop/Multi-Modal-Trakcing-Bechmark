...
class Lasot_depth(BaseVideoDataset):
    ...
    def _read_bb_anno(self, seq_path):
        bb_anno_file = os.path.join(seq_path, "color", "groundtruth.txt")
        gt = pandas.read_csv(bb_anno_file, delimiter=',', header=None, dtype=np.float32, na_filter=False, low_memory=False).values
        return torch.tensor(gt)

    def _read_target_visible(self, seq_path):
        # Read full occlusion and out_of_view
        occlusion_file = os.path.join(seq_path, "color", "full_occlusion.txt")
        out_of_view_file = os.path.join(seq_path, "color", "out_of_view.txt")
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        '''
        return os.path.join(seq_path, 'color', 'img', '{:08}.jpg'.format(frame_id+1)), os.path.join(seq_path, 'depth', '{:08}.png'.format(frame_id+1)) # frames start from 1
