...
from lib.models.mixformer.sa_gate import SAGate
...
class MixFormer(nn.Module):
    ...
    def __init__(self, backbone, backbone_depth, box_head, head_type="CORNER", merge_type="max"):
    ...
    def forward(self, template, online_template, search, run_score_head=False, gt_bboxes=None):
        ...
        template_color, search_color = self.backbone(template[:, :3, :, :], online_template[:, :3, :, :], search[:, :3, :, :])
        template_depth, search_depth = self.backbone_depth(template[:, 3:, :, :], online_template[:, 3:, :, :], search[:, 3:, :, :])
        template_merge_feat = self.merge(template_color, template_depth)
        search_merge_feat = self.merge(search_color, search_depth)
        # Forward the corner head
        return self.forward_box_head(search_merge_feat)
    ...
    def merge(self, color_feat, depth_feat):
        ...
        if self.merge_type == 'conv':
            feat = self.merge_layer(torch.cat((color_feat, depth_feat), 1))
            ...
            feat = self.weights[0] * color_feat + self.weights[1] * depth_feat
        elif self.merge_type == 'SAGate':
            feat = self.sa_gate(color_feat, depth_feat)
...
def build_mixformer(cfg, train=True):
    backbone = get_mixformer_model(cfg)  # backbone without positional encoding and attention mask
    backbone_depth = get_mixformer_model(cfg)  # backbone without positional encoding and attention mask
    ...
    if cfg.MODEL.PRETRAINED_MODEL_PATH and train:
        try:
            ckpt_path = cfg.MODEL.PRETRAINED_MODEL_PATH
            ckpt = torch.load(ckpt_path, map_location='cpu')
            missing_keys, unexpected_keys = model.load_state_dict(ckpt, strict=False)
            if is_main_process():
                print("[Loading pretrained weights for SAMixFormer]missing keys:", missing_keys)
                print("[Loading pretrained weights for SAMixFormer]unexpected keys:", unexpected_keys)
                print("Loading pretrained SAMixFormer weights done.")
        except:
            print("Warning: Pretrained SAMixFormer weights are not loaded")
    return model
