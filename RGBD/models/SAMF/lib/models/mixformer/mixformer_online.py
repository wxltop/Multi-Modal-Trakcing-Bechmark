...
class MixFormerOnlineScore(nn.Module):
    ...
    def __init__(self, backbone, backbone_depth, box_head, score_branch=None, head_type="CORNER", merge_type="max"):
        """ Initializes the model.
        """
        super().__init__()
    ...
    def forward(self, template, online_template, search, run_score_head=True, gt_bboxes=None):
        ...
        out, outputs_coord_new = self.forward_head(search_merge_feat, template_merge_feat, run_score_head, gt_bboxes)
    ...
    def forward_test(self, search, run_score_head=True, gt_bboxes=None):
        ...
        if search.dim() == 5:
            search = search.squeeze(0)
        template_color, search_color = self.backbone.forward_test(search[:, :3, :, :])
        template_depth, search_depth = self.backbone_depth.forward_test(search[:, 3:, :, :])
        template_merge_feat = self.merge(template_color, template_depth)
        search_merge_feat = self.merge(search_color, search_depth)
        # search (b, 384, 20, 20)
        # Forward the corner head and score head
