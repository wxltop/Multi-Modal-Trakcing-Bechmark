...
class Preprocessor_wo_mask(object):
    ...
    def process(self, img_arr: np.ndarray):
        # Deal with the image patch
        img_tensor = torch.tensor(img_arr).cuda().float().permute((2,0,1)).unsqueeze(dim=0)
        img_color_tensor_norm = ((img_tensor[:, :3, :, :] / 255.0) - self.mean) / self.std  # (1,3,H,W)
        img_depth_tensor_norm = ((img_tensor[:, 3:, :, :] / 255.0) - self.mean) / self.std  # (1,3,H,W)
        img_tensor_norm = torch.cat([img_color_tensor_norm, img_depth_tensor_norm], dim=1)
