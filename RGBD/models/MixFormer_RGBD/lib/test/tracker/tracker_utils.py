...
class Preprocessor_wo_mask(object):
    ...
    def process(self, img_arr: np.ndarray):
        # Deal with the image patch
        img_tensor = torch.tensor(img_arr).cuda().float().permute((2,0,1)).unsqueeze(dim=0)
        img_tensor_norm = ((img_tensor / 255.0) - self.mean) / self.std  # (1,3,H,W) or (1,6,H,W)
        return img_tensor_norm

class Preprocessor_wo_mask_rgbd(object):
    def __init__(self):
    ...
    def process(self, img_arr: np.ndarray):
        # Deal with the image patch
        assert img_arr.shape[2] == 6, "rgbd image array must has 6 channels"
        img_tensor = torch.tensor(img_arr).cuda().float().permute((2,0,1)).unsqueeze(dim=0)
        img_tensor_norm = ((img_tensor / 255.0) - self.mean) / self.std  # (1,3,H,W) or (1,6,H,W)
        return img_tensor_norm
...
def re_constraint_simple(prev_state, curr_state):
    prev_x, prev_y, prev_w, prev_h = prev_state
    curr_x, curr_y, curr_w, curr_h = curr_state
    prev_avg_wh = (prev_w + prev_h) / 2
    prev_cx, prev_cy = prev_x + prev_w / 2, prev_y + prev_h / 2
    curr_cx, curr_cy = curr_x + curr_w / 2, curr_y + curr_h / 2
    dis_avg_cxy = abs(curr_cx - prev_cx) + abs(curr_cy - prev_cy)
    # suppress large displacement
    if dis_avg_cxy > 2 * prev_avg_wh:
        state = prev_state
    else:
        state = curr_state
    return state
...
def re_constraint_complex(prev_state, curr_state, curr_score, his_w, his_h, conf_thr=0.5, scale_thr=1.35):
    prev_x, prev_y, prev_w, prev_h = prev_state
    curr_x, curr_y, curr_w, curr_h = curr_state
    prev_avg_wh = (prev_w + prev_h) / 2
    prev_cx, prev_cy = prev_x + prev_w / 2, prev_y + prev_h / 2
    ...
    if dis_avg_cxy > 2 * prev_avg_wh:
        state = prev_state
    elif (dis_avg_cxy > 1.5 * prev_avg_wh) and curr_score < conf_thr:
        state = prev_state
    elif max(change(curr_w/his_w), change(curr_h/his_h)) > scale_thr:
        state = prev_state
    else:
        state = curr_state
    return state
