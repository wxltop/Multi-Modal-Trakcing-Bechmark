...
import cv2
import os
from lib.models.mixformer import build_mixformer_rgbd_online_score
from lib.test.tracker.tracker_utils import Preprocessor_wo_mask_rgbd, AverageMeter
from lib.utils.box_ops import clip_box
from lib.test.tracker.tracker_utils import vis_attn_maps, re_constraint_simple, re_constraint_complex


class MixFormerRGBDOnline(BaseTracker):
    def __init__(self, params, dataset_name):
        super(MixFormerRGBDOnline, self).__init__(params)
        network = build_mixformer_rgbd_online_score(params.cfg, train=False)  # False mean no load of stage1 ckpt
        ...
        self.preprocessor = Preprocessor_wo_mask_rgbd()
        self.state = None
        # for debug
        self.debug = params.debug if hasattr(params, 'debug') else 0
        self.frame_id = 0
        if self.debug:
            self.save_dir = "debug"
        ...
        """2022/4/30 add limitations"""
        self.his_w = AverageMeter()
        self.his_h = AverageMeter()

    def initialize(self, image, info: dict):
        # forward the template once
        """2022.4.23 testify sample_target can input (h,w,6) rgbcolormap"""
        ...
        self.his_w.update(self.state[2])
        self.his_h.update(self.state[3])
    ...
    def track(self, image, info: dict = None):
        ...
        pred_boxes = out_dict['pred_boxes'].view(-1, 4)  # (b, 4) b due to multiple templates
        ...
        """2022/4/30 add some limitation"""
        curr_state = self.state
        if self.cfg.TEST.RE_CONSTRAIN_TYPE == 'simple':
            self.state = re_constraint_simple(prev_state, curr_state)
        else:
            self.state = re_constraint_complex(prev_state, curr_state, pred_score, self.his_w.avg, self.his_h.avg)
        self.his_w.update(self.state[2])
        self.his_h.update(self.state[3])
        ...
        if pred_score > 0.5 and pred_score > self.max_pred_score:
            z_patch_arr, _, _ = sample_target(image, self.state,
