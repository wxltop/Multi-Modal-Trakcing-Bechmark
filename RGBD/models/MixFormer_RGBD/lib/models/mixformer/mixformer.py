...
class ConvolutionalVisionTransformer(nn.Module):
    def __init__(self,
        ...
        self.cls_token = spec['CLS_TOKEN'][-1] # false
    ...
    if config.MODEL.BACKBONE.PRETRAINED:
        try:
            ...
            if is_main_process():
                ...
                # missing keys: []
                # unexpected keys: ['stage2.cls_token']
        except:
...
class MixFormer(nn.Module):
    ...
    def forward(self, template, online_template, search, run_score_head=False, gt_bboxes=None):
        # template: (b, 3, h, w)  online_template:(b, 3, h, w)  search: (Ns, b, 3, H, W)
        # gt_bboxes: (b, 4)
        if template.dim() == 5:
...
if __name__ == "__main__":
    import importlib
    config_module = importlib.import_module("lib.config.%s.config" % 'mixformer')
    cfg = config_module.cfg
    config_module.update_config_from_file('/home/lz/PycharmProjects/MixFormer/experiments/mixformer/baseline_debug.yaml')
    backbone = get_mixformer_model(cfg)
    print(backbone)