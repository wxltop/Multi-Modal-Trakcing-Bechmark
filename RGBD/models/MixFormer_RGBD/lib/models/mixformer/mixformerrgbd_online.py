...
from lib.models.mixformer.head import build_box_head, MLP
...
class Attention(nn.Module):
    ...
    def set_online(self, x, t_h, t_w):
        ...
        template = rearrange(template, 'b (h w) c -> b c h w', h=t_h, w=t_w).contiguous()  # 1, c, 32, 32
        online_template = rearrange(online_template.squeeze(0), '(b h w) c -> b c h w', h=t_h, w=t_w).contiguous()  # b, c, 32, 32
        if self.conv_proj_q is not None:
            t_q = self.conv_proj_q(template)
            ot_q = self.conv_proj_q(online_template).flatten(end_dim=1).unsqueeze(0) # 1, b*1024, c
        else:
            t_q = rearrange(template, 'b c h w -> b (h w) c').contiguous()
            ot_q = rearrange(online_template, 'b c h w -> (b h w) c').contiguous().unsqueeze(0)
        q = torch.cat([t_q, ot_q], dim=1) # 1, (b+1)*1024, c
...
# each stage is an instance of VisionTransformer
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.pos_drop = nn.Dropout(p=drop_rate) # p – probability of an element to be zeroed
...
class MixFormerRGBDOnlineScore(nn.Module):
    """ Mixformer tracking with score prediction module, whcih jointly perform feature extraction and interaction. """
    def __init__(self, backbone, backbone_depth, box_head, score_branch=None, head_type="CORNER", merge_type='mean'):
        """ Initializes the model.
        """
        super().__init__()
    ...
    def forward(self, template, online_template, search, run_score_head=True, gt_bboxes=None):
        ...
        if search.dim() == 5:
            ...
            # now three inputs are all 4-D tensor
        dims = template.shape
        if dims[1] == 6:
            template_rgb, search_rgb = self.backbone(template[:, :3], online_template[:, :3], search[:, :3])
            template_depth, search_depth = self.backbone_depth(template[:, 3:], online_template[:, 3:], search[:, 3:])
            template = self.merge(template_rgb, template_depth)
            search = self.merge(search_rgb, search_depth)
        else:
    ...
    def forward_test(self, search, run_score_head=True, gt_bboxes=None):
        ...
        dims = search.shape
        if dims[1] == 6:
            template_rgb, search_rgb = self.backbone.forward_test(search[:, :3])
            template_dp, search_dp = self.backbone_depth.forward_test(search[:, 3:])
            template = self.merge(template_rgb, template_dp)
            search = self.merge(search_rgb, search_dp)
        else:
    ...
    def set_online(self, template, online_template):
        ...
        # backbone set_online will generate self.template prepared for forward_test
        dims = template.shape
        if dims[1] == 6:
            self.backbone.set_online(template[:, :3], online_template[:, :3])
            self.backbone_depth.set_online(template[:, 3:], online_template[:, 3:])
        else:
    ...
    def merge(self, color_feat, depth_feat):
        ...
        if self.merge_type == 'max':
            # feat = torch.maximum(color_feat, depth_feat)
            feat = torch.max(color_feat, depth_feat)  # pytorch1.5

        elif self.merge_type == 'mul':
            feat = torch.mul(color_feat, depth_feat)

        elif self.merge_type == 'mean':
            feat = 0.5 * color_feat + 0.5 * depth_feat

        else:
            raise NotImplementedError("Wrong merge type!")
        return feat

def build_mixformer_rgbd_online_score(cfg, settings=None, train=True):
    backbone = get_mixformer_online_model(cfg)  # backbone without positional encoding and attention mask
    backbone_depth = get_mixformer_online_model(cfg)  # backbone without positional encoding and attention mask
    ...
    model = MixFormerRGBDOnlineScore(
        backbone,
        backbone_depth,
        box_head,
        score_branch,
        head_type=cfg.MODEL.HEAD_TYPE,
        merge_type=cfg.MODEL.MERGE_TYPE
    )
    ...
    if cfg.MODEL.PRETRAINED_STAGE1 and train:
        try:
            ...
            if is_main_process():  # should only score_branch
                print("missing keys:", missing_keys)
                print("unexpected keys:", unexpected_keys)
...
if __name__ == "__main__":
    import importlib
    config_module = importlib.import_module("lib.config.%s.config" % 'mixformer_online')
    cfg = config_module.cfg
    config_module.update_config_from_file('/home/lz/PycharmProjects/MixFormer/experiments/mixformer_online/baseline_debug.yaml')
    backbone = get_mixformer_online_model(cfg)
    t = torch.randn((1,3,128,128))
    s = torch.randn((1,3,320,320))
    backbone.set_online(t, t)
    # backbone(t, t, s)
    print(backbone.template.shape) # 1, 384, 8, 8