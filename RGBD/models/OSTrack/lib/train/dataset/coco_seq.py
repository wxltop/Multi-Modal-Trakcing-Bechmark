...
from .lan_utils import init_tokenizer, text2id_mask
...
class MSCOCOSeq(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, data_fraction=None, split="train", version="2014", bert_model=None):
        """
        args:
        ...
        if 'ref' in split:
            assert version == "2014"
            self.img_pth = os.path.join(root, 'images/{}{}/'.format(split.split('_')[0], version))
    ...
    def get_sequence_info(self, seq_id):
        ...
        if self.tokenizer is not None:
            nlp_dsp = self.coco_set.imgs[anno['image_id']]['caption']
            word_id, word_mask = text2id_mask(nlp_dsp, self.tokenizer, self.query_len)
            word_id = torch.tensor(word_id, dtype=torch.int).unsqueeze(dim=0)
            word_mask = torch.tensor(word_mask, dtype=torch.int).unsqueeze(dim=0)

            return {'bbox': bbox, 'valid': valid, 'visible': visible,
                    'word_id': word_id, 'word_mask': word_mask}
