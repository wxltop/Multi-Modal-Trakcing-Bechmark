...
class Got10k(BaseVideoDataset):
    ...
    def _get_seg(self, seq_path, frame_id):
        os.path.join(seq_path, 'seg_mask', '{:08}.npy'.format(frame_id + 1))

    def get_frames(self, seq_id, frame_ids, anno=None, return_seg=False):
        seq_path = self._get_sequence_path(seq_id)
        obj_meta = self.sequence_meta_info[self.sequence_list[seq_id]]
        ...
        if return_seg:
            ...
            seg_list = [os.path.join(seq_path, 'seg_mask', '{:08}.png'.format(f_id + 1)) for f_id in frame_ids]
            seg_list = [np.asarray(Image.open(img_file))[None, ...] for img_file in seg_list]
            seg_list = [torch.from_numpy(img_file.copy()).to(torch.float) for img_file in seg_list]
            return frame_list, anno_frames, obj_meta, seg_list
