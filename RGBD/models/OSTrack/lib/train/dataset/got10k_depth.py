...
class Got10k_depth(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, dtype='rgbcolormap', split=None, seq_ids=None, data_fraction=None):
        """
        args:
        ...
        self.depth_root = '/public/yebotao/datasets/depth_gen/got10k'
    ...
    def _get_sequence_path_depth(self, seq_id):
        return os.path.join(self.depth_root, self.sequence_list[seq_id])
    ...
    def _get_frame_path(self, seq_path, seq_path_depth, frame_id):
        # return os.path.join(seq_path, '{:08}.jpg'.format(frame_id+1))    # frames start from 1
        return os.path.join(seq_path, '{:08}.jpg'.format(frame_id+1)), os.path.join(seq_path_depth, '{:08}.png'.format(frame_id+1))    # frames start from 1

    def _get_frame(self, seq_path, seq_path_depth, frame_id):
        # return self.image_loader(self._get_frame_path(seq_path, frame_id))
        color_path, depth_path = self._get_frame_path(seq_path, seq_path_depth, frame_id)
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        seq_path_depth = self._get_sequence_path_depth(seq_id)

        frame_list = [self._get_frame(seq_path, seq_path_depth, f_id) for f_id in frame_ids]
