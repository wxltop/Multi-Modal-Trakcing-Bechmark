    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:
            ...
            if dims[2] == 6:
                ...
                depth_gray = cv.cvtColor(depth, cv.COLOR_RGB2GRAY)
                depth_gray = np.stack([depth_gray, depth_gray, depth_gray], axis=2)

                image = np.concatenate([img_gray, depth_gray], axis=2)
