...
class STARKProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            if 'template_seg' in data and 'search_seg' in data:
                data['template_seg'] = self.transform['joint'](mask=data['template_seg'], new_roll=False)
                data['search_seg'] = self.transform['joint'](mask=data['search_seg'], new_roll=False)
        ...
        for s in ['template', 'search']:
            ...
            if s + '_seg' in data:
                seg_masks = [torch.from_numpy(seg_mask.squeeze()).to(torch.float) for seg_mask in data[s + '_seg']]
                crops_resize_factors = [prutils.sample_target(f, a, self.search_area_factor[s], self.output_sz[s], m)
                                        for f, a, m in zip(data[s + '_images'], jittered_anno, seg_masks)]

                _, _, _, seg_crops = zip(*crops_resize_factors)
            ...
            if s + '_seg' in data:
                data[s + '_seg'] = self.transform[s](mask=seg_crops, new_roll=False)
