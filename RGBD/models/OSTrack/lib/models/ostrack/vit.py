...
from lib.models.ostrack.layers import to_2tuple
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, return_attention=False):
        if return_attention:
            feat, attn = self.attn(self.norm1(x), mask, True)
            x = x + self.drop_path(feat)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x, attn
        else:
            # x = x + self.drop_path(self.attn(self.norm1(x), mask, True)[0])
...
class VisionTransformer(BaseBackbone):
    ...
    def no_weight_decay(self):
        return {'pos_embed', 'cls_token', 'dist_token', 'pos_embed_z', 'pos_embed_x'}
    ...
    def finetune_track(self, search_size=[320, 320], template_size=[128, 128], det_token_num=1,
        ...
        self.cls_mode = cfg.MODEL.BACKBONE.CLS_MODE
        ...
        if self.msg:
            self.msg_tokens_z = nn.Parameter(self.cls_token.clone())
            self.msg_tokens_x = nn.Parameter(self.cls_token.clone())
            self.pos_embed_msg_z = nn.Parameter(cls_pos_embed.clone())
            self.pos_embed_msg_x = nn.Parameter(cls_pos_embed.clone())

        # for patch embedding
        patch_pos_embed = self.pos_embed[:, 1:, :]
        # patch_pos_embed = self.pos_embed[:, 1:-100, :]
