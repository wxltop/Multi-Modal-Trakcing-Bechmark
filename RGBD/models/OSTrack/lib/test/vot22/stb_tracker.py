...
class ostrack(object):
    ...
    def track(self, img_rgb, f1_reg=False):
        # track
        outputs = self.tracker.track(img_rgb, f1_reg=f1_reg)
        pred_bbox = outputs['target_bbox']
        final_mask = outputs['mask_pred']
...
def run_vot_exp(tracker_name, para_name, vis=False, f1_reg=False):
    torch.set_num_threads(1)
    save_root = os.path.join('/public/yebotao/tmp', para_name)
    ...
    while True:
        ...
        if not imagefile:
            break
        image = cv2.cvtColor(cv2.imread(imagefile), cv2.COLOR_BGR2RGB)  # Right
        b1, m = tracker.track(image, f1_reg)
        x1, y1, w, h = b1
