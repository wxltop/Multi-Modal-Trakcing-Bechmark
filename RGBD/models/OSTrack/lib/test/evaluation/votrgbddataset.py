    ...
    def _construct_sequence(self, sequence_name):
        ...
        if self.dtype in ['rgbd', 'rgbcolormap', 'rgb3d']:
            ...
            frames = ['{base_path}/{sequence_path}/{group}/{frame:0{nz}}.{ext}'.format(base_path=self.base_path,
                      ...
                      for frame_num in range(start_frame, end_frame+1)]
            if self.dtype == 'color':
                frames = [{'color': c_path, 'depth': None} for c_path in frames]
            else:
                frames = [{'color': None, 'depth': d_path} for d_path in frames]
        ...
        return Sequence(sequence_name, frames, 'vot22_rgbd', ground_truth_rect)
