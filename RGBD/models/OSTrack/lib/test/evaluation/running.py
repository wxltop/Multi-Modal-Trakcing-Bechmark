...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['trackingnet', 'got10k'] and 'val' not in tracker.dataset_name:
    ...
    def save_num_candidates(file, data):
        # num_candidates = np.array(data).astype(int)
        # np.savetxt(file, num_candidates, delimiter='\t', fmt='%d')
        num_candidates = np.array(data).astype(float)
        np.savetxt(file, num_candidates, delimiter='\t', fmt='%.6f')
    ...
    for key, data in output.items():
        ...
        if key == 'num_candidates':
            if isinstance(data[0], dict):
                ...
                for obj_id, d in data_dict.items():
                    num_candidates_file = '{}_{}_num_candidates.txt'.format(base_results_path, obj_id)
                    save_num_candidates(num_candidates_file, d)
            else:
                num_candidates_file = '{}_num_candidates.txt'.format(base_results_path)
                save_num_candidates(num_candidates_file, data)
