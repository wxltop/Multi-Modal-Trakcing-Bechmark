...
'''2020.4.22 only use the mask dataset and the mask branch'''
'''2020.4.22 increase the input resolution to 384x384'''
...
from ltr.dataset import Youtube_VOS, Saliency, MSCOCOSeq17
...
def run(settings):
    ...
    settings.sample_per_epoch = 4000 * 2  # 由于batchsize比原来更小了,所以我把这个值又翻了一倍
    ...
    dataset_train = SEsampler.SEMaskSampler([youtube_vos, saliency, coco17_train],
                                            [1, 1, 1],
    ...
    net = AR_seg_mask.ARnet_seg_mask_resnet50(backbone_pretrained=True, used_layers=settings.used_layers,
                                              target_sz=int(settings.feature_sz / 2))  # taregt size is half of the feature sz
