...
# from ltr.dataset import Youtube_VOS, Saliency, MSCOCOSeq17, Got10k_mask, Lasot_mask
from ltr.dataset import MSCOCOSeq17_lmdb, Saliency_lmdb, Youtube_VOS_lmdb, Got10k_mask_lmdb, Lasot_mask_lmdb
...
def run(settings):
    ...
    lasot_mask = Lasot_mask_lmdb(split="train")
    # coco17_train = MSCOCOSeq17('train')
    # saliency = Saliency()
    # youtube_vos = Youtube_VOS()
    # got10k_mask = Got10k_mask(split="vottrain")
    # lasot_mask = Lasot_mask(split="train")

    # The sampler for training
    '''Build training dataset. focus "__getitem__" and "__len__"'''
    dataset_train = SEsampler.SEMaskSampler([youtube_vos, saliency, coco17_train, got10k_mask, lasot_mask],
                                            [1, 1, 1, 1, 1],
