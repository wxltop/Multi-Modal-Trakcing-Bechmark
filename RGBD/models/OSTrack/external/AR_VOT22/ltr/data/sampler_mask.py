...
class ATOMSampler(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        while not enough_visible_frames:
            ...
            '''似乎这里强行限制了visible frames数量要大于20,
            但是对于Youtube-VOS这种视频长度很短的,这个限制可能不太合适'''
            ...
            enough_visible_frames = ((not is_video_dataset) and num_visible > 0) or \
                                (num_visible > min_visible_frames and (not dataset.has_mask()) and len(
                                    visible) >= 20) or \
        ...
        if not dataset.has_mask():
            ...
            train_frames, _, train_anno_dict, _ = dataset.get_frames(seq_id, train_frame_ids, seq_info_dict)
            test_frames, _, test_anno_dict, _ = dataset.get_frames(seq_id, test_frame_ids, seq_info_dict)
...
class DiMPSampler(torch.utils.data.Dataset):
    """ Class responsible for sampling frames from training sequences to form batches. Each training sample is a
