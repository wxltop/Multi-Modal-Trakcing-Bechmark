...
'''2020.4.22 We introduce Distance Transformation map to help segmentation'''
class ARdtProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
        ...
        self.output_sz = int(output_sz)
        self.center_jitter_factor = center_jitter_factor
        self.scale_jitter_factor = scale_jitter_factor
        self.mode = mode
        '''2020.4.22'''
        self.get_dt_template()
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['train', 'test']:
            ...
            data[s + '_images'] = [self.transform[s](x) for x in crops] # [torch tensor (3,256,256)]
            data[s + '_anno'] = boxes # [torch tensor (4,)]

        '''changed on 2020.1.4'''
        # if data['mask']:
        mask_crops = prutils_SE.jittered_center_crop_SE(data['test_masks'], jittered_anno, data[s + '_anno'],
                                                      self.search_area_factor, self.output_sz, get_bbox_coord=False, mode=cv.BORDER_CONSTANT)
        data['test_masks'] = [self.mask_np2torch(x) for x in mask_crops]
        '''2020.4.22 add distance transformation map'''
        data['test_dtm'] = [self.coord2dt(data['test_anno'])]
        # Prepare output
    ...
    '''2020.4.22 We introduce Distance Transformation map to help segmentation'''
    def get_dt_template(self, stride = 16):
        map_size = self.output_sz // stride
        index_tensor = torch.tensor(range(map_size)).view((-1,1)).type(torch.float32)
        # self.dtm_x = index_tensor.repeat((1,map_size))
        # self.dtm_y = index_tensor.transpose(0,1).repeat((map_size,1))
        '''2020.4.26 晚上我发现之前把x,y的顺序弄反了'''
        self.dtm_y = index_tensor.repeat((1,map_size))
        self.dtm_x = index_tensor.transpose(0,1).repeat((map_size,1))

    def coord2dt(self, coord_list: list, stride = 16):
        '''transform coordinate to distance maps'''
        '''get original center coord'''
        coord = coord_list[0] / stride  # torch tensor (4,) coord on the feature map
        x1, y1, w, h = coord[0], coord[1], coord[2], coord[3]
        cx = x1 + 0.5 * w  # x1 + 0.5 * w
        cy = y1 + 0.5 * h  # y1 + 0.5 * y
        '''Add some perturbation'''
        delta_x = 1/8 * w * 2 * (torch.rand(1) - 0.5)
        delta_y = 1/8 * h * 2 * (torch.rand(1) - 0.5)
        cx_new = cx + delta_x
        cy_new = cy + delta_y
        '''Tranform it to distance maps'''
        dtm = torch.sqrt((self.dtm_x - cx_new)**2 + (self.dtm_y - cy_new)**2)
        '''add a dim'''
        dtm_3d = dtm.unsqueeze(0) # (1,size,size)
        return dtm_3d
# if __name__ == '__main__':
    map_size = 16
    index_tensor = torch.tensor(range(map_size)).view((-1, 1)).type(torch.float32)
    # dtm_x = index_tensor.repeat((1, map_size))
    # dtm_y = index_tensor.transpose(0, 1).repeat((map_size, 1))
    '''2020.4.26 晚上 发现之前把x,y的顺序弄反了'''
    dtm_x = index_tensor.repeat((1, map_size))
    dtm_y = index_tensor.transpose(0, 1).repeat((map_size, 1))
    # print(dtm_x.size()) # (16,16)
    # print(dtm_y.size()) # (16,16)
    # print(torch.rand(1))
