...
class BaseTrainer:
    ...
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None):
        ...
        '''就是在这块,作者强行把device定死了,我们应该把它纠正过来'''
        # if self.device is None:
        # self.device = torch.device("cuda:"+str(settings.local_rank))
            ...
            try:
                ...
                for epoch in range(self.epoch + 1, max_epochs + 1):
                    ...
                    if self._checkpoint_dir:
                        '''train scale estimator'''
                        # '''这块才是关键,虽然我们开多个进程训练,但是只能用一个进程(主进程)来保存数据
                        # 不然会引起数据冲突,导致在读取的时候出错...'''
    ...
    def save_checkpoint(self):
        ...
        '''with selector'''
        # net = self.actor.selector.module if multigpu.is_multi_gpu(self.actor.selector) else self.actor.selector
