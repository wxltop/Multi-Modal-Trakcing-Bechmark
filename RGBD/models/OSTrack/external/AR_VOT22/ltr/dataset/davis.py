...
class DAVIS(BaseDataset):
    ...
    def __init__(self, root=None, image_loader=default_image_loader, min_length=0, max_target_area=1):
        ...
        if os.path.isfile(cache_file):
            ...
            self.sequence_list = self.convert_davis(root)
    ...
    def _get_frame(self, sequence, frame_id):
        ...
        frame_path = join(self.root, 'JPEGImages/480p', frame_name + '.jpg')
        frame_img = self.image_loader(frame_path)
        '''mask 1 channel'''
        mask_path = join(self.root,'Annotations/480p',frame_name+'.png')
    ...
    def convert_davis(self, data_dir):
        anno_root = os.path.join(data_dir,'Annotations/480p')
        video_list = sorted(os.listdir(anno_root))
        num_obj = 0
        num_ann = 0
        ann_list = []
        for v_idx, video_name in enumerate(video_list):
            vid_anno_dir = os.path.join(anno_root,video_name)
            anno_path_list = sorted(glob.glob(os.path.join(vid_anno_dir,'*.png')))
            annotations = []
            instanceIds = []
            for idx, anno_path in enumerate(anno_path_list):
                img = cv2.imread(anno_path,0)
                h, w = img.shape[:2]
                objects = dict()
                ...
                for objId in objects:
                    ...
                    ann['h'] = h
                    ann['w'] = w
                    file_name = os.path.join(video_name,'%05d'%idx)
            ...
            if v_idx % 50 == 0 and v_idx != 0:
                print("process: %d video" % (v_idx + 1))
