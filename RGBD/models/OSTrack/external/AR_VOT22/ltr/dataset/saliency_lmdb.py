...
import cv2
import numpy as np
from lib.utils.lmdb_utils import decode_json, decode_img
...
class Saliency_lmdb(BaseDataset):
    """ Saliency dataset.
    """
    ...
    def __init__(self, root=None, image_loader=default_image_loader, min_length=0, max_target_area=1):
        ...
        """
        root = env_settings().saliency_lmdb_dir if root is None else root
        super().__init__(root, image_loader)
        print("building saliency from lmdb")
        self.sequence_dict = decode_json(root, "cache.json")
    ...
    def _get_frame(self, seq_id):
        ...
        mask = cv2.cvtColor(decode_img(self.root, os.path.join('gt', gt_name)),
                            cv2.COLOR_RGB2GRAY)
        cv2.imwrite("mask.png", mask)
        frame = decode_img(self.root, os.path.join('images', frame_name))
