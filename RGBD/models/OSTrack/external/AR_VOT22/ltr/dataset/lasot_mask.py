...
class Lasot_mask(BaseDataset):
    ...
    def __init__(self, root=None, image_loader=default_image_loader, vid_ids=None, split=None, data_fraction=None):
        ...
        args:
            ...
            data_fraction (None) - Fraction of videos to be used. The videos are selected randomly. If None, all the
                                   videos will be used
        """
        print("building lasot mask dataset")
        root = env_settings().lasot_dir if root is None else root
        super().__init__(root, image_loader)
        self.mask_root = env_settings().lasot_mask_dir
    ...
    def _get_mask(self, seq_id, frame_id):
        ...
        class_name = seq_name.split('-')[0]
        vid_id = seq_name.split('-')[1]
        seq_path = os.path.join(self.mask_root, class_name, class_name + '-' + vid_id)
        mask_path = os.path.join(seq_path, "%08d.jpg" % (frame_id + 1))
