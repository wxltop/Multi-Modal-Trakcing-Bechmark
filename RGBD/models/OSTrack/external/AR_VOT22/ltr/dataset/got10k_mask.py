...
class Got10k_mask(BaseDataset):
    """ GOT-10k mask dataset"""
    ...
    def __init__(self, root=None, image_loader=default_image_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        args:
            ...
            data_fraction (None) - Fraction of videos to be used. The videos are selected randomly. If None, all the videos
                                   will be used
        """
        print("building got10k mask dataset")
        root = env_settings().got10k_dir if root is None else root
        super().__init__(root, image_loader)
        self.mask_root = env_settings().got10k_mask_dir
    ...
    def _get_mask(self, seq_id, frame_id):
        seq_mask_dir = os.path.join(self.mask_root, "train", self.sequence_list[seq_id])
        mask_img_path = os.path.join(seq_mask_dir, "%08d.jpg" % (frame_id + 1))
