...
class STARK_ALPHA_SEG(object):
    def __init__(self, tracker_name='stark_s', para_name='baseline',
                 refine_model_name='ARcm_coco_seg', threshold=0.15, trt='false'):
        ...
        refine_root = os.path.join(project_path, 'checkpoints/ltr/ARcm_seg/')
    ...
    def initialize(self, img_RGB, mask):
        ...
        self.alpha.initialize(img_RGB, np.array(gt_bbox_np), trt=self.trt)
    ...
    def track(self, img_RGB):
        ...
        '''Step2: Mask report'''
        pred_mask, search, search_mask = self.alpha.get_mask(img_RGB, np.array(pred_bbox), vis=True, trt=self.trt)
        ...
        # constrain mask
        final_mask = constraint_mask(final_mask, pred_bbox)
        return pred_bbox, final_mask, search_region, search_mask


def run_vot_exp(tracker_name, para_name, refine_model_name, threshold, VIS=False, trt='false'):
    ...
    tracker = STARK_ALPHA_SEG(tracker_name=tracker_name, para_name=para_name,
                              refine_model_name=refine_model_name, threshold=threshold, trt=trt)
    handle = vot.VOT("mask")
