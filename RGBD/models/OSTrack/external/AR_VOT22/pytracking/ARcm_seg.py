...
class ARcm_seg(object):
    ...
    def initialize(self, frame1, bbox1, trt='false'):
        """
        ...
        '''Step2: forward prop (reference branch)'''
        with torch.no_grad():
            self.refine_network.forward_ref(patch1_tensor, trt=trt)


    '''refine'''

    def get_mask(self, Cframe, Cbbox, dtm=None, vis=False, trt='false'):
        """
        ...
        with torch.no_grad():
            if dtm is not None:
                ...
                pred = self.refine_network.forward_test(Cpatch_tensor, dtm, trt=trt, mode='mask')
            else:
                pred = self.refine_network.forward_test(Cpatch_tensor, trt=trt, mode='mask')
    ...
    def predict_ego_mask(self, Cframe, Cbbox):
        """2021.04.13 predict masks given image and gt"""
        # crop search region
        Cpatch, h_f, w_f = sample_target_SE(Cframe, Cbbox, self.search_factor, self.input_sz, mode=cv2.BORDER_CONSTANT)
        Cpatch_tensor = self.img_preprocess(Cpatch)
        with torch.no_grad():
            pred = self.refine_network.forward_ego(Cpatch_tensor)
            Pmask_arr = mask_torch2numpy(pred)
            mask_arr = map_mask_back(Cframe, Cbbox, self.search_factor, Pmask_arr, mode=cv2.BORDER_CONSTANT)
        return mask_arr
    ...
    def init_tensorrt(self):
        ...
        self.test_AR_mask_backbone_trt.load_state_dict(
            torch.load(os.path.dirname(os.path.abspath(__file__)) + '/../checkpoints/trt_models/test_AR_backbone_trt.pth'))
