...
'''stark_vot22rgbd class'''


class stark_vot22_rgbd(object):
    def __init__(self, tracker_name='stark_s', para_name='rgbd'):
        # create tracker
...
def run_vot_exp(tracker_name, para_name, vis=False):
    ...
    tracker = stark_vot22_rgbd(tracker_name=tracker_name, para_name=para_name)
    ...
    image_depth = cv2.cv2.imread(imagefile[1], -1)
    ...
    tracker.initialize(img, init_box)
