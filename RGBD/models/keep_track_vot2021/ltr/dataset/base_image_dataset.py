...
class BaseImageDataset(torch.utils.data.Dataset):
    ...
    def get_images(self, image_ids, annos=None):
        if annos is None:
            outputs = [self.get_image(im_id) for im_id in image_ids]
        else:
            outputs = [self.get_image(im_id, anno) for im_id, anno in zip(image_ids,annos)]

        images, annos, object_meta = zip(*outputs)
        return images, annos, object_meta
