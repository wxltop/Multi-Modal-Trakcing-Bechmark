...
import random
from .layers import shift_features


class CVIndProcv1(nn.Module):
    def __init__(self, cost_volume, predictor, pool_before_cv=True, fix_coordinate_shift=False):
        super().__init__()

        self.cost_volume = cost_volume
        self.predictor = predictor
        self.pool_before_cv = pool_before_cv
        self.fix_coordinate_shift = fix_coordinate_shift
    ...
    def extract_motion_feat(self, backbone_feat):
        ...
        if self.pool_before_cv:
            backbone_feat = F.max_pool2d(backbone_feat, kernel_size=3, stride=2, padding=1)
        return backbone_feat

    def predict_response(self, data, dimp_thresh=None, pool_dimp_score=False, output_window=None, cost_volume_window=None):
        feat1 = data['feat1']
        feat2 = data['feat2']
        label_prev = data.get('label_prev', None)
        ...
        input_shape = dimp_score_cur.shape
        ...
        if cost_volume_window is not None:
            cost_volume_window = cost_volume_window.view(cost_volume.shape).to(cost_volume.device)
            cost_volume = cost_volume * cost_volume_window
        ...
        pred_response, state_new, auxiliary_outputs = self.predictor(cost_volume, state_prev, dimp_score_cur, label_prev, dimp_thresh, pool_dimp_score, output_window)
