...
class CostVolumeFull(nn.Module):
    """
    ...
    """
    def __init__(self):
        super(CostVolumeFull, self).__init__()

    def forward(self, feat1, feat2):
        b, c, h, w = feat1.size()

        # reshape features for matrix multiplication
        feat1 = feat1.transpose(2, 3).contiguous().view(b, c, h * w)
        feat2 = feat2.view(b, c, h * w).transpose(1, 2)
        feature_mul = torch.bmm(feat2, feat1)
