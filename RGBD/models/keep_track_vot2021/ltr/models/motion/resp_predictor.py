...
import torch
import ltr.models.layers.blocks as ltr_blocks
import torch.nn.functional as F
from ltr.models.layers.blocks import conv_block
from .conv_gru import ConvGRUCell
from .layers import Nothing, Identity


class SepProcConvGRUv6(nn.Module):
    def __init__(self, state_dim=8, representation_predictor_dims=(64, 32), gru_ksz=3, model_out_of_frame=False,
                 flip_cv_proc=False, use_init_bias=True,
                 use_sigmoid=True, use_bn_for_resp_pred=True, prev_frame_softmax=True, prev_max_pool_ksz=1,
                 conf_measure='max', dimp_thresh=None):
        super().__init__()
        self.use_bn_for_resp_pred = use_bn_for_resp_pred
        self.use_sigmoid = use_sigmoid
        self.flip_cv_proc = flip_cv_proc
        self.model_out_of_frame = model_out_of_frame
        self.prev_frame_softmax = prev_frame_softmax
        ...
        for out_dim in representation_predictor_dims:
            representation_predictor_list.append(conv_block(in_dim, out_dim, kernel_size=3, stride=1, padding=1,
                                                            batch_norm=use_bn_for_resp_pred, relu=True))
            in_dim = out_dim
        ...
        self.init_hidden_state_predictor = nn.Sequential(
                conv_block(1, state_dim, kernel_size=3, stride=1, padding=1, batch_norm=False, relu=False, bias=use_init_bias),
                nn.Tanh())

        self.patch_going_oof_w = nn.Parameter(torch.zeros(1))
        self.patch_coming_in_w = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, cost_volume, state_prev, dimp_score_cur, init_label=None, dimp_thresh=None,
                pool_dimp_score=False, output_window=None):
        ...
        if self.flip_cv_proc:
            # TODO clean this
            cost_volume = cost_volume.view(-1, 1, feat_sz[0], feat_sz[1])
            cost_volume_p2 = self.cost_volume_proc2(cost_volume.view(-1, 1, feat_sz[0], feat_sz[1]))

            cost_volume_p2 = cost_volume_p2.view(num_sequences, -1, feat_sz[0], feat_sz[1])

            if self.model_out_of_frame:
                patch_coming_in_w = self.patch_coming_in_w.view(1, 1, 1, 1).expand(num_sequences, -1,
                                                                                   feat_sz[0], feat_sz[1])
                cost_volume_p2 = torch.cat((cost_volume_p2, patch_coming_in_w), dim=1)
                cost_volume_p2 = F.softmax(cost_volume_p2, dim=1)[:, :-1, :, :]
            else:
                cost_volume_p2 = F.softmax(cost_volume_p2, dim=1)

            cost_volume_p2 = cost_volume_p2.view(-1, 1, feat_sz[0], feat_sz[1])

            cost_volume_p1 = self.cost_volume_proc1(cost_volume_p2).view(-1, feat_sz[0] * feat_sz[1])

            if self.model_out_of_frame:
                patch_going_oof_w = self.patch_going_oof_w.view(1, 1).expand(cost_volume_p1.shape[0], -1)
                cost_volume_p1 = torch.cat((cost_volume_p1, patch_going_oof_w), dim=1)
                cost_volume_p1 = F.softmax(cost_volume_p1, dim=1)[:, :-1]
            else:
                cost_volume_p1 = F.softmax(cost_volume_p1, dim=1)

            cost_volume_p1 = cost_volume_p1.view(num_sequences, -1, 1, feat_sz[0], feat_sz[1])
            auxiliary_outputs['cost_volume_processed'] = cost_volume_p1
            cost_volume_p2 = cost_volume_p1
        else:
            cost_volume = cost_volume.view(-1, 1, feat_sz[0], feat_sz[1])
            ...
            if self.prev_frame_softmax:
                ...
                cost_volume_p2 = cost_volume_p1.view(num_sequences, -1, 1, feat_sz[0], feat_sz[1])
        ...
        if state_prev is None:
            ...
            tracking_state_prev = None
        else:
            state_prev_ndhw, tracking_state_prev = state_prev
        ...
        if dimp_thresh is not None:
            if pool_dimp_score:
                dimp_score_cur_p = F.max_pool2d(dimp_score_cur, 3, 1, 1)
                fused_prediction = fused_prediction * (dimp_score_cur_p > dimp_thresh).float()
            else:
        ...
        tracking_state_new = None

        return fused_prediction, (state_new, tracking_state_new), auxiliary_outputs


class SepProcConvGRUv6v2(nn.Module):
    def __init__(self, state_dim=8, representation_predictor_dims=(64, 32), gru_ksz=3, use_init_bias=True,
                 use_sigmoid=True, use_bn_for_resp_pred=True, prev_max_pool_ksz=1,
                 conf_measure='max', dimp_thresh=None):
        super().__init__()
        self.use_bn_for_resp_pred = use_bn_for_resp_pred
        self.use_sigmoid = use_sigmoid
