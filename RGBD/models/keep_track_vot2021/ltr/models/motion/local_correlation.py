...
class _FunctionCorrelationTranspose(torch.autograd.Function):
	...
	def backward(self, gradOutput):
		...
		gradInput = input.new_zeros([ input.size(0), input.size(1), input.size(2), input.size(3) ]) if self.needs_input_grad[0] == True else None
		gradSecond = second.new_zeros([ second.size(0), second.size(1), second.size(2), second.size(3) ]) if self.needs_input_grad[1] == True else None
			...
			if gradSecond is not None:
				for intSample in range(second.size(0)):
					...
					cupy_launch('kernel_Correlation_updateGradSecond', cupy_kernel('kernel_Correlation_updateGradSecond', {
						...
						grid=tuple([ int((n + 512 - 1) / 512), 1, 1 ]),
						block=tuple([ 512, 1, 1 ]),
						args=[ n, intSample, rbot0.data_ptr(), rbot1.data_ptr(), input.data_ptr(), None, gradSecond.data_ptr() ],
						stream=Stream
					)
