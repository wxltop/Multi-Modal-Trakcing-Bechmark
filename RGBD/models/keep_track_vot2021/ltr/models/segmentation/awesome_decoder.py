...
class TSE(nn.Module):
    ...
    def __init__(self, fc, ic, oc):
        ...
        self.reduce = nn.Sequential(conv(fc+ic, fc, 3), relu(), conv(fc, oc, 3), relu())  # Reduce number of feature dimensions
        #nc = ic + oc
...
class SegNetwork(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=64, ft_channels=None, use_bn=False, skip_rrb=None):
    ...
    def forward(self, scores, features, image_size, output_layers=()):
        ...
        for i, L in enumerate(self.ft_channels):
            ...
            if multi_targets:
                h = self.TSE[L](ft.repeat(num_targets,1,1,1), scores, x)
            else:
                h = self.TSE[L](ft, scores, x)
