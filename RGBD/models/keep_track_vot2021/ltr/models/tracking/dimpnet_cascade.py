...
class DiMPnetCascade(nn.Module):
    """The DiMP network.
    args:
    ...
    def __init__(self, feature_extractor, classifier, bb_regressors, classification_layer, bb_regressor_layer):
        super().__init__()

        self.feature_extractor = feature_extractor
        self.classifier = classifier

        for i, bbr in enumerate(bb_regressors):
            setattr(self, 'bb_regressor_l{}'.format(i), bbr)
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        iou_pred_all = []
        for cascade_level, proposals in enumerate(test_proposals):
            iou_pred = getattr(self, 'bb_regressor_l{}'.format(cascade_level))(train_feat_iou, test_feat_iou,
                                                         train_bb, proposals)
            iou_pred_all.append(iou_pred)

        # iou_pred = self.bb_regressor(train_feat_iou, test_feat_iou, train_bb, test_proposals)

        return target_scores, iou_pred_all
...
@model_constructor
def dimpnet50_hr(num_cascades=1, filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    for _ in range(num_cascades):
        bb_regressors.append(bbmodels.AtomIoUNet(input_dim=(4 * 128, 4 * 256), pred_input_dim=iou_input_dim,
                                                  pred_inter_dim=iou_inter_dim))

    # DiMP network
    net = DiMPnetCascade(feature_extractor=backbone_net, classifier=classifier, bb_regressors=bb_regressors,
                         classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net


@model_constructor
def dimpnet50(num_cascades=1, filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
