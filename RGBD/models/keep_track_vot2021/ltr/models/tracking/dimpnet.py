...
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    elif bb_regressor_type == 'atom_hr':
        bb_regressor = bbmodels.AtomIoUNetHR(input_dim=(4 * 128, 4 * 256), pred_input_dim=iou_input_dim,
                                             pred_inter_dim=iou_inter_dim)
    elif bb_regressor_type == 'bbr_mod_cascade':
        bb_regressor = fpn_bbrnet.BBRNetModCascadeL23(num_stages=3, input_dim=(4*128,4*256),
...
@model_constructor
def dimpnet50_hinge(filter_size=1, optim_iter=5, optim_init_reg=0.01,
              classification_layer='layer3', feat_stride=16, backbone_pretrained=True, clf_feat_blocks=0,
              clf_feat_norm=True, init_filter_norm=False, final_conv=True,
...
@model_constructor
def kldimpnet18(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
              classification_layer='layer3', feat_stride=16, backbone_pretrained=True, clf_feat_blocks=1,
              clf_feat_norm=True, init_filter_norm=False, final_conv=True,
              out_feature_dim=256, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
              mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
              score_act='relu', act_param=None, target_mask_act='sigmoid',
              detach_length=float('Inf'), alpha_eps=0.0):
    # Backbone
    backbone_net = backbones.resnet18(pretrained=backbone_pretrained)
    ...
    # Optimizer for the DiMP classifier
    optimizer = clf_optimizer.KLDiMPSteepestDescentGN(num_iter=optim_iter, feat_stride=feat_stride,
                                                    init_step_length=optim_init_step,
                                                    init_filter_reg=optim_init_reg, init_gauss_sigma=init_gauss_sigma,
                                                    num_dist_bins=num_dist_bins,
                                                    bin_displacement=bin_displacement,
...
@model_constructor
def klceinterp_dimpnet18(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                  classification_layer='layer3', feat_stride=16, backbone_pretrained=True, clf_feat_blocks=1,
                  clf_feat_norm=True, init_filter_norm=False, final_conv=True,
                  out_feature_dim=256, gauss_sigma=1.0,
                  iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
                  detach_length=float('Inf'), alpha_eps=0.0, train_feature_extractor=True,
                  init_uni_weight=None, optim_min_reg=1e-3, init_initializer='default', normalize_label=False,
                  label_shrink=0, softmax_reg=None, label_threshold=0, final_relu=False, interp_method='none', interp_factor=1,
                         frozen_backbone_layers=()):
    ...
    # Optimizer for the DiMP classifier
    score_predictor = clf_probabilistic.InterpScorePredictor(feat_stride=feat_stride,
                                                            init_filter_reg=optim_init_reg, gauss_sigma=gauss_sigma,
                                                            init_uni_weight=init_uni_weight,
                                                            min_filter_reg=optim_min_reg, normalize_label=normalize_label,
                                                            label_shrink=label_shrink, softmax_reg=softmax_reg,
                                                            label_threshold=label_threshold, interp_method=interp_method,
                                                       interp_factor=interp_factor)

    optimizer = steepestdescent.KLRegSteepestDescent(score_predictor=score_predictor, num_iter=optim_iter,
                                                            init_step_length=optim_init_step,
                                                            detach_length=detach_length, steplength_reg=alpha_eps,
                                                            softmax_reg=softmax_reg)

    # The classifier module
    classifier = clf_probabilistic.KLFilter(filter_size=filter_size, filter_initializer=initializer,
                                         filter_optimizer=optimizer, feature_extractor=clf_feature_extractor)
...
@model_constructor
def dimpnet_baby(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                 classification_layer='layer3', feat_stride=16, backbone_pretrained=True, clf_feat_blocks=0,
                 clf_feat_norm=True, init_filter_norm=False, final_conv=False,
                 out_feature_dim=64, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
                 mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
                 score_act='relu', act_param=None, target_mask_act='sigmoid',
                 detach_length=float('Inf'),
                 resnet_inplanes=16):
    # Backbone
    backbone_net = backbones.resnet_baby(inplanes=resnet_inplanes)
    ...
    # Bounding box regressor
    bb_regressor = bbmodels.AtomIoUNetBaby(input_dim=(resnet_inplanes*2,resnet_inplanes*4))
...
@model_constructor
def dimpnet50_mrcnn(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                    classification_layer='layer3', feat_stride=16, clf_feat_blocks=0,
                    clf_feat_norm=True, init_filter_norm=False, final_conv=True,
                    out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
                    mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
                    score_act='relu', act_param=None, target_mask_act='sigmoid',
                    detach_length=float('Inf'), train_feature_extractor=True, frozen_backbone_layers=()):

    if not train_feature_extractor:
        frozen_backbone_layers = 'all'

    # Backbone
    backbone_net = resnet_mrcnn.resnet50(pretrained=False, frozen_layers=frozen_backbone_layers)
...
@model_constructor
def dimpnet50_hr(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                 classification_layer='layer3', feat_stride=32, backbone_pretrained=True, clf_feat_blocks=0,
                 clf_feat_norm=True, init_filter_norm=False, final_conv=True,
                 out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
                 mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
                 score_act='relu', act_param=None, target_mask_act='sigmoid',
...
@model_constructor
def dimpnet50_mrcnn_hr(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                 classification_layer='layer3', feat_stride=32, backbone_pretrained=True, clf_feat_blocks=0,
                 clf_feat_norm=True, init_filter_norm=False, final_conv=True,
                 out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
                 mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
                 ...
                 detach_length=float('Inf'), frozen_backbone_layers=()):
    # Backbone
    backbone_net = resnet_mrcnn.resnet50(pretrained=backbone_pretrained, frozen_layers=frozen_backbone_layers)
...
@model_constructor
def dimpnet50_fpn_hr(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                     classification_layer='fpn_output4',  backbone_pretrained=True,
                     clf_feat_norm=True, init_filter_norm=False, clf_feat_blocks=0,
                     cls_feature_dim=256, final_conv=False,
                     init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
                     mask_init_factor=4.0,
                     score_act='relu', act_param=None, target_mask_act='sigmoid',
                     detach_length=float('Inf'), frozen_backbone_layers=(),
                     bb_regressor_type='iou_mod', backbone='resnet50_fpn',
    ...
    if classification_layer == 'layer3' or classification_layer == 'fpn_output4':
        cls_feat_stride = 16
    else:
        raise ValueError

    # Optimizer for the DiMP classifier
    optimizer = clf_optimizer.DiMPSteepestDescentGN(num_iter=optim_iter, feat_stride=cls_feat_stride,
                                                    init_step_length=optim_init_step,
                                                    init_filter_reg=optim_init_reg, init_gauss_sigma=init_gauss_sigma,
                                                    num_dist_bins=num_dist_bins,
                                                    bin_displacement=bin_displacement,
    ...
    if bb_regressor_layer == 'fpn_output3':
        ...
        raise Exception('Unknown layer for bbreg')

    # Bounding box regressor
    if bb_regressor_type == 'iou_mod':
...
@model_constructor
def dimpnet_custom_res50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
              classification_layer='layer3', feat_stride=16, backbone_pretrained=True, backbone_path=None, clf_feat_blocks=0,
              clf_feat_norm=True, init_filter_norm=False, final_conv=True,
              out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
              mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
              score_act='relu', act_param=None, target_mask_act='sigmoid',
    ...
    # Backbone
    backbone_net = backbones.resnet.resnet50_custom(pretrained=backbone_pretrained, path=backbone_path, frozen_layers=frozen_backbone_layers)
    ...
    # Classifier features
    feature_dim = 256
    input_dim = getattr(backbone_net, classification_layer)[-1].bn3.num_features

    clf_feature_extractor = clf_features.residual_bottleneck(feature_dim=feature_dim,
                                                             num_blocks=clf_feat_blocks, l2norm=clf_feat_norm,
                                                             final_conv=final_conv, norm_scale=norm_scale,
                                                             out_dim=out_feature_dim, input_dim=input_dim)
    ...
    bb_input_dim = [getattr(backbone_net, l)[-1].bn3.num_features for l in ['layer2', 'layer3']]

    # Bounding box regressor
    bb_regressor = bbmodels.AtomIoUNet(input_dim=bb_input_dim, pred_input_dim=iou_input_dim, pred_inter_dim=iou_inter_dim)
