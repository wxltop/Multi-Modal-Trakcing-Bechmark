...
import ltr.models.target_classifier.weight_learn_filter as target_wlf_clf
...
import ltr.models.weight_predictor.similarity_filter as similarity_filter
...
@model_constructor
def dimpnet18_baseline(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
              classification_layer='layer3', feat_stride=16, backbone_pretrained=True, clf_feat_blocks=1,
              clf_feat_norm=True, init_filter_norm=False, final_conv=True, hinge_threshold=0.05,
              out_feature_dim=256, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
              score_act='relu', act_param=None,
    ...
    residual_module = residual_modules.PatchFilter(init_filter_reg=optim_init_reg, hinge_threshold=hinge_threshold,
    ...
    # DiMP network
    net = DiMPWeightNet(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                        classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net


@model_constructor
def dimpnet18_similarity_filter(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
              classification_layer='layer3', feat_stride=16, backbone_pretrained=True, clf_feat_blocks=1,
              clf_feat_norm=True, init_filter_norm=False, final_conv=True, hinge_threshold=0.05,
              out_feature_dim=256, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
              score_act='relu', act_param=None, weight_scaling_activation="softmax", similarity_feature_dim=64,
              detach_length=float('Inf'), frozen_backbone_layers=()):
    # Backbone
    backbone_net = backbones.resnet18(pretrained=backbone_pretrained, frozen_layers=frozen_backbone_layers)
    ...
    # The weight predictor module
    weight_predictor = similarity_filter.LinearSimilarityFilter(weight_scaling_activation=weight_scaling_activation,
                                                                similarity_feature_dim=similarity_feature_dim)

    # The classifier module
    classifier = target_wlf_clf.LinearFilterWeightLearn(filter_size=filter_size, filter_initializer=initializer,
                                                        filter_optimizer=optimizer, feature_extractor=clf_feature_extractor,
...
@model_constructor
def dimpnet50_weight_learn(filter_size=1, optim_iter=5, optim_init_reg=0.01, optim_steplength_reg=0.,
                           ...
                           detach_length=float('Inf'), frozen_backbone_layers=(), weight_predictor_hidden_dim=32,
                           weight_predictor_feat_dim=512, weight_predictor_kernel_size=1,
                           weight_predictor_feature_list=None, weight_predictor_softmax_temp_init=1,
                           weigh_predictor_use_batch_norm=True
                           ):
    ...
    # Construct generic optimizer module
    optimizer = steepestdescent.GNSteepestDescent(residual_module=residual_module, num_iter=optim_iter, detach_length=detach_length,
                                                  residual_batch_dim=1, compute_losses=True, steplength_reg=optim_steplength_reg)

    weight_predictor = similarity_filter.SimilarityWeightPredictor(hidden_dim=weight_predictor_hidden_dim,
                                                                   feat_dim=weight_predictor_feat_dim,
                                                                   kernel_size=weight_predictor_kernel_size,
                                                                   feature_list=weight_predictor_feature_list,
                                                                   softmax_temp_init=weight_predictor_softmax_temp_init,
                                                                   use_batch_norm=weigh_predictor_use_batch_norm)

    # The classifier module
    classifier = target_wlf_clf.LinearFilterSimilarityWeightLearn(filter_size=filter_size, filter_initializer=initializer,
