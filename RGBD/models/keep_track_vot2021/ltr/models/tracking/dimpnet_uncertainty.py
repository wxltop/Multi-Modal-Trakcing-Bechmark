...
import ltr.models.target_classifier.uncertainty as clf_uncertainty
import ltr.models.bbreg as bbmodels
import ltr.models.backbone as backbones
from ltr import model_constructor


class DiMPnetUncertainty(nn.Module):
    """The DiMP network.
    args:
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        target_scores, scores_var, final_scores = self.classifier(train_feat_clf, test_feat_clf, train_bb, *args, **kwargs)
        ...
        return target_scores, iou_pred, scores_var, final_scores
...
def dimpnet18(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    # DiMP network
    net = DiMPnetUncertainty(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                  classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net


@model_constructor
def dimpnet50_gauss(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
              ...
              detach_length=float('Inf'), frozen_backbone_layers=(),
              filter_var_bn=True, resp_predictor_dims=()):
    ...
    filter_variance_estimator = clf_uncertainty.FilterVariancePredictor(feature_dim=out_feature_dim, filter_size=filter_size,
                                                                        feature_stride=feat_stride,
                                                                        batchnorm=filter_var_bn)
    final_predictor = clf_uncertainty.ResponsePredictor(hidden_dims=resp_predictor_dims)

    # The classifier module
    classifier = target_clf.LinearGaussianFilter(filter_size=filter_size, filter_initializer=initializer,
                                         filter_optimizer=optimizer, feature_extractor=clf_feature_extractor,
                                                 filter_variance_estimator=filter_variance_estimator,
