...
import ltr.models.motion.motion_predictor as motion_pred
import ltr.models.motion.resp_predictor as resp_pred
import ltr.models.motion.cost_volume as cost_volume
import ltr.models.bbreg as bbmodels
import ltr.models.backbone as backbones
from ltr import model_constructor


class DiMPMotionTracker(nn.Module):
    ...
    def train_classifier(self, train_imgs, train_bb, is_distractor=None):
    ...
    def track(self, current_scores, prev_scores, current_feat, prev_feat, prev_label):
        raise NotImplementedError
        target_scores = torch.cat((prev_scores, current_scores), dim=1)

        motion_feat = self.motion_encoder(prev_feat, current_feat)

        response_map_pred = self.resp_regressor(target_scores, motion_feat, prev_label)
        return response_map_pred



@model_constructor
def cv_ind_proc_convgru_v6_res50(optim_iter=3, appearance_feature_dim=512, cv_kernel_size=3,
                                 cv_max_displacement=9, cv_stride=1, init_gauss_sigma=1.0,
                                 train_feature_extractor=True, train_iounet=True,
                                 detach_length=float('Inf'),
                                 state_dim=8, representation_predictor_dims=(64, 32),
                                 pool_before_cv=False,
                                 gru_ksz=3, model_out_of_frame=False, flip_cv_proc=False, use_init_bias=True,
                                 use_sigmoid=True, fix_coordinate_shift=False, use_bn_for_resp_pred=True,
                                 prev_frame_softmax=True, conf_measure='max', dimp_thresh=None,
                                 num_dist_bins=100, bin_displacement=0.1):
    # Dimp params
    filter_size = 4
    optim_init_step = 0.9
    optim_init_reg = 0.1
    ...
    motion_response_predictor = resp_pred.SepProcConvGRUv6(state_dim=state_dim,
                                                           representation_predictor_dims=representation_predictor_dims,
                                                           gru_ksz=gru_ksz,
                                                           model_out_of_frame=model_out_of_frame,
                                                           flip_cv_proc=flip_cv_proc, use_init_bias=use_init_bias,
                                                           use_sigmoid=use_sigmoid,
                                                           use_bn_for_resp_pred=use_bn_for_resp_pred,
                                                           prev_frame_softmax=prev_frame_softmax,
                                                           conf_measure=conf_measure,
                                                           dimp_thresh=dimp_thresh
                                                           )
    response_predictor = motion_pred.CVIndProcv1(cost_volume_layer, motion_response_predictor,
                                                 pool_before_cv=pool_before_cv,
                                                 fix_coordinate_shift=fix_coordinate_shift)

    net = DiMPMotionTracker(backbone_feature_extractor=backbone_net, dimp_classifier=classifier,
...
@model_constructor
def cv_ind_proc_convgru_v6_res50_noprop(optim_iter=3, appearance_feature_dim=512, cv_kernel_size=3,
                                 cv_max_displacement=9, cv_stride=1, init_gauss_sigma=1.0,
                                 train_feature_extractor=True, train_iounet=True,
                                 detach_length=float('Inf'),
                                 state_dim=8, representation_predictor_dims=(64, 32),
    ...
    motion_response_predictor = resp_pred.SepProcConvGRUv6NoProp(state_dim=state_dim,
                                                           representation_predictor_dims=representation_predictor_dims,
                                                           gru_ksz=gru_ksz,
                                                           model_out_of_frame=model_out_of_frame,
                                                           flip_cv_proc=flip_cv_proc, use_init_bias=use_init_bias,
...
@model_constructor
def cv_ind_proc_convgru_v6_res50_wta(optim_iter=3, appearance_feature_dim=512, cv_kernel_size=3,
                                 cv_max_displacement=9, cv_stride=1, init_gauss_sigma=1.0,
                                 train_feature_extractor=True, train_iounet=True,
                                 detach_length=float('Inf'),
                                 state_dim=8, representation_predictor_dims=(64, 32),
    ...
    motion_response_predictor = resp_pred.SepProcConvGRUv6WTA(state_dim=state_dim,
                                                           representation_predictor_dims=representation_predictor_dims,
                                                           gru_ksz=gru_ksz,
                                                           model_out_of_frame=model_out_of_frame,
                                                           flip_cv_proc=flip_cv_proc, use_init_bias=use_init_bias,
...
@model_constructor
def cv_ind_proc_convgru_v6_res50_nostate(optim_iter=3, appearance_feature_dim=512, cv_kernel_size=3,
                                 cv_max_displacement=9, cv_stride=1, init_gauss_sigma=1.0,
                                 train_feature_extractor=True, train_iounet=True,
                                 detach_length=float('Inf'),
                                 state_dim=8, representation_predictor_dims=(64, 32),
    ...
    motion_response_predictor = resp_pred.SepProcConvGRUv6NoState(state_dim=state_dim,
                                                           representation_predictor_dims=representation_predictor_dims,
                                                           gru_ksz=gru_ksz,
                                                           model_out_of_frame=model_out_of_frame,
                                                           flip_cv_proc=flip_cv_proc, use_init_bias=use_init_bias,
...
@model_constructor
def cv_ind_proc_convgru_v6_res50_nodimp(optim_iter=3, appearance_feature_dim=512, cv_kernel_size=3,
                                 cv_max_displacement=9, cv_stride=1, init_gauss_sigma=1.0,
                                 train_feature_extractor=True, train_iounet=True,
                                 detach_length=float('Inf'),
                                 state_dim=8, representation_predictor_dims=(64, 32),
    ...
    motion_response_predictor = resp_pred.SepProcConvGRUv6NoDimp(state_dim=state_dim,
                                                           representation_predictor_dims=representation_predictor_dims,
                                                           gru_ksz=gru_ksz,
                                                           model_out_of_frame=model_out_of_frame,
                                                           flip_cv_proc=flip_cv_proc, use_init_bias=use_init_bias,
...
@model_constructor
def cv_ind_proc_convgru_v6_res50_lab(optim_iter=3, appearance_feature_dim=512, cv_kernel_size=3,
                                 cv_max_displacement=9, cv_stride=1, init_gauss_sigma=1.0,
                                 train_feature_extractor=True, train_iounet=True,
                                 detach_length=float('Inf'),
                                 state_dim=8, representation_predictor_dims=(64, 32),
    ...
    motion_response_predictor = resp_pred.SepProcConvGRUv6Lab(state_dim=state_dim,
                                                           representation_predictor_dims=representation_predictor_dims,
                                                           gru_ksz=gru_ksz,
                                                           model_out_of_frame=model_out_of_frame,
                                                           flip_cv_proc=flip_cv_proc, use_init_bias=use_init_bias,
...
@model_constructor
def cv_ind_proc_convgru_v6_res50_nomotion(optim_iter=3, appearance_feature_dim=512, cv_kernel_size=3,
                                 cv_max_displacement=9, cv_stride=1, init_gauss_sigma=1.0,
                                 train_feature_extractor=True, train_iounet=True,
                                 detach_length=float('Inf'),
                                 state_dim=8, representation_predictor_dims=(64, 32),
    ...
    motion_response_predictor = resp_pred.SepProcConvGRUv6NoMotion(state_dim=state_dim,
                                                           representation_predictor_dims=representation_predictor_dims,
                                                           gru_ksz=gru_ksz,
                                                           model_out_of_frame=model_out_of_frame,
                                                           flip_cv_proc=flip_cv_proc, use_init_bias=use_init_bias,
