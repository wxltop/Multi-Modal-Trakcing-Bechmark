...
import ltr.models.aux.semantic_cls as aux_cls
...
class DiMPnet(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, test_bb=None, *args, **kwargs):
        ...
        if self.semantic_classifier is not None:
            class_pred = self.semantic_classifier(test_feat_clf, test_bb)
        else:
            class_pred = None

        return target_scores, iou_pred, class_pred
...
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    if class_dict is not None:
        semantic_classifier = aux_cls.SemanticClassifier(feature_dim*4, feat_stride, 3, class_dict)
    else:
        semantic_classifier = None
