...
import ltr.models.bbreg.bbr_cascade_net as bbr_cascade_net
import ltr.models.bbreg.fpn_iou_net as fpn_iou_net
...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, iou_regressor,
        ...
        self.iou_regressor = iou_regressor
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_bbr_proposals, test_iou_proposals, *args, **kwargs):
        ...
        bb_deltas, bb_pred = self.bb_regressor(train_feat_iou, test_feat_iou, train_bb, test_bbr_proposals)

        if self.iou_regressor is not None:
            iou_pred = self.iou_regressor(train_feat_iou, test_feat_iou, train_bb, test_iou_proposals)
        else:
            iou_pred = None

        target_scores = torch.stack(target_scores)
        target_scores = target_scores.view(-1, *target_scores.shape[-3:])

        bb_deltas = torch.stack(bb_deltas)
        bb_deltas = bb_deltas.view(-1, *bb_deltas.shape[-3:])

        bb_pred = torch.stack(bb_pred)
        bb_pred = bb_pred.view(-1, *bb_pred.shape[-3:])

        return target_scores, bb_deltas, bb_pred, iou_pred
    ...
    def get_backbone_iou_feat(self, backbone_feat):
        return [backbone_feat[l] for l in self.bb_regressor_layer]
...
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
              ...
              mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
              score_act='relu', act_param=None, target_mask_act='sigmoid',
              iou_regressor_type='atom',
              bb_regressor_type='cascade_mod',
              detach_length=float('Inf'), frozen_backbone_layers=()):
    ...
    # Bounding box regressor
    if iou_regressor_type == 'atom':
        iou_regressor = bbmodels.AtomIoUNet(input_dim=(4*128,4*256), pred_input_dim=iou_input_dim, pred_inter_dim=iou_inter_dim)
    elif iou_regressor_type == 'atom_hr':
        iou_regressor = bbmodels.AtomIoUNetHR(input_dim=(4 * 128, 4 * 256), pred_input_dim=iou_input_dim,
                                             pred_inter_dim=iou_inter_dim)
    elif iou_regressor_type == 'none':
        iou_regressor = None
    else:
        raise Exception

    if bb_regressor_type == 'cascade_mod':
        bb_regressor = bbr_cascade_net.BBRNetModCascadeL23(num_stages=3, input_dim=(4*128,4*256),
                                                      pred_input_dim=iou_input_dim, pred_inter_dim=iou_inter_dim)
    elif bb_regressor_type == 'cascade_cat_d':
        bb_regressor = bbr_cascade_net.CatPredictorConv(num_stages=3, input_dim=(4*128, 4*256))
    else:
        raise Exception

    # DiMP network
    net = DiMPnet(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                  iou_regressor=iou_regressor,
                  classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net


@model_constructor
def dimpnet152(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
               classification_layer='fpn_output4', feat_stride=16,
               clf_feat_norm=True, init_filter_norm=False,
               out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
               mask_init_factor=4.0,
               score_act='relu', act_param=None, target_mask_act='sigmoid', final_conv_stride=2,
               detach_length=float('Inf'), frozen_backbone_layers=()):

    # Backbone
    backbone_net = backbones.resnext_152_32x8d_dconv_fpn(frozen_layers=frozen_backbone_layers)
    ...
    if classification_layer == 'layer3':
        ...
        feature_dim = 512 * 4
    elif classification_layer == 'fpn_output4':
        feature_dim = 256
    else:
        raise Exception

    clf_feature_extractor = clf_features.conv(feature_dim=feature_dim, l2norm=clf_feat_norm, norm_scale=norm_scale,
                                              out_dim=out_feature_dim, stride=final_conv_stride)
    ...
    # Bounding box regressor
    iou_regressor = fpn_iou_net.FPNCatIoUPredictorConv()

    bb_regressor = bbr_cascade_net.FPNCatPredictorConv(num_stages=3)

    # DiMP network
    net = DiMPnet(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                  iou_regressor=iou_regressor,
                  classification_layer=classification_layer, bb_regressor_layer=['fpn_output3', ])
    return net


@model_constructor
def dimpnet50_fpn(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                  classification_layer='fpn_output4', feat_stride=16,
                  clf_feat_norm=True, init_filter_norm=False,
                  out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
                  mask_init_factor=4.0,
                  score_act='relu', act_param=None, target_mask_act='sigmoid', final_conv_stride=1,
                  detach_length=float('Inf'), frozen_backbone_layers=()):

    # Backbone
    backbone_net = backbones.resnet50_fpn(frozen_layers=frozen_backbone_layers)
...
@model_constructor
def dimpnet50_fpn_gn(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                     classification_layer='fpn_output4', feat_stride=16,
                     clf_feat_norm=True, init_filter_norm=False,
                     out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
                     mask_init_factor=4.0,
    ...
    # Backbone
    backbone_net = backbones.resnet50_fpn_gn(frozen_layers=frozen_backbone_layers)
