...
class DistanceValueEncoder(nn.Module):
    def __init__(self, num_bins, min_val=-1.0, max_val=1.0):
        super().__init__()
        self.bin_displacement = ((max_val - min_val) / (num_bins - 1))
        self.bin_centers = torch.arange(num_bins, dtype=torch.float32).reshape(1,1,-1,1,1) + min_val / self.bin_displacement

    def forward(self, x):
        assert x.dim() == 4

        self.bin_centers = self.bin_centers.to(x.device)
        bin_diff = x.unsqueeze(dim=2) / self.bin_displacement - self.bin_centers
        bin_val = F.relu(1.0 - torch.abs(bin_diff), inplace=True)
        return bin_val.view(x.shape[0], -1, *x.shape[-2:])
