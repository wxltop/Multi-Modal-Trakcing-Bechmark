...
def apply_filter(feat, filter, dilation_factors=None):
    ...
    if multiple_filters:
        if dilation_factors is None:
            scores = F.conv2d(feat.reshape(num_images, -1, feat.shape[-2], feat.shape[-1]), filter.reshape(-1, *filter.shape[-3:]),
                              padding=padding, groups=num_sequences*groups)

            return scores.reshape(num_images, num_sequences, -1, scores.shape[-2], scores.shape[-1])
        else:
            scores_all = []
            start_id = 0
            ...
            for d_factor, num_filters_with_d in dilation_factors.items():
                ...
                scores_d = F.conv2d(feat.reshape(num_images, -1, feat.shape[-2], feat.shape[-1]),
                                    f_d.reshape(-1, *f_d.shape[-3:]),
                                    padding=padding_d, groups=num_sequences * groups,
                                    dilation=d_factor)
                scores_d = scores_d.reshape(num_images, num_sequences, -1, scores_d.shape[-2], scores_d.shape[-1])
    ...
    return scores.reshape(num_images, num_sequences, scores.shape[-2], scores.shape[-1])
...
def _apply_filter_ksz1(feat, filter):
    ...
    scores = torch.matmul(filter.reshape(filter.shape[:-2]), feat.reshape(*feat.shape[:-2], -1))
...
def apply_feat_transpose(feat, input, filter_ksz, training=True, groups=1):
    ...
    output:
        Output of transposed operation. Dimensions (sequences, feat_dim, fH, fW) or (sequences, filters, feat_dim, fH, fW)
    """
...
def _apply_feat_transpose_v1(feat, input, filter_ksz):
    ...
    filter_grad = F.conv_transpose2d(input.flip((2, 3)).reshape(1, -1, input.shape[-2], input.shape[-1]),
                                     feat.reshape(-1, feat.shape[-3], feat.shape[-2], feat.shape[-1]),
                                     padding=trans_pad, groups=num_images * num_sequences)

    return filter_grad.reshape(num_images, num_sequences, -1, filter_grad.shape[-2], filter_grad.shape[-1]).sum(dim=0)
...
def _apply_feat_transpose_v2(feat, input, filter_ksz):
    ...
    if multiple_filters:
        ...
        if num_images == 1:
            return filter_grad.reshape(num_filters, num_sequences, -1, filter_grad.shape[-2], filter_grad.shape[-1]).flip((3,4)).permute(1,0,2,3,4)
        return filter_grad.reshape(num_filters, num_images, num_sequences, -1, filter_grad.shape[-2], filter_grad.shape[-1]).sum(dim=1).flip((3,4)).permute(1,0,2,3,4)

    filter_grad = F.conv2d(input.reshape(1, -1, input.shape[-2], input.shape[-1]),
                                     feat.reshape(-1, 1, feat.shape[-2], feat.shape[-1]),
                                     padding=trans_pad, groups=num_images * num_sequences)

    return filter_grad.reshape(num_images, num_sequences, -1, filter_grad.shape[-2], filter_grad.shape[-1]).sum(dim=0).flip((2,3))
...
def _apply_feat_transpose_v3(feat, input, filter_ksz):
    ...
    if multiple_filters:
        if num_images == 1:
            return filter_grad.reshape(-1, num_sequences, num_filters, filter_grad.shape[-2], filter_grad.shape[-1]).permute(1,2,0,3,4)
        return filter_grad.reshape(-1, num_images, num_sequences, num_filters, filter_grad.shape[-2], filter_grad.shape[-1]).sum(dim=1).permute(1,2,0,3,4)

    if num_images == 1:
        return filter_grad.permute(1,0,2,3)
    return filter_grad.reshape(-1, num_images, num_sequences, filter_grad.shape[-2], filter_grad.shape[-1]).sum(dim=1).permute(1,0,2,3)
...
def _apply_feat_transpose_ksz1(feat, input, filter_ksz=1):
    """Fast forward and slow backward"""
    ...
    filter_grad = torch.matmul(feat.reshape(num_images, num_sequences, 1, feat_dim, -1),
                               input.reshape(num_images, num_sequences, num_filters, -1, 1))

    if num_images > 1:
        filter_grad = filter_grad.sum(dim=0)

    if multiple_filters:
        return filter_grad.reshape(num_sequences, num_filters, feat_dim, 1, 1)

    return filter_grad.reshape(num_sequences, feat_dim, 1, 1)
