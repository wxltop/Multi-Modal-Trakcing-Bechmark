...
def resnet50_custom(output_layers=None, pretrained=False, path=None, **kwargs):
    """Constructs a ResNet-50 model.
    """

    def _load_state_dict(model, state_dict, strict=False):
        """
    ...
    _load_state_dict(model, model_dict, strict=False)
    _set_dims(model)

    return model


def resnet34(output_layers=None, pretrained=False, dilation_factor=1):
    """Constructs a ResNet-18 model.
    """
    ...
    model = ResNet(BasicBlock, [3, 4, 6, 3], output_layers, dilation_factor=dilation_factor)
