...
class LinearAttentionFusionFilter(nn.Module):
    ...
    def forward(self, weights, test_feat, train_feats, train_labels):
        raw_scores = self.raw_scores(weights, test_feat)
        return self.fusion_module(raw_scores, train_labels, test_feat, train_feats)
    ...
    def raw_scores(self, weights, test_feat):
        scores = filter_layer.apply_filter(test_feat, weights)
        return scores

    def fuse_scores(self, sim, scores, train_labels, test_feat, train_feat):
        return self.fusion_module.fuse_scores(sim, scores, train_labels, test_feat, train_feat)
