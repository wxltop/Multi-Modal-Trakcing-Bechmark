...
def residual_bottleneck(feature_dim=256, num_blocks=1, l2norm=True, final_conv=False, norm_scale=1.0, out_dim=None,
                        interp_cat=False, final_relu=False, final_pool=False, input_dim=None):
    ...
    for i in range(num_blocks):
        ...
        dim = 4*feature_dim
    if final_conv:
        feat_layers.append(nn.Conv2d(dim, out_dim, kernel_size=3, padding=1, bias=False))
        if final_relu:
            feat_layers.append(nn.ReLU(inplace=True))
        if final_pool:
            feat_layers.append(nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
...
def conv(feature_dim=256, l2norm=True, norm_scale=1.0, out_dim=None,
         final_relu=False, stride=1):
    if out_dim is None:
        out_dim = feature_dim
    feat_layers = []

    feat_layers.append(nn.Conv2d(feature_dim, out_dim, kernel_size=3, padding=1, bias=False, stride=stride))
    if final_relu:
        feat_layers.append(nn.ReLU(inplace=True))
