...
import ltr.models.lwtl.linear_filter as target_clf
import ltr.models.target_classifier.features as clf_features
import ltr.models.lwtl.initializer as seg_initializer
import ltr.models.lwtl.label_encoder as seg_label_encoder
import ltr.models.lwtl.loss_residual_modules as loss_residual_modules
import ltr.models.lwtl.decoder as lwtl_decoder
...
class LWTLNet(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_masks, test_masks, num_refinement_iter=2):
        ...
        mask_encoding_pred = [self.target_model.apply_target_model(f, test_feat_tm) for f in filter_iter]
        mask_encoding_pred_last_iter = mask_encoding_pred[-1]

        # Run decoder to obtain the segmentation mask
        mask_pred, decoder_feat = self.decoder(mask_encoding_pred_last_iter, test_feat_backbone,
                                               test_imgs.shape[-2:])
    ...
    def extract_features(self, im, layers=None):
        ...
        if 'classification' not in layers:
            return self.feature_extractor(im, layers)
        backbone_layers = sorted(list(set([l for l in layers + self.target_model_input_layer
                                           if l != 'classification'])))
        all_feat = self.feature_extractor(im, backbone_layers)
        all_feat['classification'] = self.extract_target_model_features(all_feat)
