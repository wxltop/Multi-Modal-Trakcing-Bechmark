...
class AtomIoUNetNoMod(nn.Module):
    ...
    def forward(self, feat1, feat2, bb1, proposals2):
        ...
        feat2 = [f.reshape(batch_size * num_test_images, f.shape[2], f.shape[3], f.shape[4]) for f in feat2]
        iou_feat = self.get_iou_feat(feat2)

        proposals2 = proposals2.reshape(batch_size*num_test_images, -1, 4)
        pred_iou = self.predict_iou(None, iou_feat, proposals2)
        return pred_iou.reshape(num_test_images, batch_size, -1)
    ...
    def predict_iou(self, filter, feat2, proposals):
        ...
        # Add batch_index to rois
        batch_index = torch.Tensor([x for x in range(batch_size)]).reshape(batch_size, 1).to(c3_t.device)
        ...
        roi4t = self.prroi_pool4t(c4_t, roi2)
    ...
    def get_modulation(self, feat1, bb1):
        return None, None
    ...
    def optimize_boxes(self, filter, iou_feat, proposals, step_length=1, num_iterations=5):
        # Optimize iounet boxes
        output_boxes = proposals.clone()
        ...
        for i_ in range(num_iterations):
            ...
            outputs = self.predict_iou(filter, iou_feat, bb_init)
        ...
        return output_boxes.detach(), outputs.detach()


class AtomIoUNetBaby(nn.Module):
    """Network module for IoU prediction. Refer to the ATOM paper for an illustration of the architecture.
    It uses two backbone feature layers as input.
    args:
        input_dim:  Feature dimensionality of the two input backbone layers.
    ...
    def __init__(self, input_dim=(32,64), pred_input_dim=(64,64), pred_inter_dim=(64,64)):
        super().__init__()
        # _r for reference, _t for test
    ...
    def get_iou_feat(self, feat2):
        ...
        c3_t = self.conv3_1t(feat3_t)
        c4_t = self.conv4_1t(feat4_t)
        # c3_t = self.conv3_2t(self.conv3_1t(feat3_t))
        # c4_t = self.conv4_2t(self.conv4_1t(feat4_t))
...
class AtomIoUNetHR(nn.Module):
    ...
    def __init__(self, input_dim=(128,256), pred_input_dim=(256,256), pred_inter_dim=(256,256)):
        super().__init__()
        # _r for reference, _t for test
        self.conv3_1r = conv(input_dim[0], 256, kernel_size=3, stride=1)
        self.conv3_1t = conv(input_dim[0], 256, kernel_size=3, stride=1)
        ...
        self.fc3_1r = conv(256, 256, kernel_size=3, stride=1, padding=0)
        self.fc3_2r = conv(256, 256, kernel_size=3, stride=1, padding=0)
        ...
        self.prroi_pool4r = PrRoIPool2D(3, 3, 1 / 16)
        self.prroi_pool4t = PrRoIPool2D(4, 4, 1 / 16)

        self.fc4_1r = conv(256, 256, kernel_size=3, stride=1, padding=0)

        self.fc34_3r = conv(256 + 256, pred_input_dim[0], kernel_size=1, stride=1, padding=0)
        self.fc34_4r = conv(256 + 256, pred_input_dim[1], kernel_size=1, stride=1, padding=0)

        self.fc3_rt = LinearBlock(pred_input_dim[0], pred_inter_dim[0], 7)
        self.fc4_rt = LinearBlock(pred_input_dim[1], pred_inter_dim[1], 4)
    ...
    def get_modulation(self, feat, bb):
        ...
        fc3_r = self.fc3_1r(roi3r)
        fc3_2r = self.fc3_2r(fc3_r)

        fc4_r = self.fc3_1r(roi4r)

        # Concatenate from block 3 and 4
        fc34_r = torch.cat((fc3_2r, fc4_r), dim=1)
