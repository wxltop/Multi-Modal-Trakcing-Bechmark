...
class Transform:
    ...
    The following arguments are inputs to be transformed. They are either supplied as a single instance, or a list of instances.
        image  -  Image
        img_coords  -  2xN dimensional Tensor of 2D image coordinates [y, x]
    ...
    The following parameters can be supplied with calling the transform object:
        joint [Bool]  -  If True then transform all images/img_coords/bbox/mask in the list jointly using the same transformation.
                         Otherwise each tuple (images, img_coords, bbox, mask) will be transformed independently using
    ...
    def __init__(self, *transforms):
        ...
        self._valid_inputs = ['image', 'img_coords', 'bbox', 'mask']
    ...
    def __call__(self, **inputs):
        ...
        out_valid = {k: v for k, v in inputs.items() if k in self._valid_inputs and v is not None}
        out_empty = {k: v for k, v in inputs.items() if k in self._valid_inputs and v is None}

        for t in self.transforms:
            out_valid = t(**out_valid, joint=joint_mode, new_roll=new_roll)

        out = {**out_valid, **out_empty}
    ...
    def __call__(self, **inputs):
        ...
        for var_name, var in input_vars.items():
            if var is not None:
                ...
                if var_name in ['img_coords', 'bbox']:
...
class RandomBlur(TransformBase):
    ...
    def transform_image(self, image, do_blur=None):
        if do_blur is None:
            do_blur = False
