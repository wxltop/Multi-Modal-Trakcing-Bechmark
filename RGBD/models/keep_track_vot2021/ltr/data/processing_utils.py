...
def sample_target_adaptive(im, target_bb, search_area_factor, output_sz, mode: str = 'replicate',
    ...
    # Crop image
    crop_sz_x, crop_sz_y = (output_sz * (target_bb[2:].prod()/output_sz.prod()).sqrt() * search_area_factor).ceil().long().tolist()
...
def crop_and_resize(im, box, crop_bb, output_sz, mask=None, border_mode='replicate'):
    if isinstance(output_sz, (float, int)):
        output_sz = (output_sz, output_sz)
    ...
    if border_mode == 'replicate':
        cv_border_mode = cv.BORDER_REPLICATE
    elif border_mode == 'zeros':
        cv_border_mode = cv.BORDER_CONSTANT
    else:
        raise Exception

    im_crop_padded = cv.copyMakeBorder(im_crop, y1_pad, y2_pad, x1_pad, x2_pad, cv_border_mode)
...
def target_image_crop(frames, box_extract, box_gt, search_area_factor, output_sz, mode: str = 'replicate',
    ...
    if masks is None:
        ...
        frame_crops_boxes_masks = [sample_target_adaptive(f, a, search_area_factor, output_sz, mode, max_scale_change, mask=m)
                                   for f, a, m in zip(frames, box_extract, masks)]
...
def sample_target_fpn(im, target_bb, resize_factor, output_sz):
    # output sz: (width, height)
    if isinstance(output_sz, (float, int)):
        output_sz = (output_sz, output_sz)
    output_sz = torch.Tensor(output_sz)

    output_sz_base = (output_sz / resize_factor).int().tolist()
    ...
    x1_lo = max(bbx + bbw - output_sz_base[0], min(0, im_w - output_sz_base[0]))
    x1_hi = min(bbx, max(im_w - output_sz_base[0], 0))

    y1_lo = max(bby + bbh - output_sz_base[1], min(0, im_h - output_sz_base[1]))
    y1_hi = min(bby, max(im_h - output_sz_base[1], 0))

    crop_x1 = random.randint(int(x1_lo), int(max(x1_lo, x1_hi)))
    crop_y1 = random.randint(int(y1_lo), int(max(y1_lo, y1_hi)))

    crop_x2 = crop_x1 + output_sz_base[0]
    crop_y2 = crop_y1 + output_sz_base[1]

    if output_sz_base[0] < 1 or output_sz_base[1] < 1:
        raise Exception('Too small bounding box.')

    x1_pad = max(0, -crop_x1)
    x2_pad = max(crop_x2-im.shape[1]+1, 0)

    y1_pad = max(0, -crop_y1)
    y2_pad = max(crop_y2-im.shape[0]+1, 0)

    # Crop target
    im_crop = im[crop_y1+y1_pad:crop_y2-y2_pad, crop_x1+x1_pad:crop_x2-x2_pad, :]
    ...
    crop_box = torch.Tensor([crop_x1, crop_y1, crop_x2-crop_x1, crop_y2-crop_y1])
    return im_out, crop_box


def target_image_crop_adaptive(frames, box_gt, resize_factor, output_sz):
    if isinstance(output_sz, (float, int)):
        output_sz = (output_sz, output_sz)

    frame_crops_boxes = [sample_target_fpn(f, a, rf, output_sz) for f, a, rf, in zip(frames, box_gt, resize_factor)]
...
def iou_gen(reference, proposals):
    assert reference.dim() == proposals.dim()
...
def ncep_sample_gmm_centered(std, beta, num_samples=1):
    """Sample from a GMM distribution centered at zero:
    args:
        std - Tensor of standard deviations
        num_samples - number of samples
...
def ncep_sample_box_gmm(mean_box, proposal_sigma, beta=0.1, gt_sigma=None, num_samples=1, add_mean_box=False):
    """Sample boxes from a Gaussian mixture model.
    args:
        mean_box - Center (or mean) bounding box
        proposal_sigma - List of standard deviations for each Gaussian
    ...
    proposals_rel_centered_0, proposal_density_0 = ncep_sample_gmm_centered(std, beta, 1)
    # (proposals_rel_centered_0 has shape: (1, 4))
    # (proposal_density_0 has shape: (1))


    # Add mean and map back
    mean_box_rel = rect_to_rel(mean_box, sz_norm)
    proposals_rel = proposals_rel_centered + mean_box_rel # (shape: (num_samples, 4))
    proposals = rel_to_rect(proposals_rel, sz_norm) # (shape: (num_samples, 4))

    proposals_rel_0 = proposals_rel_centered_0 + mean_box_rel # (shape: (1, 4))
    proposals_0 = rel_to_rect(proposals_rel_0, sz_norm) # (shape: (1, 4))
    ...
    proposals = torch.cat((proposals_0, proposals)) # (shape: (1+num_samples, 4))
    proposal_density = torch.cat((proposal_density_0, proposal_density)) # (shape: (1+num_samples))
    gt_density = torch.cat((torch.Tensor([1]), gt_density)) # (shape: (1+num_samples))
