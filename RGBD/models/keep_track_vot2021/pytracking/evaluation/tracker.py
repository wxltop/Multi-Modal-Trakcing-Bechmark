...
class FrameReader:
    def __init__(self, seq_info, cache_frames=False):
    ...
    def get_frame(self, frame_number):
        ...
        if frame is None:
            frame = self._read_image(self.frame_names[frame_number])
    ...
    def get_bbox(self, frame_number, object_id):
...
class Tracker:
    ...
    def run_sequence(self, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None,
        ...
        args:
            ...
            cache_image_frames: Cache already loaded image frames in case they are read again.
        """
        ...
        frame_reader = FrameReader(seq, cache_image_frames)

        if multiobj_mode == 'default' or is_single_object:
            tracker = self.create_tracker(params)
            tracker.frame_reader = frame_reader
        elif multiobj_mode == 'parallel':
            tracker = MultiObjectWrapper(self.tracker_class, params, self.visdom, frame_reader=frame_reader)
        else:
            raise ValueError('Unknown multi object mode {}'.format(multiobj_mode))

        output = self._track_sequence(tracker, frame_reader, init_info)
        return output

    def _track_sequence(self, tracker, frame_reader, init_info):
        # Define outputs
    ...
    def run_vot2020(self, debug=None, visdom_info=None):
        ...
        while True:
            ...
            if 'object_presence_score' in out:
                object_presence_score = out['object_presence_score']

            handle.report(pred, object_presence_score)
    ...
    def run_vot(self, debug=None, visdom_info=None):
        ...
        while True:
            ...
            state = out['target_bbox']
            confidence = out['object_presence_score'] if 'object_presence_score' in out else None

            handle.report(vot.Rectangle(state[0], state[1], state[2], state[3]), confidence=confidence)
