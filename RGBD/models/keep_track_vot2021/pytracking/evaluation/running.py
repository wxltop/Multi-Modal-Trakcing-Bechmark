...
def _save_tracker_output_oxuva(seq: Sequence, tracker: Tracker, output: dict):
    ...
    img_h, img_w = output['image_shape'][0]
    ...
    with open(pred_file, 'w') as fp:
        ...
        for i in range(0, len(frame_numbers)):
            row = {
                ...
                'present': str(object_presence_scores[i] > 0.55).lower(),  # True or False
...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict, save_time_to_disk: bool):
    ...
    for key, data in output.items():
        ...
        if key == 'target_bbox':
        ...
        elif key == 'time' and save_time_to_disk:
            ...
            for frame_name, frame_seg in zip(frame_names, data):
                # TODO convert to uint8 in the tracker?
                imwrite_indexed(os.path.join(segmentation_path, '{}.png'.format(frame_name)), frame_seg.astype(np.uint8))

def run_sequence(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None, save_time=True, is_oxuva=False,
                 cache_image_frames=False):
    ...
    if debug:
        output = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info, cache_image_frames=False)
    else:
        try:
            output = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info, cache_image_frames=cache_image_frames)
        except Exception:
            traceback.print_exc()
            return
    ...
    if not debug:
        if not is_oxuva:
            _save_tracker_output(seq, tracker, output, save_time_to_disk=save_time)
        else:
            _save_tracker_output_oxuva(seq, tracker, output)


def run_dataset(dataset, trackers, debug=False, threads=0, visdom_info=None, save_time=True, is_oxuva=False):
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug, visdom_info=visdom_info, save_time=save_time,
                             is_oxuva=is_oxuva, cache_image_frames=False)
    elif mode == 'parallel':
        cache_img_frames = True
        param_list = [(seq, tracker_info, debug, visdom_info, save_time, is_oxuva, cache_img_frames)
