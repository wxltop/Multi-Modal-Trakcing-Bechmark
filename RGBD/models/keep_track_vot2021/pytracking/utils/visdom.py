    ...
    def generate_display_text(self, data):
        ...
        for key, value in data.items():
            ...
            if value is None:
                ...
                display_text += '<b>{}</b>: {:.3f}<br>'.format(key, value)
...
class VisLinePlot(VisBase):
    ...
    def draw_data(self):
        ...
        get_data_y = lambda y: (y[0] if isinstance(y, (list, tuple)) else y).clone()

        if isinstance(self.raw_data, dict):
            names = list(self.raw_data.keys())
            opts['legend'] = names
            data_y = torch.stack([get_data_y(v) for v in self.raw_data.values()], dim=-1)
            if isinstance(self.raw_data[names[0]], (list, tuple)):
                data_x = torch.stack([v[1].clone() for v in self.raw_data.values()], dim=-1)
        else:
            data_y = get_data_y(self.raw_data)
            if isinstance(self.raw_data, (list, tuple)):
                data_x = self.raw_data[1].clone()
        if data_x is None:
            data_x = torch.arange(data_y.shape[0])

        self.visdom.line(data_y, data_x, opts=opts, win=self.title)
