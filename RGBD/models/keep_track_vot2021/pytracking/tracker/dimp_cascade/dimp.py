...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # Initialize IoUNet
        if self.params.get('bbreg_mode', 'iou') == 'iou':
            self.init_iou_net(init_backbone_feat)
        elif self.params.get('bbreg_mode', 'iou') == 'cascade':
            self.init_bbr_net(init_backbone_feat)
        elif self.params.get('bbreg_mode', 'iou') == 'cascade_iou':
            self.init_iou_net(init_backbone_feat)
            self.init_bbr_net(init_backbone_feat)
    ...
    def get_iou_backbone_features(self, backbone_feat):
        return self.net.get_backbone_iou_feat(backbone_feat)

    def get_bbr_backbone_features(self, backbone_feat):
        return self.net.get_backbone_bbreg_feat(backbone_feat)

    def get_iou_features(self, backbone_feat):
        with torch.no_grad():
            return self.net.iou_regressor.get_iou_feat(self.get_iou_backbone_features(backbone_feat))

    def get_iou_modulation(self, iou_backbone_feat, target_boxes):
        with torch.no_grad():
            return self.net.iou_regressor.get_modulation(iou_backbone_feat, target_boxes)

    def get_bbr_features(self, backbone_feat):
        with torch.no_grad():
            return self.net.bb_regressor.get_iou_feat(self.get_bbr_backbone_features(backbone_feat))

    def get_bbr_modulation(self, bbr_backbone_feat, target_boxes):
        with torch.no_grad():
            return self.net.bb_regressor.get_modulation(bbr_backbone_feat, target_boxes)
    ...
    def init_iou_net(self, backbone_feat):
        ...
        for p in self.net.iou_regressor.parameters():
    ...
    def init_bbr_net(self, backbone_feat):
        # Setup IoU net and objective
        ...
        # Get iou features
        bbr_backbone_feat = self.get_bbr_backbone_features(backbone_feat)

        # Remove other augmentations such as rotation
        bbr_backbone_feat = TensorList([x[:target_boxes.shape[0],...] for x in bbr_backbone_feat])

        # Get modulation vector
        self.bbr_modulation = self.get_bbr_modulation(bbr_backbone_feat, target_boxes)
        if torch.is_tensor(self.bbr_modulation[0]):
            self.bbr_modulation = TensorList([x.detach().mean(0) for x in self.bbr_modulation])
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        if self.params.get('bbreg_mode', 'iou') == 'iou':
            predicted_box = self.optim_box_regression(backbone_feat, init_box, scale_ind)
        elif self.params.get('bbreg_mode', 'iou') == 'cascade':
            predicted_box = self.direct_box_regression(backbone_feat, init_box, scale_ind)
        elif self.params.get('bbreg_mode', 'iou') == 'cascade_iou':
            init_box_ = self.direct_box_regression(backbone_feat, init_box, scale_ind)
            if init_box_ is not None:
                init_box = init_box_
            predicted_box = self.optim_box_regression(backbone_feat, init_box, scale_ind)
        else:
            raise Exception
    ...
    def optim_box_regression(self, backbone_feat, init_box, scale_ind):
    ...
    def optimize_boxes_default(self, iou_features, init_boxes):
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            outputs = self.net.iou_regressor.predict_iou(self.iou_modulation, iou_features, bb_init)
    ...
    def direct_box_regression(self, backbone_feat, init_box, scale_ind):
        """Implementation of direct bounding box regression."""
        # Extract features from the relevant scale
        bbr_features = self.get_bbr_features(backbone_feat)
        bbr_features = TensorList([x[scale_ind:scale_ind+1,...] for x in bbr_features])

        # Generate random initial boxes
        init_boxes = init_box.view(1, 1, 4).clone().to(self.params.device)

        # Optimize the boxes
        output_boxes = self.net.bb_regressor.predict_bb(self.bbr_modulation, bbr_features, init_boxes).view(-1,4).cpu()
