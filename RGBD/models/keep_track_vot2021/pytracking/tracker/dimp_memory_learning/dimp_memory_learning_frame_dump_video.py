...
class DiMPMemoryLearning(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if self.visdom is not None:
            ...
            if self.params.get('enable_peak_localization_by_matching', False):
                ...
                self.draw_target_candidates_on_image(image, search_area_box, output_state)
            ...
            # ax.imshow(im_patches[0].permute((1,2,0))/255.)
            # self.visdom.visdom.matplot(plt,
            #                            opts={'title': 'im_patch'},
            #                            win='im_patch')
            # plt.close(fig)
    ...
    def draw_target_candidates_on_image(self, image, search_area_box, output_state):
        colors = ['deepskyblue', 'darkorange', 'darkviolet', 'brown', 'fuchsia', 'blue',
                  'gold', 'pink', 'cyan', 'peru', 'black', 'salmon', 'mediumpurple', 'yellow', 'purple']
        img = np.array(image)

        # fig, ax = plt.subplots(figsize=(10, 5))

        radius = int(np.max(image.shape)/50)
        linewidth = 2

        x, y, w, h = search_area_box.tolist()

        for peak_id, peak in self.peak_collection.d.items():
            obj_id = peak.object_id

            if self.peak_collection.selected_object_id == obj_id:
                continue

            color = colors[obj_id % len(colors)]
            color = 255 * np.array(matplotlib.colors.to_rgba(color))
            color = color.astype(int).tolist()

            ts_coords = peak.peak_ts_coords[-1]
            im_coords = np.round(np.array([
                h * (float(ts_coords[0]) / (30 - 1)) + y,
                w * (float(ts_coords[1]) / (30 - 1)) + x,
            ]), 0).astype(int)

            cv2.circle(img, (im_coords[1], im_coords[0]), radius, color, -1)

        bbox_gth = self.frame_reader.get_bbox(self.frame_num - 1, None)

        color = 255 * np.array(matplotlib.colors.to_rgba('red'))
        color = color.astype(int).tolist()
        bx, by, bw, bh = np.round(np.array(bbox_gth), 0).astype(int)
        cv2.rectangle(img, (bx, by), (bx + bw, by + bh), color, linewidth)

        color = 255 * np.array(matplotlib.colors.to_rgba('lime'))
        color = color.astype(int).tolist()
        bx, by, bw, bh = np.round(np.array(output_state), 0).astype(int)
        cv2.rectangle(img, (bx, by), (bx + bw, by + bh), color, linewidth)
        cv2.circle(img, (bx+bw//2, by+bh//2), radius, color, -1)



        # ax.imshow(img.astype(np.uint8))

        cv2.imwrite('/scratch_net/clariden/chmayer/Videos/pool-3/im{:04d}.jpg'.format(self.frame_num-1), cv2.cvtColor(img, cv2.COLOR_RGB2BGR))

        # self.visdom.visdom.matplot(plt, opts={'title': 'image with distractors'},
        #                            win='image with distractors')
        # plt.close(fig)
