...
class DiMP(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        scores_mean, scores_var, scores_final = self.classify_target(test_x)

        # Localize the target
        translation_vec, scale_ind, s, flag = self.localize_target(scores_mean, sample_pos, sample_scales)
        ...
        if self.visdom is not None:
            ...
            self.visdom.register(scores_mean[scale_ind,...], 'heatmap', 3, 'Score Mean' + self.id_str)
            self.visdom.register(scores_var[scale_ind,...], 'heatmap', 3, 'Score Var' + self.id_str)
            self.visdom.register(scores_final[scale_ind,...], 'heatmap', 3, 'Score Final' + self.id_str)
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            scores_mean = self.net.classifier.classify(self.target_filter, sample_x)
            scores_var = self.net.classifier.get_response_variance(self.target_filter_var, sample_x)
            scores_final = self.net.classifier.final_response(scores_mean, scores_var)
        return scores_mean, scores_var, scores_final
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        with torch.no_grad():
            ...
            self.target_filter_var = self.net.classifier.get_filter_variance(self.target_filter, x, target_boxes)
        ...
        if plot_loss:
            ...
            if self.visdom is not None:
                self.visdom.register((self.losses, torch.arange(self.losses.numel())), 'lineplot', 4, 'Training Loss' + self.id_str)
    ...
    def update_classifier(self, train_x, target_box, learning_rate=None, scores=None):
        ...
        if num_iter > 0:
            ...
            with torch.no_grad():
                ...
                self.target_filter_var = self.net.classifier.get_filter_variance(self.target_filter, samples, target_boxes, sample_weights)
