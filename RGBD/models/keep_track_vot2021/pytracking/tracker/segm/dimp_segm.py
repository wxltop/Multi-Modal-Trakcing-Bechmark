...
# Get target position and size
        ...
        # Initialize SegmNet
        self.init_segm_net(init_backbone_feat)

        # Loc prior
        self.use_loc_prior = self.params.get('loc_prior', False)
        self.loc_prior_padding = self.params.get('loc_prior_padding', 0)
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        # Code for saving heat map. TODO: Make this cleaner
        # scores_raw_vis = scores_raw.clone()
        # scores_raw_vis = F.interpolate(scores_raw_vis, (288, 288))
        # scores_raw_vis = F.relu(scores_raw_vis)
        # # scores_raw_vis = scores_raw_vis / scores_raw_vis.max()
        # if not hasattr(self, 'colormap'):
        #     cmap = cm.viridis
        #
        #     self.colormap = torch.zeros(1005, 3).to(scores_raw.device)
        #     for v in range(0, 1005):
        #         val = v / 1000.0
        #         self.colormap[v, :] = torch.tensor(cmap(val)[:3]).to(scores_raw.device)
        #
        # scores_raw_ids = (scores_raw_vis.view(-1) * 1000).long()
        # scores_raw_ids = scores_raw_ids.clamp(0, 1000)
        # if scores_raw_ids.min() < 0 or scores_raw_ids.max() >= 5001:
        #     yo = 1
        # scores_raw_mapped = self.colormap[scores_raw_ids, :].view(288, 288, 3)
        # scores_raw_mapped = (scores_raw_mapped * 255).int()
        # scores_raw_mapped = scores_raw_mapped[:, :, [2,1,0]]
        # scores_raw_mapped_cv = scores_raw_mapped.cpu().numpy()
        # cv2.imwrite('/home/goutam/dimp_vid/vid1/{:04d}_scores.png'.format(self.frame_num), scores_raw_mapped_cv)
        # cv_image = im_patches[0].cpu().permute(1, 2, 0).numpy()
        # # draw_box = predicted_box.clone()
        # # draw_box[2:] = draw_box[:2] + draw_box[2:]
        # # draw_box = draw_box.cpu().int().tolist()
        # # cv_image = cv2.rectangle(cv_image, (draw_box[0], draw_box[1]), (draw_box[2], draw_box[3]), (255, 0, 0), 1)
        # cv2.imwrite('/home/goutam/dimp_vid/vid1/{:04d}.png'.format(self.frame_num), cv_image)
        ...
        segmentation = self.get_segmentation(backbone_feat, target_box)
        ...
        if self.visdom is not None:
            ...
            self.visdom.register(segm_map[0, 0], 'heatmap', 1, 'Segmentation Map' + self.id_str)
        ...
        # Compute segmentation map (float tensor) in the input domain
        segmentation = torch.zeros(self.image_sz.int().tolist())
        # Upsample segm_map to the search window size and copy segm_map to segmentation using the clamped search window area
        H, W = self.image_sz
        search_window_coords = sample_coords[scale_ind, [1, 0, 3, 2]]
        s_x, s_y = search_window_coords[2:] - search_window_coords[:2]
        segm_map = F.interpolate(segm_map, (int(s_y), int(s_x)), mode="bilinear", align_corners=False)
        search_window_coords_clamped = search_window_coords.clamp(min=0).min(search_window_coords.new_tensor([W, H, W, H]))
        offset_x, offset_y = search_window_coords_clamped[:2] - search_window_coords[:2]
        s_x, s_y = search_window_coords_clamped[2:] - search_window_coords_clamped[:2]
        x1, y1, x2, y2 = search_window_coords_clamped
        # TODO: perhaps also crop segmentation using the predicted bbox (nothing outside the bbox)
        segmentation[y1:y2, x1:x2].copy_(segm_map[0, 0, offset_y:offset_y+s_y, offset_x:offset_x+s_x])

        out = {'target_bbox': new_state.tolist(), 'segmentation': segmentation.numpy()}
        return out
    ...
    def get_segmentation(self, backbone_feat, target_box):
        with torch.no_grad():
            loc_prior_masks = None
            if self.use_loc_prior:
                bb = target_box.unsqueeze(0)
                loc_prior_masks = self.net.get_loc_prior_masks(backbone_feat, bb, padding=self.loc_prior_padding)
            segm_pred = self.net.segm_predictor.predict_segmentation(self.segm_modulation, backbone_feat, loc_prior_masks)

            # Upscale to the original image size
            # segm_pred = F.interpolate(segm_pred, scale_factor=2, mode='bilinear', align_corners=False)
    ...
    def get_segm_modulation(self, segm_backbone_feat, target_boxes):
        with torch.no_grad():
            return self.net.segm_predictor.get_modulation(segm_backbone_feat, target_boxes)
    ...
    def init_segm_net(self, backbone_feat):
        # Setup SegmNet and objective
        for p in self.net.segm_predictor.parameters():
            p.requires_grad = False
        ...
        # Remove other augmentations such as rotation
        backbone_feat = OrderedDict({lname: x[:target_boxes.shape[0], ...] for lname, x in backbone_feat.items()})

        # Get modulation vector
        self.segm_modulation = self.get_segm_modulation(backbone_feat, target_boxes)
        self.segm_modulation = OrderedDict({lname: x.detach().mean(0).unsqueeze(0) for lname, x in self.segm_modulation.items()})
    ...
    def visdom_draw_tracking(self, image, box, segmentation=None):
        ...
        if hasattr(self, 'search_area_box'):
            data.append(self.search_area_box)
