...
class PrevStateData:
    ...
    def insert_data(self, frame_number, feat, state, im, label, bb_patch):
        self.info_dict_list = {'frame_number': frame_number, 'feat': feat, 'state': state, 'im': im, 'label': label,
                               'bb_patch': bb_patch}

    def get_data(self):
        return self.info_dict_list['feat'], self.info_dict_list['state'], self.info_dict_list['im'], \
               self.info_dict_list['label'], self.info_dict_list['bb_patch']
...
# Get target position and size
        ...
        if not self.params.get('use_only_dimp', False):
            ...
            self.prev_state_handler = PrevStateData()
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if not self.params.get('use_only_dimp', False):
            if flag != 'not_found' or self.params.get('update_motion_state_during_occlusion', True):
                ...
                self.prev_state_handler.insert_data(self.frame_num, motion_feat, new_state_vector, test_patch, None,
                                                    box_patch)
    ...
    def init_motion_module(self, im):
        ...
        self.prev_state_handler.insert_data(0, motion_feat, None, test_patch, prev_label, box_patch)
    ...
    def get_response_prediction(self, backbone_feat, sample_scale, scores_dimp, prev_box_patch):
        ...
        if self.visdom is not None:
            ...
            self.visdom.register(aux_outputs['propagation_weights'], 'cost_volume', 3, 'propagation_weights')
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        if self.visdom is not None and self.params.debug > 1:
