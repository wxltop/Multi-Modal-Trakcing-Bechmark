...
from pytracking.utils.plotting import show_tensor, show_image_state
from pytracking.libs.optimization import GaussNewtonCG, ConjugateGradient, GradientDescentL2, NewtonCG, SoftmaxCECG
from pytracking.features.preprocessing import sample_patch
from .optim import ConvProblem, ConvProblemCE, ConvProblemJoint, ConvProblemLogistic, FactorizedConvProblem, \
    FactorizedConvProblemCE, FactorizedConvProblemJoint, FactorizedConvProblemLogistic, FactorizedConvProblemHinge, ConvProblemHinge, \
    FactorizedConvProblemSoftmaxCE, ConvProblemSoftmaxCE
...
class ATOM(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        if self.params.get('window_output', False):
            if self.params.get('use_clipped_window', False):
                self.output_window = dcf.hann2d_clipped(self.output_sz.long(), self.output_sz.long()*self.params.effective_search_area / self.params.search_area_scale, centered=True).to(self.params.device)
            else:
    ...
    def init_optimization(self, train_x, init_y):
        ...
        optimization_problem = self.params.get('factorized_problem', 'L2')
        ...
        if self.params.update_projection_matrix:
            ...
            elif optimization_problem == 'L2Hinge':
                init_mask = (init_y >= self.params.hinge_threshold).float()
                self.joint_problem = FactorizedConvProblemHinge(self.init_training_samples, init_y, self.filter_reg,
                                                           self.fparams.attribute('projection_reg'), self.params, self.init_sample_weights,
                                                           self.projection_activation, self.response_activation, init_mask)
            elif optimization_problem == 'L2joint':
                self.joint_problem = FactorizedConvProblemJoint(self.init_training_samples, max(init_y, key=lambda e: e.shape[-2]), self.filter_reg, self.fparams.attribute('projection_reg'),
                                                                self.params, self.init_sample_weights[-1],
                                                                self.projection_activation, self.response_activation)
            elif optimization_problem == 'logistic':
                self.joint_problem = FactorizedConvProblemLogistic(self.init_training_samples, init_y, self.filter_reg,
                                                                   self.fparams.attribute('projection_reg'), self.params, self.init_sample_weights,
                                                                   self.projection_activation)
            elif optimization_problem == 'crossentropy':
                self.joint_problem = FactorizedConvProblemCE(self.init_training_samples, init_y, self.filter_reg,
                                                             self.fparams.attribute('projection_reg'), self.params, self.init_sample_weights,
                                                             self.projection_activation)
            elif optimization_problem == 'SoftmaxCE':
                self.joint_problem = FactorizedConvProblemSoftmaxCE(self.init_training_samples, init_y, self.filter_reg,
                                                           self.fparams.attribute('projection_reg'), self.params, self.init_sample_weights,
                                                           self.projection_activation, self.response_activation, self.params.get('softmax_reg', None),
                                                                    self.params.get('label_shrink', 0), self.params.get('label_threshold', 0))
            ...
            if optimizer == 'GaussNewtonCG':
                ...
                self.joint_optimizer = GradientDescentL2(self.joint_problem, joint_var,
                                                         ...
                                                         debug=(self.params.debug >= 1 or analyze_convergence),
                                                         visdom=self.visdom)
            elif optimizer == 'NewtonCG':
                self.joint_optimizer = NewtonCG(self.joint_problem, joint_var, init_hessian_reg=self.params.hessian_reg,
                                                hessian_reg_factor=self.params.hessian_reg_factor, plotting=(self.params.debug >= 3), analyze=analyze_convergence)
            elif optimizer == 'SoftmaxCECG':
                self.joint_optimizer = SoftmaxCECG(self.joint_problem, joint_var, debug=(self.params.debug >= 1),
                                                     plotting=(self.params.debug >= 3), analyze=analyze_convergence,
                                                     visdom=self.visdom)
        ...
        if optimization_problem == 'L2':
            ...
            self.hinge_mask = (self.y >= self.params.hinge_threshold).float()
            self.conv_problem = ConvProblemHinge(self.training_samples, self.y, self.filter_reg, self.sample_weights, self.response_activation, self.hinge_mask)
        elif optimization_problem == 'L2joint':
            self.conv_problem = ConvProblemJoint(self.training_samples, max(self.y, key=lambda e: e.shape[-2]), self.filter_reg, self.sample_weights[-1],
                                                 self.response_activation)
        elif optimization_problem == 'logistic':
            self.conv_problem = ConvProblemLogistic(self.training_samples, self.y, self.filter_reg, self.sample_weights)
        elif optimization_problem == 'crossentropy':
            self.conv_problem = ConvProblemCE(self.training_samples, self.y, self.filter_reg, self.sample_weights)
        elif optimization_problem == 'SoftmaxCE':
            self.conv_problem = ConvProblemSoftmaxCE(self.training_samples, self.y, self.filter_reg, self.sample_weights, self.response_activation, self.params.get('softmax_reg', None),
                                                                    self.params.get('label_shrink', 0), self.params.get('label_threshold', 0))
        ...
        if optimizer == 'GaussNewtonCG':
            ...
            self.filter_optimizer = NewtonCG(self.conv_problem, self.filter, init_hessian_reg=self.params.hessian_reg, fletcher_reeves=self.params.fletcher_reeves,
                                             direction_forget_factor=self.params.direction_forget_factor, plotting=(self.params.debug>=3))
            self.filter_optimizer.hessian_reg = self.joint_optimizer.hessian_reg
        elif optimizer == 'SoftmaxCECG':
            self.filter_optimizer = SoftmaxCECG(self.conv_problem, self.filter,
                                                      fletcher_reeves=self.params.fletcher_reeves,
                                                      direction_forget_factor=self.params.direction_forget_factor,
                                                      debug=(self.params.debug >= 1),
                                                      plotting=(self.params.debug >= 3), visdom=self.visdom)
    ...
    def localize_target(self, scores_raw):
        if self.params.get('use_fourier_interpolation', True):
            ...
            if preprocess_method == 'none':
                ...
                reg_val = self.params.get('softmax_reg', None)
                scores_view = scores.reshape(scores.shape[0], -1)
                scores_softmax = activation.softmax_reg(scores_view, dim=-1, reg=reg_val)
                scores = scores_softmax.reshape(scores.shape)
            else:
    ...
    def refine_target_box(self, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        ## Code for generating plot in DCTD
        # self.visualize_iou_pred(iou_features, predicted_box)
        # TODO refactor this into a visualization function
        if self.params.debug >= 3 and False:
            corner_scores = self.visualize_iou_pred_corner(iou_features, predicted_box)
            corner_scores = corner_scores / corner_scores.max()
            corner_scores_shape = corner_scores.shape
            if not hasattr(self, 'colormap'):
                cmap = cm.viridis

                self.colormap = torch.zeros(1001, 3).to(corner_scores.device)
                for v in range(0, 1001):
                    val = v / 1000.0
                    self.colormap[v, :] = torch.tensor(cmap(val)[:3]).to(corner_scores.device)

            corner_scores_ids = (corner_scores.view(-1) * 1000).long()
            corner_scores_mapped = self.colormap[corner_scores_ids, :].view(*corner_scores_shape, 3).permute(2, 0, 1).contiguous()
            corner_scores_mapped = (corner_scores_mapped * 255).int()

            predicted_box_tr = (predicted_box[1].long(), predicted_box[0].long() + predicted_box[2].long())

            im_crop = sample_patch(self.im, sample_pos, sample_scale * self.iou_img_sample_sz, self.iou_img_sample_sz)
            im_crop = im_crop[0].int().squeeze(0)

            max_corner_disp = (corner_scores.shape[0] - 1) / 2

            mask = (corner_scores).unsqueeze(0).float().cpu() * 0.5
            mask = (corner_scores > -1).unsqueeze(0).float().cpu() * 0.5

            draw_box = predicted_box.clone()
            draw_box[2:] = draw_box[:2] + draw_box[2:]
            draw_box = draw_box.cpu().int().tolist()

            r_select1 = torch.arange(draw_box[1], draw_box[3], 6)
            r_select2 = torch.arange(draw_box[1]+1, draw_box[3], 6)
            r_select3 = torch.arange(draw_box[1]+2, draw_box[3], 6)

            r_select = torch.cat((r_select1, r_select2, r_select3))

            c_select1 = torch.arange(draw_box[0], draw_box[2], 6)
            c_select2 = torch.arange(draw_box[0] + 1, draw_box[2], 6)
            c_select3 = torch.arange(draw_box[0] + 2, draw_box[2], 6)

            c_select = torch.cat((c_select1, c_select2, c_select3))

            color_tensor = torch.tensor([255, 0, 0]).view(3, 1, 1).int()
            im_crop[:,r_select, draw_box[0]:draw_box[0]+1] = color_tensor
            im_crop[:, r_select, draw_box[2]:draw_box[2]+1] = color_tensor
            im_crop[:, draw_box[1]:draw_box[1]+1, c_select] = color_tensor
            im_crop[:, draw_box[3]:draw_box[3]+1, c_select] = color_tensor

            r1 = predicted_box_tr[0] - max_corner_disp
            r1_pad = max(-r1, 0)
            r2 = predicted_box_tr[0] + max_corner_disp + 1
            r2_pad = max(0, r2 - im_crop.shape[1])

            c1 = predicted_box_tr[1] - max_corner_disp
            c1_pad = max(-c1, 0)
            c2 = predicted_box_tr[1] + max_corner_disp + 1
            c2_pad = max(0, c2 - im_crop.shape[2])
            im_region = im_crop[:, r1 + r1_pad:r2 - r2_pad, c1 + c1_pad:c2 - c2_pad]
            im_crop[:, r1 + r1_pad:r2 -r2_pad, c1 + c1_pad:c2 - c2_pad] = (im_region.float()*0.5 +
                                                                           0.5*corner_scores_mapped[:, r1_pad:corner_scores_mapped.shape[1] - r2_pad, c1_pad:corner_scores_mapped.shape[2] - c2_pad].float().cpu()).int()

            cv_image = im_crop.permute(1, 2, 0).numpy()
            cv_image = cv_image[:, :, [2, 1, 0]]
            # cvtColor(im, cv.COLOR_BGR2RGB)
            # cv_image = cv2.cvtColor(cv_image, cv2.COLOR_RGB2BGR)
            cv2.imwrite('/home/goutam/mlvid/nfs_running_100_m/{:04d}.png'.format(self.frame_num), cv_image)
            if self.visdom is not None:
                self.visdom.register(im_crop, 'image', 2, 'im current')

                # self.visdom.register(corner_scores_vis, 'heatmap', 2, 'TR Corner')

        if self.params.debug >= 4:
            im_crop = sample_patch(self.im, sample_pos, sample_scale*self.iou_img_sample_sz, self.iou_img_sample_sz)
            show_image_state(im_crop, init_box, init_boxes, None, 'Init', False)
            show_image_state(im_crop, predicted_box, output_boxes, output_iou, 'Final', False)
            ...
            disp_boxes = torch.cat((gt_optimized, predicted_box.view(1,4)), dim=0)
            disp_iou = torch.cat((gt_iou, predicted_iou), dim=0)
            show_image_state(im_crop, gt_box, disp_boxes, disp_iou, 'GT', False)
            cv2.waitKey(0)
    ...
    def optimize_boxes(self, iou_features, init_boxes):
        ...
        if isinstance(step_length, (tuple, list)):
            init_step_length = torch.Tensor([step_length[0], step_length[0], step_length[1], step_length[1]]).to(self.params.device).view(1,1,4)
        ...
        if box_refinement_space == 'default':
            for i_ in range(self.params.box_refinement_iter):
                ...
                # Update proposal
                step = update_mask_float * step_length * bb_init.grad * bb_init[:, :, 2:].repeat(1, 1, 2) - (1.0 - update_mask_float) * step
    ...
    def visualize_iou_pred(self, iou_features, center_box):
        ...
        pos_scores = self.iou_predictor.predict_iou(self.target_feat, iou_features, pos_boxes)
        sz_scores = self.iou_predictor.predict_iou(self.target_feat, iou_features, sz_boxes)
    ...
    def visualize_iou_pred_corner(self, iou_features, center_box):
        center_box = center_box.view(1,1,4)
        center_box_x1x2y1y2 = torch.cat((center_box[:, :, :2], center_box[:, :, :2] + center_box[:, :, 2:]), dim=2)

        pos_dist = 25
        pos_step = 1

        jitter_amount = torch.arange(-pos_dist, pos_dist+pos_step, step=pos_step)

        bbx2 = torch.zeros(1, jitter_amount.numel(), 4)
        bbx2[0,:,1] = jitter_amount.clone()
        bby1 = torch.zeros(jitter_amount.numel(), 1, 4)
        bby1[:, 0, 2] = jitter_amount.clone()

        tr_boxes_x1x2y1y1 = ((center_box_x1x2y1y2 + bbx2) + bby1).to(self.params.device)
        tr_boxes = torch.cat((tr_boxes_x1x2y1y1[:, :, :2], tr_boxes_x1x2y1y1[:, :, 2:] - tr_boxes_x1x2y1y1[:, :, :2]), dim=2)
        tr_boxes = tr_boxes.view(1,-1,4)
        tr_scores = self.iou_predictor.predict_iou(self.target_feat, iou_features, tr_boxes)
        tr_scores = tr_scores.exp()
        return tr_scores.view(jitter_amount.numel(), -1)
        # show_tensor(tr_scores.view(jitter_amount.numel(), -1), title='Top Right Scores', fig_num=21)
