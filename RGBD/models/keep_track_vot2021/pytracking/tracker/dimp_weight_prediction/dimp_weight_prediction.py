...
class DiMPWeightPrediction(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # Feature_sz
        # self.feature_sz = self.params.get('feature_sz', 18)
    ...
    def do_classifier_update(self, flag, feats):
        update = False
        max_peak = 1.0

        update_logic_is_on = self.params.get('classifier_update_logic_on', False)
        is_max_peak_avail = 'ProjTestScoreOnMemoryFramesFeature' in feats

        if is_max_peak_avail:
            max_peak = torch.max(feats['ProjTestScoreOnMemoryFramesFeature'][0, 0])
            self.max_peak.append(max_peak.item())

        if (self.frame_num - 1) % self.params.get('update_classifier_every_nth_frame', 1) == 0:
            update = True

            if flag == 'not_found':
                update = False

            if update_logic_is_on and is_max_peak_avail and (max_peak < self.params.get('max_peak_th', 0.2)):
                update = False

        return update

    def do_memory_update(self, flag, feats):
        max_peak = 1.0
        update = False

        update_logic_is_on = self.params.get('memory_update_logic_on', False)
        is_max_peak_avail = 'ProjTestScoreOnMemoryFramesFeature' in feats

        if is_max_peak_avail:
            max_peak = torch.max(feats['ProjTestScoreOnMemoryFramesFeature'][0, 0])

        if self.frame_num % self.params.get('update_memory_every_nth_frame', 1) == 0 or (flag == 'hard_negative'):
            update = True

            if flag == 'not_found' or flag == 'uncertain':
                update = False
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        self.msg = "{}: ".format(self.frame_num)
        ...
        scores_raw_pre = self.classify_target(test_x)

        scores_sz = scores_raw_pre.shape[-1]

        # ------- COMPUTE WEIGHTS ------- #
        train_x = self.training_samples[0][:self.num_stored_samples[0]]
        train_y = self.target_labels[0][:self.num_stored_samples[0]].reshape(-1, 1, scores_sz, scores_sz)
        train_bb = self.target_boxes[:self.num_stored_samples[0], :]
        train_certainty = self.target_certainties[:self.num_stored_samples[0]]
        sample_weights = self.sample_weights[0][:self.num_stored_samples[0]].reshape(-1, 1, 1, 1)

        size_of_non_permanent_memory = (self.params.sample_memory_size - self.num_init_samples[0])
        ind_hist = torch.FloatTensor(self.replace_ind_hist[-size_of_non_permanent_memory:]).long()
        # indices to retrieve frames from memory in sorted order [permanent | oldest:newest]
        indices = torch.cat([torch.arange(0, self.num_init_samples[0]), ind_hist])

        debug_feature_list = None
        update_logic_on = self.params.get('update_logic_on', False)

        if update_logic_on:
            debug_feature_list = ['ProjTestScoreOnMemoryFramesFeature']

        debug_feature_list = ['ProjTestScoreOnMemoryFramesFeature']

        weights, feats_raw_val_dict = self.get_predicted_weights(test_x, train_x, scores_raw_pre, train_y, train_bb,
                                                                 train_certainty, debug_feature_list)

        if self.params.get('scale_with_temporal_sample_weight', False):
            weights = sample_weights*weights

        if self.params.get('baseline', False):
            weights = sample_weights

        weights = torch.relu(train_certainty - 0.2) * sample_weights



        # update classifier using the newly computed patch weights for the memory
        flag = self.localize_target(scores_raw_pre, sample_pos, sample_scales)[3]

        # _, _, h, w = train_x.size()

        if self.do_classifier_update(flag, feats_raw_val_dict):
            self.update_classifier_new(weights)

            # Compute final classification scores

            # if self.params.get('transductive', False):
            scores_raw = self.classify_target(test_x)
            # else:
            #     scores_raw = scores_raw_pre.clone()
        else:
            self.num_iters.append(0)
            # No update so use the preliminary score as final score
            scores_raw = scores_raw_pre.clone()
        ...
        train_y = self.get_label_function(sample_pos[scale_ind, :], sample_scales[scale_ind]).to(self.params.device)
        ...
        if self.do_memory_update(flag, feats_raw_val_dict):
            target_certainty = None
            if 'ProjTestScoreOnMemoryFramesFeature' in feats_raw_val_dict:
                target_certainty = torch.max(feats_raw_val_dict['ProjTestScoreOnMemoryFramesFeature'][0, 0])

            self.update_memory(TensorList([train_x]), train_y, target_box, scores_raw, target_certainty, learning_rate)
        ...
        if self.params.get('use_gt_box', False):
            iou = bbutils.calc_iou(new_state, torch.from_numpy(self.frame_reader.get_bbox(self.frame_num - 1, None)))
            self.ious.append(iou)
            self.debug_info["IoU"] = iou
            self.debug_info["mIoU"] = sum(self.ious) / len(self.ious)



        if self.visdom is not None:
            feats_proj = feats_raw_val_dict['ProjTestScoreOnMemoryFramesFeature']
            all_mem_proj = feats_raw_val_dict['ProjWholeMemLabelsOnTestFrameFeature']
            gth_mem_proj = feats_raw_val_dict['ProjGthMemLabelsOnTestFrameFeature']

            self.visdom.register(scores_raw[0,0], 'heatmap', 2, 'Score Map' + self.id_str)
            self.visdom.register(scores_raw_pre[0,0], 'heatmap', 2, 'Score Map Preliminary' + self.id_str)
            self.visdom.register(self.debug_info, 'info_dict', 1, 'Status')
            self.visdom.register(train_y[0][0, 0].cpu(), 'heatmap', 2, 'Train Label Map' + self.id_str)
            self.visdom.register(feats_proj[0,0], 'heatmap', 2, 'ProjTestScoreOnMemoryFrames: GTH')
            self.visdom.register(feats_proj[indices[-1],0], 'heatmap', 2, 'ProjTestScoreOnMemoryFrames: Recent')
            self.visdom.register(all_mem_proj[0,0], 'heatmap', 2, 'ProjWholeMemLabelsOnTestFrame')
            self.visdom.register(gth_mem_proj[0,0], 'heatmap', 2, 'ProjGthMemLabelsOnTestFrame')

            self.visdom.register((torch.FloatTensor(self.num_iters), torch.arange(len(self.num_iters))), 'lineplot', 3, 'Num Iters')

            if not self.params.get('baseline', False):
                w_mean = torch.mean(weights.view(weights.shape[0], -1), dim=1)  # 50x1xHxW
                self.visdom.register((w_mean[indices], torch.arange(w_mean.numel())), 'lineplot',
                                     2, 'Weights-Mean-per-Sample')
                self.visdom.register(weights[0, 0], 'heatmap', 2, 'Weights: GTH' + self.id_str)
                self.visdom.register(weights[indices[-1], 0], 'heatmap', 2, 'Weights: Recent' + self.id_str)

            if 'BBoxDistanceMapFeature' in feats_raw_val_dict:
                dist_map = torch.sum(feats_raw_val_dict['BBoxDistanceMapFeature'] * torch.arange(0, 16, dtype=torch.float32,
                                                                                                                 device=train_x.device).view(
                    1, -1, 1, 1), dim=1).unsqueeze(1)
                self.visdom.register(dist_map[indices[-1], 0], 'heatmap', 2, 'BBoxDistanceMap: Recent')
                self.visdom.register(dist_map[0, 0], 'heatmap', 2, 'BBoxDistanceMap: First')

            certainty = self.target_certainties[:self.num_stored_samples[0]].view(-1)
            self.visdom.register((certainty[indices], torch.arange(certainty[indices].numel())), 'lineplot',
                                 2, 'MemoryLabelCertainty')

            weights = weights.view(-1)
            self.visdom.register((weights[indices], torch.arange(weights[indices].numel())), 'lineplot',
                                 2, 'Weights plot')

            if self.params.get('use_gt_box', False):
                self.visdom.register((torch.tensor(self.ious), torch.arange(len(self.ious))), 'lineplot', 3, 'IoU')

            if len(self.max_peak) > 0:
                self.visdom.register((torch.tensor(self.max_peak), torch.arange(len(self.max_peak))), 'lineplot', 3, 'max peak')
    ...
    # def get_predicted_weights(self, test_x, train_x, test_y, train_y, train_bb):
    #     with torch.no_grad():
    #         test_x = test_x.unsqueeze(1)
    #         train_x = train_x.unsqueeze(1)
    #
    #         feature_list = self.params.get('feature_list', None)
    #         debug_feature_list = []
    #         if self.params.debug > 0:
    #             debug_feature_list = ['proj_all_mem_on_test_label', 'proj_gth_mem_on_test_label']
    #
    #         sim = self.net.classifier.weight_predictor.cosine_similarity(train_x, test_x)
    #         feats_dict, debug_feat_dict = self.net.classifier.weight_predictor.prepare_features(sim, train_y, test_y,
    #                                                                                             train_x, test_x, train_bb,
    #                                                                                             feature_list, debug_feature_list)
    #
    #         w = self.net.classifier.weight_predictor.compute_weights(feats_dict, train_y)
    #         return w, feats_dict, debug_feat_dict

    def get_predicted_weights(self, test_x, train_x, test_y, train_y, train_bb, train_certainty, debug_feature_list=None):
        with torch.no_grad():
            test_x = test_x.unsqueeze(1)
            train_x = train_x.unsqueeze(1)

            if debug_feature_list is None:
                debug_feature_list = []

            if self.params.debug > 0:
                debug_feature_list = ['ProjWholeMemLabelsOnTestFrameFeature', 'ProjGthMemLabelsOnTestFrameFeature',
                                      'ProjTestScoreOnMemoryFramesFeature']

            feature_raw_val_dict = dict()
            feature_enc_val_dict = dict()
            feature_module_dict = self.net.classifier.weight_predictor.feature_module_dict

            sim = self.net.classifier.weight_predictor.cosine_similarity(train_x, test_x)

            for feature_name, feature in feature_module_dict.items():
                raw_val = feature.compute_feature(sim=sim, test_label=test_y, train_label=train_y, train_bb=train_bb,
                                                  train_certainty=train_certainty, train_feat=train_x, test_feat=test_x)
                enc_val = feature.encode(raw_val)

                feature_raw_val_dict[feature_name] = raw_val
                feature_enc_val_dict[feature_name] = enc_val

            debug_feature_module_dict = self.net.classifier.weight_predictor.build_features(debug_feature_list)

            for feature_name, feature in debug_feature_module_dict.items():
                if feature_name not in feature_raw_val_dict:
                    raw_val = feature.compute_feature(sim=sim, test_label=test_y, train_label=train_y,
                                                      train_bb=train_bb, train_certainty=train_certainty,
                                                      train_feat=train_x, test_feat=test_x)
                    feature_raw_val_dict[feature_name] = raw_val

            w = self.net.classifier.weight_predictor.compute_weights(feature_enc_val_dict, train_y)
            return w, feature_raw_val_dict
    ...
    def init_target_certainties(self, train_x: TensorList):
        num_train_samples = train_x[0].shape[0]
        self.target_certainties = train_x[0].new_zeros(self.params.sample_memory_size, 1, 1, 1)
        self.target_certainties[:num_train_samples] = 1.
    ...
    # def init_memory_new(self, train_x: TensorList):
    #     n, c, h, w = train_x[0].size()
    #     # Initialize first-frame spatial training samples
    #     self.num_init_samples = train_x.size(0)
    #     init_sample_weights = TensorList([x.new_ones(h, w) / x.shape[0] for x in train_x])
    #     print(init_sample_weights.size())
    #
    #     # Sample counters and weights for spatial
    #     self.num_stored_samples = self.num_init_samples.copy()
    #     self.previous_replace_ind = [None] * len(self.num_stored_samples)
    #     self.sample_weights = TensorList([x.new_zeros(self.params.sample_memory_size, h, w) for x in train_x])
    ...
    #         ts[:x.shape[0],...] = x
    #
    #     # if self.params.get('plot_sample_weights', None) is not None and self.params.debug > 0:
    #     #     self.visdom.register((self.sample_weights[0], torch.arange(self.sample_weights[0].numel())), 'lineplot', 3, 'Sample Weights')
    ...
    def update_memory(self, sample_x: TensorList, sample_y: TensorList, target_box, scores, certainty=None, learning_rate=None):
        ...
        self.replace_ind_hist.append(self.previous_replace_ind[0])
        ...
        # Update certainties memory
        if certainty is not None:
            certainty = certainty.reshape(scores.shape[0], scores.shape[1], 1, 1)
            self.target_certainties[replace_ind[0]] = certainty
        ...
        if self.params.get('plot_sample_weights', None) is not None and self.params.debug > 0:
            ...
            self.visdom.register((self.sample_weights[0][indices], torch.arange(0, indices.size()[0])), 'lineplot', 3, 'Sample Weights')
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        # Get target certainties
        self.init_target_certainties(TensorList([x]))
    ...
    def update_classifier_new(self, patch_weights, hard_negative_flag=False):
        plot_loss = self.params.debug > 0

        num_iter = self.params.get('net_opt_num_iter', 1)
        if hard_negative_flag:
            num_iter += self.params.get('net_opt_hn_iter', None)

        self.num_iters.append(num_iter)
    ...
    def visdom_draw_tracking(self, image, box, segmentation=None):
        if hasattr(self, 'search_area_box'):
            if self.params.get('use_gt_box', False):
                self.visdom.register((image, box, self.search_area_box, torch.from_numpy(
                    self.frame_reader.get_bbox(self.frame_num - 1, None))), 'Tracking', 1, 'Tracking')
            else:
