...
class ETCOM(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        if self.params.get('score_upsample_factor', None) is None:
        ...
        self.params.score_fusion_strategy = self.params.get('score_fusion_strategy', 'default')
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        test_x, sample_coords = self.extract_sample(im, self.pos, self.target_scale * self.params.scale_factors, self.img_sample_sz)
        ...
        if flag != 'not_found':
            if self.use_iou_net:
                ...
                self.refine_target_box(sample_pos[scale_ind,:], sample_scales[scale_ind], scale_ind, update_scale_flag)
        ...
        if self.params.get('update_classifier', False) and update_flag:
            # Get train sample
    ...
    def localize_target(self, scores_raw, sample_scales):
    ...
    def localize_advanced(self, scores, sample_scales):
        ...
        if self.params.get('use_hn_fix', False):
            target_neigh_sz = self.params.target_neighborhood_scale * (self.target_sz / sample_scale) * (self.output_sz / self.img_support_sz)
        else:
        ...
        translation_vec2 = target_disp2 * (self.img_support_sz / self.output_sz) * sample_scale
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        if self.params.features.patch_mode == 'inside':
            ...
            # self.init_sample_pos = (self.pos + shift).round()
        else:
        ...
        if 'shift' in self.params.augmentation:
            self.transforms.extend([augmentation.Translation(shift, aug_output_sz, global_shift.long().tolist()) for shift in self.params.augmentation['shift']])
        if 'relativeshift' in self.params.augmentation:
            get_absolute = lambda shift: (torch.Tensor(shift) * self.img_sample_sz/2).long().tolist()
            self.transforms.extend([augmentation.Translation(get_absolute(shift), aug_output_sz, global_shift.long().tolist()) for shift in self.params.augmentation['relativeshift']])
        ...
        init_samples = self.params.features.extract_transformed(im, self.init_sample_pos, self.init_sample_scale, aug_expansion_sz, self.transforms)
    ...
    def init_iou_net(self):
        ...
        # Extract target feat
        with torch.no_grad():
            target_feat = self.iou_predictor.get_modulation(iou_res_features, target_boxes)
    ...
    def init_classifier(self, x):
        ...
        if 'Simple' in self.classifier_net_name:
            ...
            # if self.params.get('update_strategy', 'default') == 'convex':
            #     plot_loss = False
            #     num_iter = 0

            with torch.no_grad():
                if self.classifier_net_name == 'LinearFilterOld':
                    self.target_filter, _, losses = self.target_classifier.get_filter(x[0], target_boxes,
                                                                                      label=y[0], num_iter=num_iter,
                                                                                      compute_losses=plot_loss)
                else:
                    self.target_filter, _, losses = self.target_classifier.get_filter(x[0], target_boxes, num_iter=num_iter)
        ...
        elif 'Dlearn' in self.classifier_net_name:
            # Generate label function
            y = self.init_label_function(x)

            with torch.no_grad():
                self.dlearn_coeff_num, self.dlearn_coeff_den, losses = self.target_classifier.get_filter_coeff(x[0], target_boxes, y[0])
                self.target_filter = self.target_classifier.reconstruct_filter(self.dlearn_coeff_num, self.dlearn_coeff_den)

        elif 'Clearn' in self.classifier_net_name:
            # Generate label function
            y = self.init_label_function(x)

            with torch.no_grad():
                self.clearn_coeff, losses = self.target_classifier.get_filter_coeff(x[0], target_boxes, y[0])
                self.target_filter = self.target_classifier.reconstruct_filter(self.clearn_coeff)
    ...
    def update_classifier(self, train_x, train_y, target_box, learning_rate=None, scores=None):
        ...
        if 'Simple' in self.classifier_net_name:
        ...
        elif self.classifier_net_name.startswith('LinearFilter') and self.params.get('update_strategy', 'default') == 'default':
            ...
            if num_iter != 0:
                ...
                with torch.no_grad():
                    if self.classifier_net_name == 'LinearFilterOld':
                        self.target_filter, _, losses = self.target_classifier.filter_optimizer(self.target_filter,
                                                                                             samples,
                                                                                             bb=target_boxes,
                                                                                             label=labels,
                                                                                             num_iter=num_iter,
                        ...
                        target_weights, _, _ = self.target_classifier.filter_optimizer(TensorList([self.target_filter]),
                                                                                             num_iter=num_iter,
                                                                                             feat=samples,
                                                                                             bb=target_boxes,
                                                                                             sample_weight=sample_weights)

                        self.target_filter = target_weights[0] if isinstance(target_weights, TensorList) else target_weights

                if plot_loss:
                    self.losses = torch.cat((self.losses, torch.stack(losses['train'])))
                    plot_graph(self.losses, 10, title='Training loss')

        elif self.classifier_net_name.startswith('LinearFilter') and self.params.get('update_strategy', 'default') == 'convex':
            ...
            if not hasattr(self, 'init_dlearn_coeff_num'):
                self.init_dlearn_coeff_num = self.dlearn_coeff_num
                self.init_dlearn_coeff_den = self.dlearn_coeff_den
            with torch.no_grad():
                coeff_num, coeff_den, losses = self.target_classifier.get_filter_coeff(train_x[0], target_box, train_y[0])
                self.dlearn_coeff_num = init_samples_weight * self.init_dlearn_coeff_num + (1 - init_samples_weight) * \
                                        ((1 - learning_rate) * self.dlearn_coeff_num + learning_rate * coeff_num)
                self.dlearn_coeff_den = init_samples_weight * self.init_dlearn_coeff_den + (1 - init_samples_weight) * \
                                        ((1 - learning_rate) * self.dlearn_coeff_den + learning_rate * coeff_den)
                self.target_filter = self.target_classifier.reconstruct_filter(self.dlearn_coeff_num, self.dlearn_coeff_den)

        elif 'Clearn' in self.classifier_net_name:
            if not hasattr(self, 'init_clearn_coeff'):
                self.init_clearn_coeff = self.clearn_coeff
            with torch.no_grad():
                coeff, losses = self.target_classifier.get_filter_coeff(train_x[0], target_box, train_y[0])
                self.clearn_coeff = init_samples_weight * self.init_clearn_coeff + (1 - init_samples_weight) * \
                                        ((1 - learning_rate) * self.clearn_coeff + learning_rate * coeff)
                self.target_filter = self.target_classifier.reconstruct_filter(self.clearn_coeff)
    ...
    def refine_target_box(self, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        if self.params.num_init_random_boxes > 0:
            ...
            if self.params.has('iounet_use_small_proposals') and self.params.iounet_use_small_proposals:
                if self.params.has('iounet_use_iterative_shrinking') and self.params.iounet_use_iterative_shrinking:
        ...
        if not self.params.get('iounet_update_aspect_ratio', True):
        ...
        if self.params.debug >= 4:
            im_crop, _ = sample_patch(self.im, sample_pos, sample_scale*self.iou_img_sample_sz, self.iou_img_sample_sz, mode=self.params.features.patch_mode)
