...
def cosine_similarity(test, train):
    if test.dim() == 5 and train.dim():
        nframes, _, c, w, h = train.size()
        train = train.view(nframes, c, w, h)
        test = test.view(1, c, w, h)
    else:
        nframes, c, w, h = train.size()

    train_reshuffle = train.permute(1, 0, 2, 3)  # (nframes, C, W, H) -> (C, nframes, W, H)
    test_reshuffle = test.permute(1, 0, 2, 3)  # (nframes, C, W, H) -> (C, nframes, W, H)
    train_reshuffle = train_reshuffle.reshape(c, nframes * w * h)  # merge dimensions into one patch dimension
    test_reshuffle = test_reshuffle.reshape(c, w * h)  # merge dimensions into one patch dimension

    train_norm = torch.sqrt(torch.einsum('ij,ij->j', train_reshuffle, train_reshuffle)).view(1, nframes * w * h)
    test_norm = torch.sqrt(torch.einsum('ij,ij->j', test_reshuffle, test_reshuffle)).view(1, w * h)

    train_normalized = train_reshuffle / train_norm
    test_normalized = test_reshuffle / test_norm

    return torch.einsum('ij,ik->jk', test_normalized, train_normalized)  # (w*h, nframes*w*h)

class DiMPPatchSimilarityLearning(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        feats_sz = test_x.shape[-1]

        if scores_sz != feats_sz:
            scores_raw_pre_rescaled = F.interpolate(scores_raw_pre, size=(feats_sz, feats_sz), mode="bilinear")
        else:
            scores_raw_pre_rescaled = scores_raw_pre

        # ------- COMPUTE WEIGHTS ------- #

        # Cosine similarity between test frame and all frames in the memory.
        sims = cosine_similarity(test_x, self.training_samples[0][:self.num_stored_samples[0]])
        ...
        # id of most similar patch from memory for the test frame (18 x 18)
        idx_mem_to_test = torch.max(sims, dim=1)[1]
        # id of most similar patch from test frame for each patch in the memory (memsize x 18 x 18)
        idx_test_to_mem = torch.max(sims, dim=0)[1]

        # Compute patch weights for each patch in memory using the test frame
        scores_projected_on_mem_frames_tmp = scores_raw_pre_rescaled[0,0].reshape(-1)[idx_test_to_mem].reshape(
            -1, 1, feats_sz, feats_sz)

        if scores_sz != feats_sz:
            scores_projected_on_mem_frames = F.interpolate(scores_projected_on_mem_frames_tmp,
                                                    size=(scores_sz, scores_sz), mode="bilinear")
        else:
            scores_projected_on_mem_frames = scores_projected_on_mem_frames_tmp

        clipped_scores = torch.clamp(scores_projected_on_mem_frames, min=0.)

        train_y = self.target_labels[0][:self.num_stored_samples[0]].reshape(-1, 1, scores_sz, scores_sz)

        diff_weights = 1. + torch.abs(train_y - clipped_scores)
        sample_weights = self.sample_weights[0][:self.num_stored_samples[0]].reshape(-1, 1, 1, 1)

        scaled_diff_weights = sample_weights*diff_weights
        ...
        if self.params.get('baseline', False):
            self.update_classifier_new(sample_weights, (flag == 'hard_negative'))
        else:
            self.update_classifier_new(scaled_diff_weights, (flag == 'hard_negative'))
        ...
        if self.visdom is not None:
            ...
            self.visdom.register(scores_raw[0,0] - scores_raw_pre[0,0], 'heatmap', 2, 'Score Map Preliminary' + self.id_str)
            self.visdom.register(scores_projected_on_mem_frames[indices[-1],0], 'heatmap', 2, 'Scores projected on recent mem frame' + self.id_str)
            self.visdom.register(clipped_scores[indices[-1],0], 'heatmap', 2, 'Clipped Scores' + self.id_str)
            self.visdom.register(diff_weights[indices[-1],0], 'heatmap', 2, 'Diff Weights' + self.id_str)
    ...
    # def track_old(self, image, info: dict = None) -> dict:
    #     self.debug_info = {}
    #
    #     self.frame_num += 1
    #     self.debug_info['frame_num'] = self.frame_num
    ...
    #     sims = cosine_similarity(test_x, self.training_samples[0][:self.num_stored_samples[0]])
    #
    #     self.visdom.register(torch.max(sims, dim=1)[0].view(18, 18).cpu(), 'heatmap', 2,
    #                          'Highest Mem Similarities Val' + self.id_str)
    #
    #     size_of_non_permanent_memory = (self.params.sample_memory_size - self.num_init_samples[0])
    #     ind_hist = torch.FloatTensor(self.replace_ind_hist[-size_of_non_permanent_memory:]).long()
    #     # indices stores the location of frame in memory, the value at index i shows where frame i is stored in memory.
    #     indices = torch.cat([torch.arange(0, self.num_init_samples[0]), ind_hist])
    #
    #     # lut_indices allows to look up the frame number for a frame in memory.
    #     lut_indices = torch.argsort(indices)
    #
    #     self.visdom.register(lut_indices[torch.max(sims, dim=1)[1]//(18*18)].view(18, 18).cpu(), 'heatmap', 2,
    #                          'Highest Mem Similarities Frame ID' + self.id_str)
    #
    #     _, idx = torch.max(sims, dim=1)
    #     print(torch.max(self.target_labels[0].reshape(-1)[idx].reshape(18,18)))
    #     self.visdom.register(self.target_labels[0].reshape(-1)[idx].reshape(18,18).cpu(), 'heatmap', 2,
    #                          'Mem Label from Sim' + self.id_str)
    #
    #
    #     # Location of sample
    #     sample_pos, sample_scales = self.get_sample_location(sample_coords)
    ...
    #     new_state = torch.cat((self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]]))
    #
    #     if hasattr(self, 'gt_state'):
    #         iou = bbutils.calc_iou(new_state, torch.from_numpy(self.gt_state))
    #         self.ious.append(iou)
    #         self.debug_info["IoU"] = iou
    #         self.debug_info["mIoU"] = sum(self.ious)/len(self.ious)
    #
    ...
    #         score_projected_on_mem_frames = score_map.reshape(-1)[idx].reshape(-1,18,18)
    #
    #         self.visdom.register(torch.clamp(score_projected_on_mem_frames[indices[-1]], min=0.),
    #                              'heatmap', 2, 'score projected on mem frames recent' + self.id_str)
    #         self.visdom.register(torch.clamp(score_projected_on_mem_frames[0], min=0.),
    #                              'heatmap', 2, 'score projected on mem frames first' + self.id_str)
    #
    #
    #         self.visdom.register(torch.max(sims, dim=0)[0].view(-1, 18, 18)[indices[-1]].cpu(), 'heatmap', 2,
    #                              'Similarities of test frame to most recent' + self.id_str)
    #
    #
    ...
    def update_classifier_new(self, patch_weights, hard_negative_flag):
        plot_loss = self.params.debug > 0
