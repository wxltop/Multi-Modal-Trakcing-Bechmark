...
class DiMPVosBox(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        init_mask = None#info.get('init_mask', None)
        #if self.params.has('use_seg'):
        #    if not self.params.use_seg:
        #        init_mask = None
        ...
        if init_mask is not None:
            ...
            if hasattr(self.net, 'box_label_encoder'):
                if self.params.get('generate_init_mask', True):
                    with torch.no_grad():
                        init_backbone_feat = self.net.extract_backbone(im)
                        init_feat_clf = self.net.extract_target_model_features(init_backbone_feat)
                        if self.params.get('generate_init_mask_with_label_encoder', False):
                            init_mask = self.bbox_to_mask(torch.tensor(state).unsqueeze(dim=0).to(im.device),
                                                          sz=im.shape[-2:])
                            init_mask_enc = self.net.label_encoder(init_mask.to(init_feat_clf.device))
                        else:
                            init_mask_enc = self.net.box_label_encoder(torch.tensor(state).unsqueeze(dim=0).to(init_feat_clf.device), init_feat_clf, im.shape[-2:])
                        if isinstance(init_mask_enc, (list,tuple)):
                            init_mask_enc = init_mask_enc[0]

                        init_mask, not_used = self.net.decoder(init_mask_enc, init_backbone_feat, im.shape[-2:])
                        init_mask_raw = init_mask.clone()

                        init_mask = torch.sigmoid(init_mask)
                        for mask_iter in range(self.params.get("mask_init_iter", 0)):
                            init_mask_enc = self.net.label_encoder(init_mask.to(init_feat_clf.device))
                            if isinstance(init_mask_enc, (list, tuple)):
                                init_mask_enc = init_mask_enc[0]
                            init_mask, not_used = self.net.decoder(torch.sigmoid(init_mask_enc), init_backbone_feat, im.shape[-2:])

                else:
                    init_mask = self.bbox_to_mask(torch.tensor(state).unsqueeze(dim=0).to(im.device), sz=im.shape[-2:])
            else:
        ...
        self.init_classifier(init_backbone_feat, init_masks, bbox=state, im_sz=im.shape)

        self.prev_flag = 'init'
        self.prev_test_x = None

        out = {'time': time.time() - tic, 'init_mask': init_mask.squeeze().cpu(),
               'segmentation_raw': init_mask_raw.squeeze().cpu().numpy(),
               'segmentation': init_mask.squeeze().cpu().numpy()}
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      ...
                                                                      self.img_sample_sz)
        self.backbone_clf_feat = self.net.get_backbone_target_model_features(backbone_feat)
        ...
        #     segmentation_out = seg_mask_im_raw_np
        # else:
        #     segmentation_out = seg_mask_im_np
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            scores = self.net.target_model.classify(self.target_filter, sample_x)
        return scores
    ...
    def segment_target(self, sample_clf_feat, sample_x):
        """Classify target by applying the DiMP filter."""
        with torch.no_grad():
            mask, box = self.net.segment_target(self.target_filter, sample_clf_feat, sample_x)
        return mask, box
    ...
    def localize_target(self, scores, sample_pos, sample_scales):
        ...
        if preprocess_method == 'none':
            ...
            reg_val = getattr(self.net.target_model.filter_optimizer, 'softmax_reg', None)
    ...
    def generate_init_samples(self, im: torch.Tensor, init_mask) -> TensorList:
        ...
        init_masks = sample_patch_transformed(init_mask,
                                              self.init_sample_pos, self.init_sample_scale, aug_expansion_sz,
                                              self.transforms,
                                              is_mask=True)
    ...
    def init_memory(self, train_x: TensorList, masks, mask_weights=None):
        assert masks.dim() == 4
        ...
        if not mask_weights is None:
            self.target_mask_weights = masks.new_zeros(self.params.sample_memory_size, mask_weights.shape[-3], masks.shape[-2],
                                                mask_weights.shape[-1])
            self.target_mask_weights[:mask_weights.shape[0], :, :, :] = mask_weights
        else:
            self.target_mask_weights = None

        for ts, x in zip(self.training_samples, train_x):
            ts[:x.shape[0],...] = x


        self.fram_ids = [-1 for fid in range(self.params.sample_memory_size)]
        for fid in range(self.num_init_samples[0]):
            self.fram_ids[fid] = fid

    def update_memory(self, sample_x: TensorList, mask, mask_weights=None, learning_rate = None):
        ...
        if not mask_weights is None:
            assert not self.target_mask_weights is None
            self.target_mask_weights[replace_ind[0], :, :, :] = mask_weights[0, ...]

        self.num_stored_samples += 1

        self.fram_ids[replace_ind[0]] = self.frame_num - 1
    ...
    def init_classifier(self, init_backbone_feat, init_masks, bbox=None, im_sz=None):
        ...
        use_seg = self.params.get('use_seg', True)
        if use_seg or self.params.get('generate_init_mask', True):
            if self.net.label_encoder is not None:
                if self.params.get('label_encoder_backbone_feat', False):
                    enc_feat = self.net.get_backbone_target_model_features(init_backbone_feat)
                else:
                    enc_feat = x
                mask_enc = self.net.label_encoder(init_masks, enc_feat.unsqueeze(1))
            else:
                mask_enc = init_masks
        else:
            if self.net.box_label_encoder is not None:
                if self.params.get('label_encoder_backbone_feat', False):
                    enc_feat = self.net.get_backbone_target_model_features(init_backbone_feat)
                else:
                    enc_feat = x
                mask_enc = self.net.box_label_encoder(torch.tensor(bbox).unsqueeze(dim=0).to(enc_feat.device),
                                                      enc_feat.unsqueeze(1), im_sz)
            else:
                mask_enc = init_masks
        # Get target filter by running the discriminative model prediction module
        with torch.no_grad():
            self.target_filter, _, losses = self.net.target_model.get_filter(x.unsqueeze(1), *mask_enc,
                                                                           num_iter=num_iter)
        ...
        if self.params.get('update_classifier', True):
            if self.params.get('store_enc', False):
                if isinstance(mask_enc, (tuple, list)):
                    self.init_memory(TensorList([x]), masks=mask_enc[0].view(-1, *mask_enc[0].shape[-3:]),
                                     mask_weights=mask_enc[1].view(-1, *mask_enc[1].shape[-3:]))
                else:
    ...
    def update_classifier(self, train_x, mask, mask_weights=None, learning_rate=None):
        ...
        if hard_negative_flag or self.frame_num % self.params.get('train_sample_interval', 1) == 0:
            if not mask_weights is None:
                self.update_memory(TensorList([train_x]), mask, mask_weights=mask_weights, learning_rate=learning_rate)
            else:
                self.update_memory(TensorList([train_x]), mask, learning_rate=learning_rate)
        ...
        if num_iter > 0:
            if self.params.get('felix_sample_scheme', False):
                ids_list = self.frame_sceduler()
            else:
                ids_list = [i for i in range(min(self.num_stored_samples[0],self.training_samples[0].shape[0]))]
            # Get inputs for the DiMP filter optimizer module
            samples = self.training_samples[0][ids_list,...]
            masks = self.target_masks[ids_list, ...]

            if self.params.get('store_enc', False):
                if not self.target_mask_weights is None:
                    mask_weights = self.target_mask_weights[ids_list, ...]
                    mask_enc_info = (masks, mask_weights.unsqueeze(1))
                else:
                    mask_enc_info = masks
            else:
                with torch.no_grad():
            ...
            if self.params.get('uniform_weights', False):
                num_samp = min(self.num_stored_samples[0], self.params.sample_memory_size)
                sample_weights = torch.ones(num_samp,).to(train_x.device)
                sample_weights = sample_weights[ids_list]
            else:
                sample_weights = self.sample_weights[0][ids_list]
            ...
            if self.frame_num > self.params.get("start_updating_frame_num", 0):
    ...
    def frame_sceduler(self):
        num_train_frames = self.params.get('sample_scheme_num_samples', 3)
        latest_id = self.fram_ids[self.previous_replace_ind[0]]
        if self.num_stored_samples[0] > num_train_frames - 1:
            min_id = min([p for p in self.fram_ids if p > -1])
            min_idp = [i for i, p in enumerate(self.fram_ids) if p == min_id and p > -1]
            ids_list = [min_idp[0]]
            diff = latest_id - min_id

            cnt=1
            while cnt*diff//(num_train_frames - 1) < latest_id:
                den = (num_train_frames - 1)
                mid_id =cnt*diff // den

                mid_idp = [i for i, p in enumerate(self.fram_ids) if p == mid_id and p > -1]
                mid_idp2 = [i for i, p in enumerate(self.fram_ids) if p == mid_id + 1 and p > -1]
                mid_idp3 = [i for i, p in enumerate(self.fram_ids) if p == mid_id - 1 and p > -1]
                if len(mid_idp) > 0:
                    ids_list.append(mid_idp[0])
                elif len(mid_idp2)>0:
                    ids_list.append(mid_idp2[0])
                elif len(mid_idp3)>0:
                    ids_list.append(mid_idp3[0])
                cnt+=1

            latest_id_pos = [i for i, p in enumerate(self.fram_ids) if p == latest_id]
            ids_list.append(latest_id_pos[0])

            # for i, fid in enumerate(self.fram_ids):
            #    if fid < curent_id - 5 and i % 5 == 0 and fid > 0:
            #        ids_list.append(i)
            #        curent_id = fid
        else:
            ids_list = [i for i in range(self.num_stored_samples[0])]
