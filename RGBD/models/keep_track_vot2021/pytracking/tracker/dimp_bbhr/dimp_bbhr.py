...
class DiMPbbhr(BaseTracker):

    multiobj_mode = 'parallel'

    def initialize_features(self):
        if not getattr(self, 'features_initialized', False):
            self.params.net_cls.initialize()
            self.params.net_bbr.initialize()
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # The DiMP network
        self.net_cls = self.params.net_cls
        self.net_bbr = self.params.net_bbr
...
# Get target position and size
        ...
        sz_bbr = self.params.image_sample_size_bbr
        self.img_sample_sz_bbr = torch.Tensor([sz_bbr, sz_bbr] if isinstance(sz_bbr, int) else sz_bbr)

        # Set search area
        search_area = torch.prod(self.target_sz * self.params.search_area_scale).item()
        self.target_scale = math.sqrt(search_area) / self.img_sample_sz.prod().sqrt()

        search_area_bbr = torch.prod(self.target_sz * self.params.search_area_scale_bbr).item()
        target_scale_bbr = math.sqrt(search_area_bbr) / self.img_sample_sz_bbr.prod().sqrt()

        self.bbr_target_scale_factor = target_scale_bbr / self.target_scale
        ...
        # Initialize IoUNet
        backbone_feat_bbr, sample_coords_bbr, im_patches_bbr = self.extract_backbone_features_bbr(im,
                                                                                                  self.get_centered_sample_pos(),
                                                                                                  self.target_scale * self.bbr_target_scale_factor * self.params.scale_factors,
                                                                                                  self.img_sample_sz_bbr)

        # Location of sample
        sample_pos_bbr, sample_scales_bbr = self.get_sample_location(sample_coords_bbr)
        scale_ind_bbr = 0
        self.init_iou_net(backbone_feat_bbr, sample_pos_bbr[scale_ind_bbr, :], sample_scales_bbr[scale_ind_bbr])
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if not self.params.get('use_gt_box_for_cls', False):
            ...
            new_pos = torch.Tensor([gt_state[1] + (gt_state[3] - 1) / 2, gt_state[0] + (gt_state[2] - 1) / 2])
            flag = 'GT'
        ...
        if flag != 'not_found':
            ...
            if self.params.get('use_gt_box_for_bbinit', False):
                gt_state = self.gt_state
            ...
            # Extract hr patch for bbreg
            backbone_feat_bbr, sample_coords_bbr, im_patches_bbr = self.extract_backbone_features_bbr(im,
                                                                                                      self.get_centered_sample_pos(),
                                                                                                      self.target_scale * self.bbr_target_scale_factor * self.params.scale_factors,
                                                                                                      self.img_sample_sz_bbr)

            test_patch_bbr = im_patches_bbr[0].int()
            self.test_patch_bbr = test_patch_bbr

            # Location of sample
            sample_pos_bbr, sample_scales_bbr = self.get_sample_location(sample_coords_bbr)

            scale_ind_bbr = 0
            if self.params.get('use_bbr', False):
                self.refine_target_box_bbr(backbone_feat_bbr, sample_pos_bbr[scale_ind_bbr, :],
                                           sample_scales_bbr[scale_ind_bbr], scale_ind_bbr, True)
            else:
                self.refine_target_box(backbone_feat_bbr, sample_pos_bbr[scale_ind_bbr,:], sample_scales_bbr[scale_ind_bbr], scale_ind_bbr, True)

        # ------- UPDATE ------- #

        update_flag = flag not in ['not_found', 'uncertain', 'GT']
        ...
        if self.visdom is not None:
            if score_map is not None:
                ...
                test_patch_cls = im_patches[0].int()
                self.visdom.register(test_patch_cls, 'image', 2, 'im patch cls')

            self.visdom.register(self.debug_info, 'info_dict', 1, 'Status')

            if flag != 'not_found':
                self.visdom.register(test_patch_bbr, 'image', 2, 'im patch bbr')
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            scores = self.net_cls.classifier.classify(self.target_filter, sample_x)
        return scores
    ...
    def localize_target(self, scores, sample_pos, sample_scales):
        ...
        if preprocess_method == 'none':
            ...
            reg_val = getattr(self.net_cls.classifier.filter_optimizer, 'softmax_reg', None)
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        with torch.no_grad():
            backbone_feat = self.net_cls.extract_backbone(im_patches)
        return backbone_feat, patch_coords, im_patches

    def extract_backbone_features_bbr(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        im_patches, patch_coords = sample_patch_multiscale(im, pos, scales, sz,
                                                           mode=self.params.get('border_mode_bbr', 'replicate'),
                                                           max_scale_change=self.params.get('patch_max_scale_change_bbr', None))

        # im_patches, patch_coords = sample_patch_multiscale(im, pos, scales, sz,
        #                                                    mode=self.params.get('border_mode', 'replicate'),
        #                                                    max_scale_change=getattr(self.params,
        #                                                                             'patch_max_scale_change', None))
        with torch.no_grad():
            backbone_feat = self.net_bbr.extract_backbone(im_patches)
        return backbone_feat, patch_coords, im_patches

    def get_classification_features(self, backbone_feat):
        with torch.no_grad():
            return self.net_cls.extract_classification_feat(backbone_feat)

    def get_iou_backbone_features(self, backbone_feat):
        return self.net_bbr.get_backbone_bbreg_feat(backbone_feat)

    def get_iou_features(self, backbone_feat):
        with torch.no_grad():
            return self.bb_regressor.get_iou_feat(self.get_iou_backbone_features(backbone_feat))

    def get_iou_modulation(self, iou_backbone_feat, target_boxes):
        with torch.no_grad():
            return self.bb_regressor.get_modulation(iou_backbone_feat, target_boxes)
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        with torch.no_grad():
            init_backbone_feat = self.net_cls.extract_backbone(im_patches)
    ...
    def init_iou_net(self, backbone_feat, init_sample_pos, init_sample_scale):
        if self.params.get('cascade_level', None) is not None:
            self.bb_regressor = getattr(self.net_bbr, 'bb_regressor_l{}'.format(self.params.cascade_level))
        else:
            self.bb_regressor = self.net_bbr.bb_regressor
        ...
        self.classifier_target_box = self.get_iounet_box(self.pos, self.target_sz, init_sample_pos, init_sample_scale)
        target_boxes = TensorList()
        if self.params.iounet_augmentation:
            raise NotImplementedError
        else:
            target_boxes.append(self.classifier_target_box)
        ...
        if isinstance(self.iou_modulation[0], (list, tuple)):
            self.iou_modulation = TensorList([[x.detach().mean(0) for x in x_t] for x_t in self.iou_modulation])
        else:
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        if hasattr(self.net_cls, 'classifier'):
            pred_module = getattr(self.net_cls.classifier.filter_optimizer, 'score_predictor', self.net_cls.classifier.filter_optimizer)
        elif hasattr(self.net_cls, 'dimp_classifier'):
            self.net_cls.classifier = self.net_cls.dimp_classifier
            pred_module = getattr(self.net_cls.dimp_classifier.filter_optimizer, 'score_predictor',
                                  self.net_cls.dimp_classifier.filter_optimizer)
        else:
            raise NotImplementedError

        if self.params.get('label_threshold', None) is not None:
            self.net_cls.classifier.filter_optimizer.label_threshold = self.params.label_threshold
        if self.params.get('label_shrink', None) is not None:
            self.net_cls.classifier.filter_optimizer.label_shrink = self.params.label_shrink
        if self.params.get('softmax_reg', None) is not None:
            self.net_cls.classifier.filter_optimizer.softmax_reg = self.params.softmax_reg
        ...
        if self.params.get('filter_init_zero', False):
            self.net_cls.classifier.filter_initializer = FilterInitializerZero(self.net_cls.classifier.filter_size, x.shape[-3])
        ...
        ksz = self.net_cls.classifier.filter_size
        ...
        with torch.no_grad():
            self.target_filter, _, losses = self.net_cls.classifier.get_filter(x, target_boxes, num_iter=num_iter,
    ...
    def update_classifier(self, train_x, target_box, learning_rate=None, scores=None):
        ...
        if num_iter > 0:
            ...
            with torch.no_grad():
                self.target_filter, _, losses = self.net_cls.classifier.filter_optimizer(self.target_filter,
    ...
    def refine_target_box_bbr(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        if self.params.get('bbr_add_noise', False):
            ...
            rand_bb = (torch.rand(1, 4) - 0.5) * rand_factor
            new_sz = (init_box[2:] + rand_bb[:, 2:]).clamp(minimal_edge_size)
            new_center = (init_box[:2] + init_box[2:] / 2) + rand_bb[:, :2]
            init_box = torch.cat([new_center - new_sz / 2, new_sz], 1)

        if self.params.get('bbr_output_proposal', False):
            ...
            # Optimize the boxes
            output_boxes = self.bb_regressor.predict_bb(self.iou_modulation, iou_features, init_boxes).view(-1,4).cpu()
        ...
        new_pos = (new_pos.flip((0,)) - (self.img_sample_sz_bbr - 1) / 2) * sample_scale + sample_pos
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        if self.visdom is not None and self.params.debug > 1:
            ...
            self.visdom.register((self.test_patch_bbr, init_boxes, final_boxes, final_iou), 'bbreg', 3, 'Boxes')
    ...
    def optimize_boxes_default(self, iou_features, init_boxes):
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            outputs = self.bb_regressor.predict_iou(self.iou_modulation, iou_features, bb_init)
            ...
            # TODO test SL decay, early stopping
    ...
    def optimize_boxes_relative(self, iou_features, init_boxes):
        ...
        if isinstance(step_length, (tuple, list)):
            ...
            step_length = torch.Tensor([step_length]).to(self.params.device).view(1,1,1)

        step_length = step_length.repeat(1, output_boxes.shape[1], 1)
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            # Check if score descreased
            if self.params.get('box_refinement_early_stop', False):
                with torch.no_grad():
                    bb_refined = bbutils.rel_to_rect(output_boxes_rel, sz_norm)
                    outputs_refined = self.bb_regressor.predict_iou(self.iou_modulation, iou_features, bb_refined)

                    refinement_failed = (outputs_refined < outputs).view(-1)

                    output_boxes_rel[0, refinement_failed, :] = bb_init_rel[0, refinement_failed, :]
                    step_length[0, refinement_failed, :] *= self.params.box_refinement_step_decay
            else:
                step_length *= self.params.box_refinement_step_decay
    ...
    def visualize_iou_pred(self, iou_features, center_box):
        ...
        pos_scores = self.bb_regressor.predict_iou(self.iou_modulation, iou_features, pos_boxes).exp()
        sz_scores = self.bb_regressor.predict_iou(self.iou_modulation, iou_features, sz_boxes).exp()
