...
class UIControl:
    ...
    def on_motion(self, event):
        ...
        if event.xdata is None or event.ydata is None:
            return
        self.target_br = (event.xdata, event.ydata)
        self.rect.set_width(self.target_br[0] - self.target_tl[0])
        self.rect.set_height(self.target_br[1] - self.target_tl[1])

        self.fig.canvas.draw()

    def on_release(self, event):
        if self.mode == 'init':
            self.target_tl = (event.xdata, event.ydata)
            self.rect = patches.Rectangle((self.target_tl[0], self.target_tl[1]),
                                          1, 1, linewidth=1, edgecolor='r',
                                          facecolor='none')
            ...
            pos = torch.Tensor([(self.target_tl[1] + self.target_br[1]) / 2, (self.target_tl[0] + self.target_br[0]) / 2])
            target_sz = torch.Tensor([self.target_br[1] - self.target_tl[1], self.target_tl[0] - self.target_br[0]]).abs()

            init_box = self.atom.get_iounet_box(pos, target_sz, self.atom.sample_pos, self.atom.sample_scale)

            # Extract features from the relevant scale
            iou_features = self.atom.get_iou_features()
            iou_features = TensorList([x[0:1, ...] for x in iou_features])

            init_boxes = init_box.view(1, 4).clone()

            # Refine boxes by maximizing iou
            output_boxes, output_iou = self.atom.optimize_boxes(iou_features, init_boxes)

            output_boxes = torch.cat([init_boxes, output_boxes])
            output_iou = torch.cat([torch.Tensor([0]), output_iou])

            self.atom.visdom.register((self.atom.test_patch, output_boxes, output_iou), 'proposals', 2,
                                      'Final boxes', {'disp_sz': (1000, 1000)})

            self.rect.set_height(0)
            self.rect.set_width(0)

class ATOMv2(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        if self.params.debug >= 2:
            ...
            self.ui = UIControl(self.ax, self.fig, self)

            self.cidpress = self.fig.canvas.mpl_connect(
                'button_press_event', self.ui.on_press)
            self.cidrelease = self.fig.canvas.mpl_connect(
                'button_release_event', self.ui.on_release)
            self.cidmotion = self.fig.canvas.mpl_connect(
                'motion_notify_event', self.ui.on_motion)
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        test_x, test_patch = self.extract_processed_sample(im, self.pos, sample_scales, self.img_sample_sz)

        # TODO fix this
        test_patch = test_patch[0].int()
        self.test_patch = test_patch
        self.sample_pos = sample_pos
        self.sample_scale = sample_scales[0]
        ...
        if self.params.debug >=2:
            ...
            self.visdom.register(s[scale_ind,...], 'heatmap', 2, 'Score')
    ...
    def extract_sample(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        feat, patch = self.params.features.extract(im, pos, scales, sz, return_patches=True)
        return feat, patch
    ...
    def extract_processed_sample(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor) -> (TensorList, TensorList):
        x, test_patch = self.extract_sample(im, pos, scales, sz)
        return self.preprocess_sample(self.project_sample(x)), test_patch
    ...
    def refine_target_box(self, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        if self.params.debug >= 2:
            ...
            init_boxes = torch.cat([gt_box, init_boxes])

            self.visdom.register((self.test_patch, init_boxes.clone(), None), 'proposals', 2, 'Init proposals')

        # Refine boxes by maximizing iou
        output_boxes, output_iou = self.optimize_boxes(iou_features, init_boxes)

        if self.params.debug >= 2:
            max_ind = torch.argmax(output_iou[1:]).item()
            disp_boxes = output_boxes[[0, max_ind+1], :]
            disp_scores = output_iou[[0, max_ind+1]]
            self.visdom.register((self.test_patch, disp_boxes.clone(), disp_scores.clone()), 'proposals', 2, 'Final boxes', {'disp_sz': (1000, 1000)})
