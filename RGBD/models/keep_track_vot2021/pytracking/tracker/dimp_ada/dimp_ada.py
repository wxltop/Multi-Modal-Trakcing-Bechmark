...
class DiMPAda(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      ...
                                                                      self.img_sample_sz)
        search_area_bb = self.generate_search_area_bb(self.target_sz[[1,0]].view(1, 2) / (self.target_scale * self.params.scale_factors.view(-1, 1)))

        # Extract classification features
        test_x = self.get_classification_features(backbone_feat, search_area_bb=search_area_bb.to(self.params.device))

        # Location of crop
        sample_pos, sample_scales = self.get_sample_location(sample_coords)
        cls_sa_pos, cls_sa_scales = self.get_cls_search_area_location(sample_pos, sample_scales, search_area_bb[:, [3, 2]])

        # Compute classification scores
        scores_raw = self.classify_target(test_x)

        # Localize the target
        translation_vec, scale_ind, s, flag = self.localize_target(scores_raw, cls_sa_pos, cls_sa_scales)
        ...
        if update_flag and self.params.get('update_classifier', False):
            ...
            target_box = self.get_iounet_box(self.pos, self.target_sz, cls_sa_pos[scale_ind,:], cls_sa_scales[scale_ind])
            ...
            self.update_classifier(train_x, target_box_feat, learning_rate, s[scale_ind,...])
        ...
        if self.visdom is not None:
            self.visdom.register(F.relu(score_map), 'heatmap', 2, 'Score Map' + self.id_str)
    ...
    def get_classification_features(self, backbone_feat, search_area_bb):
        with torch.no_grad():
            return self.net.extract_classification_feat(backbone_feat, search_area_bb=search_area_bb)
    ...
    def init_classifier(self, init_backbone_feat, init_target_boxes):
        ...
        x = self.get_classification_features(init_backbone_feat, search_area_bb=search_area_bb)
        ...
        if 'dropout' in self.params.augmentation and self.params.get('use_augmentation', True):
            ...
            init_target_boxes = torch.cat([init_target_boxes, init_target_boxes[:1, :].expand(num,-1)], dim=0)
            search_area_bb = torch.cat([search_area_bb, search_area_bb[:1, :].expand(num,-1)], dim=0)
        ...
        if self.params.get('window_output', False):
            raise NotImplementedError
            # Need to fix shifting of target center due to padding
        ...
        # TODO which is the best place to do this?
        norm_factor = x.shape[-1] / search_area_bb[:, 2:3]
        ...
        with torch.no_grad():
            self.target_filter, _, losses = self.net.classifier.get_filter(x, init_target_boxes_norm, num_iter=num_iter,
                                                                           compute_losses=plot_loss)

        # Init memory
        if self.params.get('update_classifier', True):
            self.init_memory(TensorList([x]), init_target_boxes_norm)
