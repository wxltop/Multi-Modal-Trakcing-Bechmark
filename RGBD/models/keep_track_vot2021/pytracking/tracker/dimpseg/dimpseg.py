...
from ltr.models.layers.transform import interpolate
from ltr.models.segmentation.seg2box import bbox_from_edge_scores


class DiMPseg(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if flag != 'not_found':
            if self.use_iou_net:
                ...
                self.segment(sample_pos, sample_scales[scale_ind], scale_ind)
    ...
    def get_segmenter_features(self):
        feat = self.params.features.get_unique_attribute('segmentation_features')
        return feat
    ...
    def init_segmenter(self):
        # Setup IoU net and objective
        self.segmenter = self.params.features.get_unique_attribute('segmenter')
        self.bb_regressor = self.params.features.get_unique_attribute('bb_regressor')

        # Get target boxes for the different augmentations
        self.classifier_target_box = self.get_iounet_box(self.pos, self.target_sz, self.pos.round(), self.target_scale)
        target_boxes = TensorList()
        if False: #self.params.segmenter_augmentation:
            for T in self.transforms:
                if not isinstance(T, (augmentation.Identity, augmentation.Translation, augmentation.FlipHorizontal, augmentation.FlipVertical, augmentation.Blur)):
                    break
                target_boxes.append(self.classifier_target_box + torch.Tensor([T.shift[1], T.shift[0], 0, 0]))
        ...
        # Get iou features
        segmenter_feat = self.get_segmenter_features()

        # Remove other augmentations such as rotation
        segmenter_feat = OrderedDict({lname: x[:target_boxes.shape[0],...] for lname, x in segmenter_feat.items()})

        # Extract target feat
        with torch.no_grad():
            self.seg_modulation = self.segmenter.get_modulation(segmenter_feat, target_boxes)
        # self.seg_modulation = TensorList([x.detach().mean(0) for x in target_feat])
    ...
    def init_classifier(self, x):
        ...
        if self.classifier_net_name == 'LinearFilter':
            ...
            with torch.no_grad():
                self.target_filter, _, losses = self.target_classifier.get_filter(x[0], target_boxes, bb=target_boxes,
    ...
    def update_classifier(self, train_x, train_y, target_box, learning_rate=None, scores=None):
        ...
        if self.classifier_net_name == 'LinearFilter' and self.params.get('update_strategy', 'default') == 'default':
            ...
            if num_iter != 0:
                ...
                with torch.no_grad():
                    self.target_filter, _, losses = self.target_classifier.filter_optimizer(self.target_filter, samples, bb=target_boxes, label=labels,
    ...
    def segment(self, sample_pos, sample_scale, scale_ind):
        seg_feat = self.get_segmenter_features()
        with torch.no_grad():
            segmentation = self.segmenter.predict_segmentation(self.seg_modulation, seg_feat)
        segmentation = interpolate(segmentation, self.img_support_sz.long())

        bb_resp = self.bb_regressor(segmentation)
        bbox = bbox_from_edge_scores(*bb_resp)

        show_tensor(torch.sigmoid(segmentation), fig_num=35)

        predicted_box = bbox.view(-1).cpu()
        predicted_box[2:] = predicted_box[2:] - predicted_box[:2]
    ...
    def optimize_boxes(self, iou_features, init_boxes):
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            # Update proposal
            if self.params.get('iounet_use_log_scale', False):
                if self.params.get('iounet_update_aspect_ratio', True):
