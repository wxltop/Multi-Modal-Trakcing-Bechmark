...
class DiMPWeightLearning(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if flag != 'not_found':
            if self.params.get('use_iou_net', True):
                ...
                predicted_iou = self.refine_target_box(backbone_feat, sample_pos[scale_ind,:], sample_scales[scale_ind], scale_ind, update_scale_flag)
        ...
        # train_y = TensorList([torch.zeros((scores_raw.shape[2], scores_raw.shape[3]))])
        train_y = TensorList([torch.zeros_like(scores_raw)])
        ...
        if self.params.get('use_gt_box', False):
            ...
            if np.all(bbox_gth >= 0) and np.all(np.logical_not(np.isnan(bbox_gth))):
                if np.all(bbox_gth == 0) and update_flag == False:
                    pass
                else:
                    iou = bbutils.calc_iou(new_state, torch.from_numpy(bbox_gth))
                    self.ious.append(iou)
                    self.debug_info["IoU"] = self.ious[-1]
        ...
        anno_y = TensorList([torch.zeros_like(scores_raw)])
        ...
        if self.params.get('produce_tracking_data', False) or self.params.get('save_tracking_data', False):
            ...
            pmt, ptm = self.compute_projections(scores_raw, train_x_, train_y_, test_x)

            mem_y = self.target_labels[0][:self.num_init_samples[0]]

            predicted_iou = predicted_iou if predicted_iou is not None else torch.tensor([-1])

            if self.params.get('save_tracking_data', False):
                root_save_path = self.params.get('root_save_path')
                if self.frame_num == 2:
                    self.save_tracking_data(root_save_path, self.frame_num, self.seq_name, target_scores=scores_raw,
                                            target_label=train_y, anno_label=anno_y, anno_bbox=gth_box,
                                            pred_bbox=new_state, search_area_box=self.search_area_box,
                                            predicted_iou=predicted_iou, sample_weights=self.sample_weights,
                                            train_labels=mem_y, ptm=ptm, pmt=pmt)
                else:
                    self.save_tracking_data(root_save_path, self.frame_num, self.seq_name, target_scores=scores_raw,
                                            target_label=train_y, anno_label=anno_y, anno_bbox=gth_box,
                                            pred_bbox=new_state, search_area_box=self.search_area_box,
                ...
                self.save_update_flag(root_save_path, self.seq_name, self.frame_num, update_flag)

        if self.visdom is not None:
            self.max_target_scores.append(torch.max(scores_raw))
            max_vals, _ = torch.max(ptm.view(ptm.shape[0], -1), dim=1)
            self.max_proj_gth_peak.append(torch.mean(max_vals).view(1))

            self.visdom.register(anno_y[0][0, 0], 'heatmap', 2, 'Annotation Label Map' + self.id_str)
            ...
            self.visdom.register((torch.tensor(self.max_target_scores), torch.arange(len(self.max_target_scores))), 'lineplot', 3, 'Max Target Scores')
            self.visdom.register((torch.tensor(self.max_proj_gth_peak), torch.arange(len(self.max_proj_gth_peak))), 'lineplot', 3, 'Mean Max PTM')


            if self.params.get('use_gt_box', False) and len(self.ious) > 0:
                self.visdom.register((torch.tensor(self.ious), torch.arange(len(self.ious))), 'lineplot', 3, 'IoU')


        return out

    def save_update_flag(self, path, seq_name, frame_num, update_flag):
        save_path = os.path.join(path, seq_name.split('-')[0], seq_name, 'update_flag_per_frame.txt')

        if frame_num == 2 and os.path.exists(save_path):
            os.remove(save_path)

        with open(save_path, 'a') as f:
            val = 1 if update_flag else 0
            f.write('{}\n'.format(val))

    def compute_projections(self, scores_raw, train_x, train_y, test_x):
        wf, hf = train_x.shape[-2:]
        wl, hl = train_y.shape[-2:]
        n_mem = train_y.shape[0]
        nseq = train_x.shape[1]

        sim = compute_cosine_similarity(train_feat=train_x, test_feat=test_x.unsqueeze(1))
        sim = sim.reshape(nseq, wf * hf, n_mem, wf * hf).permute(2, 0, 1, 3)  # (M, N, W*H, W*H)

        train_y_down = F.interpolate(train_y, size=(wf, hf), mode='bilinear')  # (22,22)
        scores_raw_down = F.interpolate(scores_raw, size=(wf, hf), mode='bilinear')  # (22,22)

        pmt = proj_mem_to_test(sim, train_y_down).view(n_mem, 1, wf, hf)  # (M, 1, W, H)
        ptm = proj_test_to_mem(sim, scores_raw_down).view(n_mem, 1, wf, hf)  # (M, 1, W, H)

        pmt = F.interpolate(pmt, size=(wl, hl), mode='bilinear')  # (23,23)
        ptm = F.interpolate(ptm, size=(wl, hl), mode='bilinear')  # (23,23)

        return pmt, ptm

    def save_tracking_data(self, root_save_path, frame_number, seq_name, **kwargs):

        #                    target_score, target_label, anno_label,
        #                    anno_bbox, pred_bbox, search_area_box, predicted_iou, sample_weights, train_labels, ptm, pmt):
        '''
        Requirements:
        root_save_path: str
        frame_number: int
        ...
        target_score: (23x23)
        anno_label: (23x23)
        anno_bbox: in image img_coords: (4x)
        pred_bbox: in image img_coords: (4x)
        search_area_box: Field of view in image img_coords: (1x4)
        predicted_iou: float
        sample_weights: (Mx)
        train_labels: (15x23x23) store only at the beginning
        ptm: Scores projected to memory (Mx23x23)
        pmt: Memory projected to scores (Mx23x23)
        '''

        save_dir = os.path.join(root_save_path, seq_name.split('-')[0], seq_name, 'data')

        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        save_path = os.path.join(save_dir, '{:08d}.npz'.format(frame_number))
        ...
        np.savez_compressed(save_path, seq_name=seq_name, frame_number=frame_number, **d)
