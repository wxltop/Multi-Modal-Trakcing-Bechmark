...
from ltr.models.backbone.resnet_vggm import resnet18_vggmconv1, resnet50_vggmconv1

normalize = torchvision.transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                             std=[0.229, 0.224, 0.225])


class ResNetVGGm1(MultiFeatureBase):
    ...
    def __init__(self, output_layers, net_path=None, base_model='resnet18', use_gpu=True, *args, **kwargs):
        super(ResNetVGGm1, self).__init__(*args, **kwargs)

        assert base_model in ['resnet18', 'resnet50']
        ...
        self.net_path = '{0}_vggmconv1/{0}_vggmconv1.pth'.format(base_model) if net_path is None else net_path
    ...
    def initialize(self):
        ...
        self.layer_stride = {'vggconv1': 2, 'conv1': 2, 'layer1': 4, 'layer2': 8, 'layer3': 16, 'layer4': 32,
                             'fc': None}
        d = 1 if self.base_model == 'resnet18' else 4
        self.layer_dim = {'vggconv1': 96, 'conv1': 64, 'layer1': 64*d, 'layer2': 128*d, 'layer3': 256*d, 'layer4': 512*d,
                          'fc': None}
        ...
        net_func = {'resnet18': resnet18_vggmconv1,
                    'resnet50': resnet50_vggmconv1}

        self.net = None
        for net_path in net_path_full:
            try:
                self.net = net_func[self.base_model](output_layers=self.output_layers, path=net_path)
                break
            except:
                pass
        if self.net is None:
