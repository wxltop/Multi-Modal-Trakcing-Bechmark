...
class RegionDataset(data.Dataset):
    ...
    def __next__(self):
        ...
        for i, (vis_img_path, event_img_path, bbox) in enumerate(zip(self.img_list_vis[idx], self.img_list_event[idx], self.gt[idx])):
            image_vis = Image.open(vis_img_path).convert('RGB')
            image_vis = np.asarray(image_vis)

            image_event = Image.open(event_img_path).convert('RGB')
        ...
        return pos_regions_vis, neg_regions_vis, pos_regions_event, neg_regions_event 
