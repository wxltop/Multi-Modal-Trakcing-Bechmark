...
def train_mdnet(opts):
    ...
    for k, seq in enumerate(data.values()):
        dataset[k] = RegionDataset(seq['vis_images'], seq['event_images'], seq['gt'], opts)
    ...
    for i in range(opts['n_cycles']):
        ...
        for j, k in enumerate(k_list):
            tic = time.time()
            # training
            pos_regions_vis, neg_regions_vis, pos_regions_event, neg_regions_event = dataset[k].next()
            
            # print("==>> pos_regions_vis: ", len(pos_regions_vis))

            if len(pos_regions_vis) == 32: 

                if opts['use_gpu']:
                    pos_regions_vis = pos_regions_vis.cuda()
                    neg_regions_vis = neg_regions_vis.cuda()
                    pos_regions_event = pos_regions_event.cuda()
                    neg_regions_event = neg_regions_event.cuda()

                pos_score = model(pos_regions_vis, pos_regions_event, k)
                neg_score = model(neg_regions_vis, neg_regions_event, k)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--dataset', default='VisEvent', help='training dataset {vot, imagenet, VisEvent}')
