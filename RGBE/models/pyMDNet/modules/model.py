...
class MDNet(nn.Module):
    def __init__(self, model_path=None, K=1):
        ...
        self.branches = nn.ModuleList([nn.Sequential(nn.Dropout(0.5),
                                                     nn.Linear(512 * 2, 2)) for _ in range(K)])
    ...
    def forward(self, x_vis, x_event, k=0, in_layer='conv1', out_layer='fc6'):
        ...
        for name, module in self.layers.named_children():
            ...
            if run:
                x_vis = module(x_vis)
                x_event = module(x_event)
                # x = torch.cat((x_vis, x_event), 1) 

                if name == 'conv3':
                    # x = x_vis + x_event
                    x_vis = x_vis.view(x_vis.size(0), -1)
                    x_event = x_event.view(x_event.size(0), -1)
                if name == out_layer: 
                    # return torch.cat((x_vis, x_event), 1) 
                    return x_vis, x_event 

        x = self.branches[k](torch.cat((x_vis, x_event), 1) )
        if out_layer=='fc6':
            return x, x 
